---
description: Guided refactor workflow — pre-analyse, spec, approve, execute in waves, review gate. Accepts a file/path scope, an audit milestone file, or runs interactively.
argument-hint: [path/to/scope | g-docs/milestones/M-audit-*.md]
---

Use Glob to find `skills/g-refactor/SKILL.md` inside `~/.claude/plugins/cache/g-forge/g-forge/` and read it, then follow its instructions exactly. The user's argument (if any) is: $ARGUMENTS
