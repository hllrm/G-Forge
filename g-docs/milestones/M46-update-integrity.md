...
**Origin:** live consumer incident, 2026-07-23, `onlygian/G-Cash` — the first post-v2.3.0 downstream update. Developer ran `/g-update` before the manual `/plugins` marketplace update: the cache still held 2.2.1, so `/g-update` "realigned" the project with old files while presenting as an update ("weird, not sure what happened"). After the manual cache update the second run went fine. Same class visible on the G-Forge repo itself: `workflow-checkpoint.sh` printed "update available: 2.3.0 → 2.2.1" — a backwards advisory, because the check is not direction-aware. Neither failure corrupts, but both misrepresent state on the one path every consumer walks at every release.
