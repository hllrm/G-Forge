...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | synced to `a192670` · M40 + ADR-004/005 in · G-Tools 5-bug report triaged (#21–#24) @ v2.2.1 (v2.2.2 pending milestone close)
⚠ FIRST (fresh session): verify **ADR-004** (sentinel↔tree binding) and **ADR-005** (worktree enforcement) against the actual repo before writing any W1 code — clean-slate check. Key things to confirm: (a) a native git `pre-commit` hook is the right site and installs cleanly per-clone; (b) `git write-tree`/`git rev-parse --git-common-dir`/`--show-toplevel` behave as assumed on this platform (normalize relative-vs-absolute); (c) the review-flow follow-up (`/g-review` reviews `main...HEAD` but the sentinel gates the next staged commit) is resolved before coding; (d) per-worktree sentinel keying composes with ADR-004's stamp. **NEW: finding #21 (BUG-2, commit-gate false-positive) is folded into this same ADR-004 pass — read the "Wave 1 coordination" note in `g-docs/milestones/M-audit-2026-07.md` before touching `check-commit.sh` (one shared commit-detection routine for both PreToolUse + the new `pre-commit` site; atomic `/g-update` rollout; no downstream hand-patching).**
LATEST (2026-07-13): **Synced this machine to the cloud session's state** (`a192670` — M40 + ADR-004/005 pulled; manifest reverted to v2.2.1; this machine's stale parallel ADR-004/005 drafts + superseded handoff edit dropped after confirming the cloud versions supersede them). **Triaged a 5-bug report from the G-Tools retro corpus** into the M-audit ledger: **#21 (BUG-2, W1 — reproduced LIVE)** the commit gate false-*fires* on any command whose string merely contains `git commit` (a non-committing `grep`/`for` test string was denied; regex confirms `… && git commit -m z` fires) — the false-**positive** counterpart to #6/#7, un-fixable by loosening the regex; **folded into the ADR-004 pass** with the coordination note above. **#22 (BUG-3, W2)** the observer journals every agent as `unknown start/stop` — no `subagent_type`/task/RESULT — degrading /g-retro + /g-patterns. **#23 (BUG-4, W3)** g-doctor Check 21 stray-doc scan uses too narrow a dir allowlist. **#24 (BUG-5, W3)** computer-use `request_access` can't grant unregistered running binaries → doc-gap fallback in g-rules-H for native Tauri Tier-3 tests. **BUG-1 was NOT a source bug** — the doc-sentinel leak is already fixed in `f55ccdb`; G-Tools hit a **stale installed hook** → its fix is `/g-update` (live field instance of drift-finding #5, logged as 1f). All ledger edits in `g-docs/milestones/M-audit-2026-07.md`, **uncommitted**. **NEXT unchanged: verify ADR-004/005, then write the W1 code — #21 rides that same pass.**
