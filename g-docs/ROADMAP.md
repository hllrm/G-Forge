...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | ADR-013 VERIFIED · README 2.5 REWRITE DONE · 2026-08-18
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **ADR-013 verified against disk — all claims hold** (clean-slate check per §C, first task of the 2026-08-18 handoff): coverage-table defect real (`skills/g-telemetry/SKILL.md:92-108` table = 17 rows vs 19 files in `agents/`, missing exactly `doc-reviewer` + `feature-implementer`; nuance for M50 — the prose list at `:67` is complete at 19, only the TABLE is the defective typed list); `g-docs/telemetry/2026-08-17.md` has 0 `doc-reviewer` mentions; Group D at `tests/test-lib-install-completeness.sh:181`; `profiles/` = 56 dirs. Verification written to `.claude/reentry.md`. · **`/g-trim` run to completion for the first time** (`.claude/last-trim` = 2026-08-18): all 4 CLAUDE.md `@`-imports (plus the 10 G-RULES section imports) and all 24 agent-memory index links resolve; flagged only the known `CLAUDE.md:32` stale date + doc-reviewer memory `test-count-verification.md` carrying pre-564 counts (523/15-era). · **README 2.5-statement rewrite done** (per intent recorded in commit `2591c14`): freeze block compressed 4→2 paragraphs; review-gate bullet now cites the measured result (11 tool calls / 79k tokens / 1 round, caught a real blocker, vs ~3h / ~130k killed — figures verified against `g-docs/retros/2026-08-17-m50-scope-and-scoped-review.md`); M50 "Instruments that measure what they claim" bullet added. · Retro `g-docs/retros/2026-08-18-adr-013-derive.md` committed with this pass.
Next up:          · **`/g-plan` M48** (suite-runtime + suite-ordering fixes; fold todo task 5 riders; task 10 heredoc misclassification is an M48 candidate; task 11 scoping riders feed M45's design ADR). · M43 W4 governance-cadence ADR — before M43's build (slot 6, not before M48). · CARRIED: n8n slugify on the n8n side · ROADMAP status emoji deliberately kept · `commands/g-forge.md:8` env-specific cache path · inbox trust-boundary ADR · M38 delivery decision · intake rows (a)-(h) · task 6 M45 rider · task 7 remainder · `CLAUDE.md:32` date stamp (local gitignored file — per ADR-013 make it derivable or drop it; left untouched this pass) · doc-reviewer memory stale-count entry (manual curation).
Active context:   · **ADR-013 governs doc edits here** (verified in force 2026-08-18): consumers derive their lists; documents keep concrete numbers, pinned or omitted — and do not hand-type file:line/count claims not verified the same turn. · M50 ground truth (re-verified this pass): `/g-telemetry` coverage TABLE types 17 rows vs 19 in `agents/` (prose list at `SKILL.md:67` is complete — the table template is the defect); `g-specialize` frontmatter list = 48 typed (47 stacks + claude-plugin, count verified) vs 56 profile dirs, intended set still UNSTATED — settle in `/g-plan` M50; replace the `:167`-area explicit-arg consumer with a directory test. · End-to-end done condition: add an agent, run `/g-telemetry`, the persisted report must count it. · ADR-004: sentinels bind to the STAGED tree — stage, then stamp `commit_sentinel_ts=<write-tree> commit_sentinel_head=<HEAD> commit_sentinel_worktree=<toplevel>`, then commit as a separate call; commit messages via a Write-tool file, never a heredoc. · LOCAL STATE: review-holds 25 · voice `gian` · telemetry `cautious` · suite 564/18 attested 2026-08-17 · last-trim 2026-08-18. · Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass, and never repeat this line's own leading label text anywhere else on it (`workflow-checkpoint.sh` strips with a greedy BRE through the LAST occurrence; `pre-compact.sh` uses awk block capture — the two disagree when violated).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
