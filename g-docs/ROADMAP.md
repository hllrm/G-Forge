...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · ADR-011 (injection rule) COMMITTED d29d41e, unpushed · Check 24 specced, not built (2026-07-27)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **ADR-011 WRITTEN AND COMMITTED `d29d41e`** (2 files: `g-docs/decisions/011-inject-claude-md-from-committed-sources.md` NEW 96 lines + `.gitignore` comment-only currency fix, **unpushed**; `b61191a`/`b8684f7` confirmed pushed 2026-07-26 21:46Z) through both gates: `/g-review` **HOLD then MERGE READY on re-review** (attested suite via `g-forge-dev`: 17/17 exit 0, **523 passed / 0 failed**, table awk-summed independently; HOLD items = two `.gitignore` cites the ADR's own commit moved (`:54`→`:56`, `:22-53`→`:22-55`) + Check 24 asserted in present tense while `/g-doctor` ships 23 checks — the ADR committing its own sin, both gates caught it) and `/g-doc-review` **DOCS HOLD then DOCS READY on r2** (3 refuted facts fixed: T1 was NOT lost 2026-07-10 — created 2026-07-26 per `ADR-010:61`; "untracked ⇒ undetectable" refuted by tracked `tests/test-workflow-checkpoint.sh:12` carrying the same 80-vs-81 staleness; Check 16 never reads CLAUDE.md — payload comparison assigned to the detector build instead). · **THE INJECTION RULE IS NOW CANONICAL IN THE COMMITTED RECORD** — `.gitignore`'s block demoted to "warning at the point of temptation"; scope SPLIT recorded (gitignored-CLAUDE.md half = this repo only; three-branch sourcing rule ships; `/g-init` Step 5a untouched). · **BRANCH (c) HAS A CHECKABLE ARTIFACT**: `<!-- G-Forge local-only: <slug> -->` … `<!-- End G-Forge local-only: <slug> -->`, slug = pairing key, NO reason field (deliberate — syntax classifier, not rationale auditor). Survives `/g-update` because it matches two literal labels only (`skills/g-update/SKILL.md:114-115`, `:160-172`, `:191`, `:333`) — that vocabulary must NEVER grow to match `G-Forge local-only:`. Declared-block count TODAY = 0; the voice-profile section is the one sanctioned instance and is NOT yet wrapped — wrapping it is part of the detector build. · **CHECK 24 SPECCED IN ADR-011, NOT IMPLEMENTED** — advisory (✓/⚠), read-only per ADR-009, four buckets MARKER-FED / IMPORT / DECLARED-LOCAL / BARE-PROSE + unpaired/unterminated findings. Advisory because it will flag most of this repo's own CLAUDE.md prose on day one and there is no git baseline to split old debt from new regression (Check 23 rollout pattern: introduce → surface debt → promote). · **DEVELOPER DECIDED: detector-first, backlog after** — ship Check 24 + `/g-trim` import-follow, let day-one warnings BE the backlog list; cleanup is its own later pass. · `/g-trim` gap re-sized DOWN: it already Globs every `@path` for existence (`skills/g-trim/SKILL.md:14`) — missing piece is following the import to audit target content (T1 sunset detection), a Step-1 extension in a 78-line file, not a scope-add. · **Premortem (two-way door, inline)**: branch (c) as laundry chute — wrap anything awkward, check goes green, defect continues behind a compliance marker; earliest warning sign = a NUMBER or COUNT inside a local-only block (folded into ADR-011 Risks). · Voice: **TL;DR-only chat format saved as standing preference** (memory: tldr-only-response-format) — compressed bullets, he pulls detail; files still get full prose.
Next up:          · **PUSH `d29d41e` + this handoff commit.** · **THEN: `/g-plan` the Check 24 build** — detector-first scope: `skills/g-doctor/SKILL.md` (Check 24 body + count sweep `:3`/`:10`/`:341` + report template) · `README.md` ×3 count sites (`:236`, `:288`, `:549` — `23 checks (16+7)` → `24 (16+8)`) · `skills/g-trim/SKILL.md` Step-1 import-follow · `CHANGELOG.md` · wrap this repo's voice-profile CLAUDE.md section in the local-only marker (takes declared count 0→1, arms the tripwire baseline). **Spec riders from review, fold into the build:** (i) bucket precedence when an `@`-import sits INSIDE a local-only block (classify as DECLARED-LOCAL, else Follow-up 6 flags the one legitimate (c) instance as dangling); (ii) `git check-ignore` per `@`-target — an import of a gitignored file must not classify clean IMPORT (ADR-011 Follow-up 6); (iii) slug re-open handling (Follow-up 2); (iv) unterminated-marker EOF state (Follow-up 3); (v) CLAUDE.md marker-payload drift comparison is covered by NO check today — fold into build (was mis-assigned to Check 16, corrected at doc gate). No test currently pins the check count (the `23`s in `test-g-doctor-drift.sh` are M-audit finding numbers). · **THEN THE ONE GATE, one discussion, roadmap written once** — queue unchanged from the 2026-07-26 handoff, in force: (1) R0 shape (tightening-pass cadence · what process governs G-Proof's construction · fork checklist incl. wiring the T1 `@`-import · GF-20 spike · GF-23 verification, two rows not four · GF-70 test (seeded — narrow reading operative: matcher, not missing bash) · ADR-008 Spike S1 in a separate repo · suite-erosion defence, direction-agnostic, derived from the run · scoped breakdown + cost counter-estimate). (2) M-FORGE/M-PROOF split by changeset class — `Track:` field + `.claude/active-track`, NEVER two `## Active Session` blocks (`workflow-checkpoint.sh:82` greedy strip); R0 spikes run in a throwaway spike clone or the doc-only property collapses. (3) Fork timing — recommended at the freeze; post-freeze sync = cherry-pick-forward per bug fix only. (4) 8 Discuss rows (GF-20/23 · GF-60/61 · GF-63/77/78). (5) client/cockpit milestone candidate. (6) `/g-intake` ×3 (telemetry fix-loop · TDD+doc-pass · g-train deepening). (7) three-horizon re-scope AFTER the rebuild plan (M44/M41 stamps stay placeholders). (8) M44 detail calls + weekly platform-sync + GF-40 ADR + gate-hook clone-first checkpoint. · **INTAKE ROWS (a–g) carried unchanged** from the 2026-07-26 handoff — (a) `/g-patterns` derivable-counts · (b) injection-rule teeth — Check 24 now specced in ADR-011, remaining: generated Quick-commands + marker block for project prose · (c) self-safe/at-risk state (journal/GF-73 purge vector) · (d) reviewers write their own sentinels (ADR-shaped) · (e) consumer sentinel-write permission denial · (f) user-defined voice profiles · (g) per-prompt injection > loaded-once document finding. · **ADVISORY WARNINGS: five from 2026-07-26 still live** (`project_brief.md:37` · `M44.md:3` · `test-classify-changeset.sh:11` vs `:13` · `transitional-rules.md:13` · checkpoint spent-entry nudge) **+ new advisories in this pass's review records** (`g-docs/agent-output/review/*2026-07-27*`): ADR-011 `:12` cites test line "declared 80" now reading 81 without a since-fixed signpost · `:40` scope-of-intent present tense · `:20` marker examples abbreviate the injected tail · `:84` line-cite into gitignored CLAUDE.md · bucket-precedence minor. None blocking. · **STILL OPEN, developer's call:** what lands in v2.5 before the freeze (currently only M41) · what earns a `2.5.x` patch vs "fixed in G-Proof". · Rider in `g-docs/todo.md`: g-wiki M46 refresh.
Active context:   · `main` @ v2.4.0, clean tree, **`d29d41e` committed and UNPUSHED** (`b61191a`/`b8684f7` pushed). Both gate sentinels consumed by `post-commit-cleanup.sh`. **ADR-010 VERIFIED and ADR-011 LANDED — build on both, re-verify neither; the checkpoint's "a handed-off ADR needs verifying" nudge is a spent-entry false positive (`workflow-checkpoint.sh:376` fires on stale ROADMAP text and never names an ADR).** Injection rule canonical at `g-docs/decisions/011-inject-claude-md-from-committed-sources.md`; `.gitignore` block is the point-of-temptation warning only. **Check 24 does NOT exist yet** — `/g-doctor` ships 23 checks; the spec lives in ADR-011. Suite = **523 / 17 suites / green**, attested this pass by `g-forge-dev` — always awk-sum the run's own `Results:` lines, never hand arithmetic, never prose. **Version identity: G-Proof, first release 1.0 — never `3.0`.** Arc order PENDING the gate; M44/M41 stamps are placeholders until the post-rebuild-plan re-scope. LOCAL, NON-COMMITTED STATE inherited silently (all gitignored): `.claude/settings.json` shell matcher = `Bash|PowerShell` · `.claude/hooks/workflow-checkpoint.sh` at source parity · `.claude/hooks/check-commit.sh` deliberately BEHIND source (ADR-008 hold — a decision, not drift; `/g-review`'s drift line reports it every run) · `.claude/voice-profile` = `gian` + voice `@`-import at the bottom of CLAUDE.md outside the markers (NOT yet wrapped in the local-only marker — detector-build task) · caveman off · `.claude/review-holds` = 6. REPORT-VS-REPO CORRECTIONS baked in (don't re-derive): 8 hook scripts + hooks.json + 6 libs, 19 agents, 38 skills, single router (ADR-007), 56 profile dirs, 23 doctor checks, SubagentStart/Stop registered. Backlog riders: g-doctor Check-16 lib enumeration stale · `command -v` hardening · fixture sleep-leak (serial full suite ~9.6 min) · per-session counter accumulation · ADR-006 implementation. Format note: keep this line's leading label intact, use exactly the three §I labels (done-this-pass, next-up, active-context), replace this block **wholesale** each pass — never append a second generation. **Never repeat this line's own leading label text anywhere else on it:** `workflow-checkpoint.sh:82` strips with a greedy BRE through the LAST occurrence, so a repeat makes the per-prompt banner serve prose instead of state (observed live 2026-07-26); constraint is once per line — `pre-compact.sh:74` uses awk block capture and is unaffected, so the two consumers disagree when violated.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
