...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | W1.5a–W1.5g + W1.6 SHIPPED (all reviewed MERGE READY, committed) @ v2.2.1 (v2.3.0 pending milestone close — upgraded from v2.2.2, developer 2026-07-18)
⚠ FIRST: **W1.5g Pass 3 (Spike S1) in a FRESH session — now IN SCOPE for W1 close (developer 2026-07-22: non-blockers are in): plan `g-docs/plans/w1-5g-self-host-integrity.md` Waves 6–7 (local-marketplace dev-install probes + report; MANDATORY plugin-cache-state restore after). Then W1 CLOSES.** Next after: **ADR-007 fresh-window verify → /g-plan W2** (finding #19 shim retirement — sequencing constraint: amend g-skill-validate/g-skill-design/architecture rule BEFORE deleting shims; plus the W2-queued residuals on the W1.7ii ledger row: #21 heredoc-content false-positive look, journal `source`-field polish, PostToolUse-on-error characterization) → W3 → **release pass: v2.3.0 at FULL M-audit close (W1+W2+W3 — developer 2026-07-22, supersedes the 2026-07-18 at-W1-close call; one milestone, one version)** with README status strip + CHANGELOG `[2.3.0]` cut. THE GATE IS LIVE AND BATTLE-PROVEN (see LATEST 2026-07-22b). Rollback baseline: `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-22\`. On lockout: `--no-verify`/hook-delete, restore snapshot, never debug live in-session. Do NOT re-plan W1.1–W1.7. **ADR-006 implementation stays POST-M-audit.** stash@{0} superseded — droppable. Session retro for this pass UNCOMMITTED — rides next doc commit.
