...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · Check 24 shipped · v2.5 scope + narrative SETTLED, roadmap re-scope pending (2026-07-28)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **CHECK 24 SHIPPED** `91a36b8` + pass-close `b014ffe`, both pushed (see the pass report in `g-docs/todo-done.md` for the build and its 4-round gate). · **v2.5 SCOPE AND NARRATIVE SETTLED IN DISCUSSION — not yet in the record.** Version stays **2.5.0** (minor, not 3.0): nothing agreed is breaking, and ADR-010's identity clause holds — the 2.x line ends and versioning restarts at G-Proof 1.0, so a 3.0 here would make the successor read as a downgrade. **ADR-010 needs NO amendment** — an earlier "2.5 is no longer a freeze, it's standalone-viable" framing was a wrong inference by HQ and is withdrawn; the freeze premise stands exactly as written. · **AGREED 2.5 CONTENT, in build order:** (1) bug sweep — consumer sentinel-write denial, `command -v` hardening, Check-16 lib enumeration, the five live advisories; (2) **task-decomposer optimization** — own slot, own scoping session, NOT folded into M45; forecast miss-risk calibration follows it here as planning-side work; (3) **M45 review-pipeline rework, review-only** (scope narrowed back after the decomposer split out); (4) **M43 wave 1 `/g-settings`** only; (5) **M38 G-Report, REDESIGNED** — hook-triggered when something actually breaks plus on-demand, asks what to report from a menu, scrubbed incident template, delivered via git (this redesign is NOT yet recorded anywhere — the ROADMAP still carries the old M38 scope); (6) **M41 release machinery**, last, cutting 2.5 itself. Smalls alongside: M40 wave 1 mis-gating fix, M42 cold-start questions, fixture sleep-leak, the Check-24 bare-prose backlog. · **DEFERRED TO G-PROOF, decided:** salience (M36/M37), memory forge (M35), multiplayer arc (M29–M34), provable dispatch (M26), benchmark (M25), ADR-006 — each is a layer that must thread through everything, so retrofitting then rebuilding pays twice. · **M39 G-tweak → G-PROOF** on three independent grounds: its safe self-tune half duplicates `/g-settings`; its valuable half (de-gating) needs the salience model; and the G-Report redesign removes its remaining job as G-Report's caller. Interview-only was judged not worth building for a handful of users. · **COMMUNICATION PLAN WRITTEN — `g-docs/communication-plan-2.5.md`** (new — lands with this pass-close commit): the freeze narrative, the G-Proof hint, the 2.5 feature copy, placement map, and the boundaries on what must never be said (no date, no feature list, no 3.0, no claim about how G-Proof gets built). **Copy is developer-approved; the file is marked DRAFT because its §3c scope is not yet in `ROADMAP.md`.** It gated **DOCS HOLD (3 blocking · 8 warning)** and its own §7 records every finding — read §7 before touching it.
Next up:          · **STEP 0 — FIELD BUG, verified, fix first: `/g-init` installs 4 of 6 `hooks/lib` scripts.** Full verified writeup in `g-docs/todo.md` task 2 — cites the three skill lists **plus six `README.md` sites** (`:165`, `:246`, `:300`, `:307`, `:503-506`, `:561`) that carry the same stale four-lib list and must move in the SAME commit, or README ends up contradicting shipped behaviour. The field report's Bug-B count is corrected there to 2 unguarded sites, not 4 (`observe.sh:29-30` has a shim its audit missed). Ships broken in 2.4.0: fresh `/g-init` omits `stdin-read.sh` + `semver-compare.sh`, `/g-update` won't heal it, `/g-doctor` won't flag it, and `session-start.sh:23` + `workflow-checkpoint.sh:29` emit `command not found` on every session start and every prompt. Commit gate unaffected (`check-commit.sh:96` has the `cat` fallback). Sharpens the two carried riders (Check-16 enumeration, `command -v` hardening) — the new half is that the INSTALL is short, not just the check. Third fix part is the durable one: a test asserting every `lib/*.sh` a hook sources appears in g-init's Step 6 table. This is slot 1 of the 2.5 bug sweep. · **STEP 1 — `/g-intake` on task-decomposer optimization** (the agreed next action; scope it, then it becomes slot 2 of the 2.5 build order). Evidence from the 2026-07-28 session, use it: the decomposer returned an EMPTY final message and needed a resume to recover its own task list, and it over-split into 11 tasks where wave-planner immediately collapsed 5 into 1 — same-file sequential edits split across phantom agents. Two defects: task sizing (don't split a serial single-file chain) and unreliable result return across the seam. Fold forecast miss-risk calibration into the same scoping conversation — same pipeline, and the standing complaint is that the number reads high/static and gets ignored. · **STEP 2 — WHOLE-SYSTEM AUDIT at top-tier reasoning (Fable, xHigh/max), developer-requested 2026-07-28.** Writeup in `g-docs/todo.md` task 3. Runs AFTER the bug sweep so every finding is new, and BEFORE the roadmap commits 2.5's full content, since a wide audit is the one thing likely to change what 2.5 must contain. Rationale: task 2 was found by a human hand-walking an install list, survived every gate and a full release, and the riders that half-knew about it never got connected to a broken install — a detection gap diff review structurally cannot close, because the defect was never in a diff. · **STEP 3 — the roadmap re-scope pass.** It records the settled scope above and closes all three of the communication plan's blocking findings at once: (a) `ROADMAP.md` still assigns M45→v2.6.0, M38→v2.12.0, M40→v2.14.0, M43→v2.15.0 and carries no decomposer entry, while `ROADMAP.md:750` (the M44 supersession stamp), `ROADMAP.md:799` (the Version Plan) and `project_brief.md:68` all still say "only M41" is the v2.5 candidate; (b) the M38 redesign must land in the record, **including a decision on delivery — the approved copy says "hands you a file to send" while the redesign was briefed as via-git, and one of the two is wrong**; (c) the communication plan needs inbound pointers from the roadmap's 2.5 entry and a precondition line in `g-docs/milestones/M41.md`, or the session that cuts the release will never find it. Re-gate the communication plan after. · **DEVELOPER DECISIONS STILL OPEN**, all recorded in `communication-plan-2.5.md` §7: the "four other projects" count (the repo can't arbitrate it, and `alveria` is recorded as a third-party adopter's fork rather than a project you run) · whether to restore ADR-010:41's full three-cause set in the freeze narrative or record the narrowing as deliberate · announcement block order (§4's row breaks the 3a→3b adjacency the plan itself calls load-bearing) · whether the README also carries the 2.5 feature list. · **INTAKE ROWS carried:** (a) `/g-patterns` derivable-counts · (b) injection-rule teeth — Check 24 shipped; the 50-finding bare-prose backlog remains and is the advisory→required promotion gate · (c) self-safe/at-risk state · (d) reviewers write their own sentinels · (e) consumer sentinel-write denial — **now slot 1 of the 2.5 build order** · (f) user-defined voice profiles · (g) per-prompt injection finding · (h) untrusted-content gates as a standing rule for any model-executed check that shells out. · **Advisories live:** `project_brief.md:37` · `M44.md:3` · `test-classify-changeset.sh:11` vs `:13` · `transitional-rules.md:13` · checkpoint spent-entry nudge.
Active context:   · `main` @ v2.4.0, `91a36b8` + `b014ffe` pushed; this pass-close commit and its push are PENDING at pass end. **`/g-doctor` ships 24 checks (16+8)** — Check 24 advisory and injection-hardened, declared-local count = 1. ADR-010 VERIFIED and unamended, ADR-011 LANDED AND CURRENT — build on both, re-verify neither; the checkpoint's "a handed-off ADR needs verifying" nudge stays a spent-entry false positive (`workflow-checkpoint.sh:376`). Suite = **523 / 17 suites / green**, attested 2026-07-28 by `g-forge-dev` — always awk-sum the run's own `Results:` lines, never hand arithmetic, never prose. **Version identity: 2.5.0 is the last G-Forge feature release and a deliberate freeze; G-Proof ships as 1.0 — never `3.0`.** The roadmap's arc order and its v2.6–v2.15 version stamps are STALE pending the re-scope pass; `M44`/`M41` stamps stay placeholders. LOCAL, NON-COMMITTED STATE (all gitignored): `.claude/settings.json` shell matcher = `Bash|PowerShell` · `.claude/hooks/workflow-checkpoint.sh` at source parity · `.claude/hooks/check-commit.sh` deliberately BEHIND source (ADR-008 hold — a decision, not drift; the drift line reports it every run) · voice profile `gian`, its CLAUDE.md section wrapped in `<!-- G-Forge local-only: voice-profile -->` markers · caveman off · `.claude/review-holds` = 8. REPORT-VS-REPO CORRECTIONS (don't re-derive): 8 hook scripts + hooks.json + 6 libs, 19 agents, 38 skills, single router (ADR-007), 56 profile dirs, **24 doctor checks**, SubagentStart/Stop registered. Backlog riders: Check-16 lib enumeration stale · `command -v` hardening · fixture sleep-leak (serial full suite ~9.6 min) · per-session counter accumulation · ADR-006 implementation · Check-24 bare-prose backlog. Format note: keep this line's leading label intact, use exactly the three §I labels, replace this block **wholesale** each pass — never append a second generation, and never repeat this line's own leading label text anywhere else on it (`workflow-checkpoint.sh:82` strips with a greedy BRE through the LAST occurrence; `pre-compact.sh:74` uses awk block capture — the two consumers disagree when violated, observed live 2026-07-26).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
