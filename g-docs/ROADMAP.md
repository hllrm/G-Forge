...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | **STDIN-GUARD PASS 1 DONE (Waves 1–3 of 5, uncommitted by design) — release HELD for guard, ships v2.3.0 after Pass 2** @ v2.2.1
⚠ FIRST: **/g-resume → /g-execute 4 — stdin-guard Pass 2** — (1) Wave 4: eager-install batches A/B/C (`hooks/{lib/stdin-read.sh,observe.sh,post-commit-cleanup.sh,agent-lifecycle.sh,workflow-checkpoint.sh,session-start.sh,pre-compact.sh}` → `.claude/hooks/`, byte-identical; **check-commit.sh explicitly NOT eager-installed** — gate cadence). (2) Wave 5 terminal: g-forge-dev FULL attestation — 10 suites (NEW test-stdin-read.sh 10 asserts + observe + post-commit-cleanup + agent-lifecycle + workflow-checkpoint + session-start + pre-compact + class-split 26→38 + check-commit 22→25 + g-doctor-drift) — **runner output pasted, HQ sums the table independently (claim-vs-data recurrence #3 guard)**; expected new total ≈ 468+10+12+3 = 493 (RECOUNT at attest, don't trust this arithmetic). (3) `/g-review` → gated commit (13 dirty items = exactly plan scope; MIXED class ⇒ both sentinels). (4) THEN v2.3.0 release pass: README status strip + CHANGELOG `[2.3.0]` cut (stdin-guard Fixed entry already under Unreleased) + dual version bump plugin.json+marketplace.json same commit + ADR-007 migration check + push (9+ ahead). (5) `/g-retro` (milestone) + close swarm: `/g-patterns` (claim-vs-data #3; resumed-agent stall adjudication), `/g-telemetry`, `/g-align` (nudge outstanding), ADR prompt; `/g-intake` M36 note `g-docs/milestones/M36-salience-inputs/2026-07-22-review-cost-scaling-feedback.md`; forecast Outcome reconcile ×2 (`m-audit-w3-p2-minors.md` + `hook-stdin-hang-guard.md`). Developer decisions on record (2026-07-22): **HOLD release until guard lands** (hotfix-class rider); **check-commit stdin-timeout polarity = FAIL-OPEN** (native pre-commit backstops; hook fires on every Bash call). Plan+Progress: `g-docs/plans/hook-stdin-hang-guard.md` (gitignored scratch, table current). ADR-006 implementation stays POST-M-audit. Rollback baseline: `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-22\`.
