...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | M40 merged + M-audit #8/#9/#10 ADR'd @ v2.2.1 (v2.2.2 pending milestone close)
⚠ FIRST (fresh session): verify **ADR-004** (sentinel↔tree binding) and **ADR-005** (worktree enforcement) against the actual repo before writing any W1 code — clean-slate check. Key things to confirm: (a) a native git `pre-commit` hook is the right site and installs cleanly per-clone; (b) `git write-tree`/`git rev-parse --git-common-dir`/`--show-toplevel` behave as assumed on this platform (normalize relative-vs-absolute); (c) the review-flow follow-up (`/g-review` reviews `main...HEAD` but the sentinel gates the next staged commit) is resolved before coding; (d) per-worktree sentinel keying composes with ADR-004's stamp.
