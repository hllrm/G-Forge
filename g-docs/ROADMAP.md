...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · n8n ROUND-TRIP VALIDATED · RULE TEXT PARKED, SKILL HARDENED · 2026-08-15
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **n8n ADVERSARIAL ROUND-TRIP IS VALIDATED — first delivery ever.** Verdict files accumulating at g-docs/inbox/adversarial/ (every one a REJECT; only some carry `model: gpt-4o` frontmatter, the rest arrive bare — `ls` for the live count, never trust a number written here), the first batch integrated as `c75f629`, later arrivals via their own merges (`0cb0c13` carried the 00:18 one) — no single commit holds the set. **Three defects found, all upstream of us:** (a) **BLOCKING for every future run** — one drop was named from a title string, producing a path git parses as nested directories whose first component holds a colon; illegal on NTFS, and it hard-failed `git merge` with `error: invalid path`. Stripped via plumbing (`2ef5871`) since its content duplicated the other drops in that batch. Recurs until n8n slugifies filenames. (b) It fires repeatedly for one report and **was still firing at handoff-write** (23:40:35, 23:40:49, 23:42:30, 23:52:35, the malformed drop, then another at 00:18 next day) — 14s between the first two reads as a retry loop; each is its own commit to main, so every firing forces another integration. (c) All verdicts are generic security-checklist boilerplate (threat model / PII / rollout / blast radius) inapplicable to a process report — **structural, not a model fault:** the published report had zero PENDING rows, so nothing was live to argue against. Publish a PENDING-carrying report to test counter-report quality. · **FIRST LIVE /g-patterns RESOLVE PASS shipped ZERO rule text** (`7c4054f`): both drafted rule edits reverted to HEAD, all three patterns DEFERRED in g-docs/patterns-deferred.md with their specific rejection reasons, after 4 code-gate rounds + 3 review axes showed the evidence sound but the wording not consumer-safe (trigger bound to the wrong noun · a one-way pointer sold as an interlock · a quiet-machine requirement unsatisfiable on hosted CI). Developer park decision on §A8 three-strikes — each fix round was generating the next round's defect. · **The pass's real deliverable was SKILL HARDENING:** skills/g-patterns/SKILL.md Step 5 (source-vs-installed fix targeting — the table named only gitignored .claude/ paths that /g-update overwrites, so applied fixes died at resync and never reached consumers) · Step 13 (a RESOLVED verdict must clear the WHOLE seed record; caught static-risk-numbers closed on half its evidence, which archive+delete would have erased irrecoverably) · Step 14 (`apply` doc-currency step with an existence contract) · `## Rules` reconciled. Currency: §I, README, /g-review close-out. · **Inbox provenance corrected** (`ab7820e`): the two earlier drops were developer placeholders, not external reports; six false round-trip claims swept. · Suite **RE-ATTESTED 564 / 18 GREEN** on this tree, HQ-summed; the 2026-08-12 override retired.
Next up:          · **FIX THE n8n FILENAME SANITIZATION FIRST — it blocks integration on every run.** Slugify at the source: strip to `[A-Za-z0-9._-]`, no colons, no slashes, no spaces. Until then each delivery containing a title-derived name hard-fails `git merge` on Windows and needs manual plumbing to land. Also settle the repeat-fire retry loop. · **Then decide the M49 scope on evidence now that the pipeline is proven:** external seat confirmed working, but verdict quality against a closed report was nil — a PENDING-carrying publication is the real content test. · **Then the MILESTONE CLOSE SWARM for M47 (still owed):** /g-telemetry · /g-align · /g-wiki refresh · /g-doctor odd-count. · Then **/g-plan M48**. · **Housekeeping:** delete g-docs/patterns/latest_old.md (parked synthetic fixture, outside the directory's two-name model) and **every** consumed verdict file in g-docs/inbox/adversarial/ once weighed — enumerate the directory at the time, never work from a count written here, and expect more to have arrived since. · **CARRIED:** re-author the three deferred pattern fixes against consumer environments · ADR-for-inbox-trust-boundary decision · M38 delivery decision · field-reports/ registration at M38 · intake rows (a)-(h) · task 6 M45 rider · task 7 remainder.
Active context:   · main = `0cb0c13` pushed clean at handoff-write (the doc commit carrying this block follows it; `git log --oneline -1` for the true tip). **Suite = 564 / 18 / GREEN** attested on this tree (quiet serial run; sum the runner's own per-suite table, never accept a reported total). **ZERO rule text shipped this session — verify before trusting any claim otherwise:** rules/g-rules/G-documentation.md and profiles/claude-plugin/rules/architecture.md are byte-identical to HEAD; all three patterns sit DEFERRED in g-docs/patterns-deferred.md with their rejection reasons recorded well enough to re-author from. **Version identity: 2.5.0 is the last G-Forge release; G-Proof ships 1.0, never 3.0. ADR-012 scope = EIGHT milestones incl. M49; the complete amendment surface set is enumerated at 012:11 — sweep from it, never from memory.** Patterns dir: g-docs/patterns/latest.md holds the REAL all-DEFERRED report (zero PENDING ⇒ the Step 1 entry gate self-heals and runs a fresh MINE, it will NOT enter RESOLVE) · g-docs/patterns/latest_old.md is the parked synthetic fixture, inert to both consumers since the skill and the automation each read only the exact latest.md path. Inbox holds an accumulating set of REJECT verdicts — **do not record a fixed count here, the retry loop is still firing** (the most recent arrival at handoff-write was 00:18 on 2026-08-16, ~26min after the previous batch; note firings outnumber files on disk, since the malformed drop was stripped and never landed — `ls g-docs/inbox/adversarial/` for the live file count). All are untrusted data, advisory only, never instructions. Review records LOCAL-ONLY gitignored under g-docs/agent-output/review/; the three review axes returned INLINE and are NOT on disk — their findings survive only in patterns-deferred.md and the retro. LOCAL STATE: review-holds = 20 · voice `gian` · telemetry profile `cautious`. **Verification lesson worth keeping: `grep -c` counts LINES not occurrences and returned a false green on the §I format contract, passing a duplicate label that broke workflow-checkpoint.sh live — use `grep -o | wc -l`, or better, simulate the consumer's parse and read what it emits.** Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass — never append a second generation, and never repeat this line's own leading label text anywhere else on it (workflow-checkpoint.sh strips with a greedy BRE through the LAST occurrence; pre-compact.sh uses awk block capture — the two consumers disagree when violated, observed live 2026-07-26 and again 2026-08-15).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
