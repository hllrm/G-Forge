...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | ADR-013 SHIPPED · M50 SCOPE COMPACTED · 2026-08-18
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **ADR-013 written and gated** — "Consumers derive their lists; documents keep their numbers" (`g-docs/decisions/013-derive-in-consumers-keep-counts-in-prose.md`) — the derive-don't-type boundary promoted out of M50's scope bullet, where it would have died at milestone close. First draft (180 lines: 3-bucket taxonomy + admissibility gate) failed its own doc-review three rounds running (6→2→4 blocking, each round's fixes generating the next round's findings); stopped per §A8 and replaced wholesale with the 27-line two-rule form on the developer's call — both of the taxonomy's "underivable" exception cases dissolved on inspection (install paths ARE on disk under `profiles/<stack>/`; the frontmatter stack list's only consumer is `g-specialize:167` and should be a `[ -d profiles/<arg> ]` test). · ROADMAP M50 bullets (`:567-569`) compacted to match; evidence inventory lives there, not in the ADR. · Three stale "12 ADRs" counts bumped to 13 (`README.md:17`, `g-wiki/architecture.md:50`, `:106`). · Four doc-review rounds, ~525k subagent tokens total — fourth M45 datapoint: every round was tightly scoped and cheap to dispatch; the cost driver was the claim-dense ARTIFACT, not review breadth.
Next up:          · **Commit this changeset** (ADR-004 flow: stage → stamp sentinel → commit as separate calls) and **push**. · `/g-plan` M48 (suite-runtime + suite-ordering fixes, audit-7 rider). · M43 W4 governance-cadence ADR — before build. · `/g-trim` (its weekly nudge has never been clearable). · README 2.5-statement rewrite (`README.md:24-30` freeze copy, `:44-48` — the review-gate cost fix can now cite the measured result instead of a promise; M50 missing from the milestone list; `:777`/`:778` already current, do not re-edit). · CARRIED: n8n slugify on the n8n side · ROADMAP status emoji deliberately kept · `commands/g-forge.md:8` env-specific cache path · inbox trust-boundary ADR · M38 delivery decision · intake rows (a)-(h) · task 6 M45 rider · task 7 remainder.
Active context:   · **ADR-013 now governs doc edits here**: consumers derive their lists; documents keep concrete numbers, pinned or omitted — and do not hand-type file:line/count claims not verified the same turn; the 3-round review spiral was claim density, with each fix round introducing new false claims. · M50 ground truth: `/g-telemetry` coverage table types 17 rows vs 19 in `agents/` (`doc-reviewer`, `feature-implementer` missing; the 2026-08-17 committed report shipped blind to `doc-reviewer`); `g-specialize` frontmatter list → replace its one consumer (`:167`) with a directory test, intended set (48 typed vs 56 dirs) UNSTATED — settle it in `/g-plan` M50 (final review round's warning). · End-to-end done condition: add an agent, run `/g-telemetry`, the persisted report must count it. · `CLAUDE.md:32` date component stale (`green 2026-08-07` vs 2026-08-17 attestation) — per ADR-013: stamp it derivable or drop it. · ADR-004: sentinels bind to the STAGED tree — stage, then stamp `commit_sentinel_ts=<write-tree> commit_sentinel_head=<HEAD> commit_sentinel_worktree=<toplevel>`, then commit as a separate call; commit messages via a Write-tool file, never a heredoc. · LOCAL STATE: review-holds 25 · voice `gian` · telemetry `cautious` · suite 564/18 attested 2026-08-17. · Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass, and never repeat this line's own leading label text anywhere else on it (`workflow-checkpoint.sh` strips with a greedy BRE through the LAST occurrence; `pre-compact.sh` uses awk block capture — the two disagree when violated).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
