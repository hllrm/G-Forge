...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | W1.5a + W1.5b SHIPPED (both reviewed MERGE READY, committed) @ v2.2.1 (v2.2.2 pending milestone close)
⚠ FIRST: **`/g-plan` W1.5c** — continue the W1.5 split in order (W1.5c pre-commit gate fixtures → W1.5d sentinel-read extraction, riskiest, alone → W1.5e skill-layer edits → W1.5f guard-idiom + terminal attestation; slices in the M-audit section; standing rule: new minors route to W1.6/W2, never back into a slice) → W1.6 (tests for W1.3+W1.4; observe.sh sed escaped-quote item; carried minors: W1.5a newline-boundary + glued-pathspec-fidelity tests; W1.5b shadowed `.md` dir-arm fixtures + invariant grep missing `hooks/pre-commit`; ⚠ oversized, expect split) → W1.7 (live verify incl. FIRST REAL RUN of the stamped-sentinel + native pre-commit path — the native hook is still not installed LOCALLY until /g-update runs here, W1.7 territory). Do NOT re-plan/re-implement W1.1–W1.5b. **ADR-006 implementation stays a POST-M-audit slice.** stash@{0} confirmed superseded — droppable, developer's call.
