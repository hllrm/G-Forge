...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · Check 24 BUILT AND MERGED 91a36b8 through the full gate (2026-07-28)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **CHECK 24 BUILT, GATED ×4, MERGED `91a36b8`** (8 files, +142/−22): four-bucket classifier (MARKER-FED / IMPORT / DECLARED-LOCAL / BARE-PROSE) + five riders in `skills/g-doctor/SKILL.md` (~:273-321), doctor total **24 (16 required + 8 advisory)** swept across SKILL/README ×3/`g-wiki/usage.md:222` (the sweep caught g-wiki — forecast scenario 1 hit exactly as predicted, recurrence #4 of the count-string class). · `/g-trim` Step-1 import-follow shipped — line-initial `@`-imports only, path-gated (charset/relative/no-`..`), plugin-managed targets skipped per ownership rule; **T1's sunset condition now has a live detector** and `g-docs/transitional-rules.md` Tripwires updated to say so. · Gate history: code-lead HOLD r1 (6M/3m — rider (v) wrong source, rider (ii) check-ignore-only, CHANGELOG invented capabilities) → orchestrator axes HOLD (1 CRITICAL command-injection via shell interpolation of CLAUDE.md-parsed paths + 11M/11m, incl. architecture-enforcer HOLD on g-trim ownership crossing) → dedicated security-auditor HOLDING (3M/4m: pre-gate shell window, ungated payload materialization, ungated g-trim follow) → all hardened (in-model classification, first-char-excludes-`-` charset gate, payload never shelled, `[plugin-root]` sources + degrade branches, report-laundering guards) → same auditor re-verified **SECURITY: PASS 7/7 closed** → code-lead r2 HOLD (1M/2m: ADR-011:38 named `.claude/rules/` as canonical source = self-comparison, + g-trim flag-line echo + gitignored line-cite) → all three fixed, r2 re-verified → **MERGE READY, AXES all PASS**. Zero findings deferred. · CLAUDE.md voice-profile section wrapped in the local-only marker (local gitignored edit) — **declared-local count 0→1, tripwire baseline armed**. · ADR-011 follow-ups 2/3/5/6 annotated resolved; CHANGELOG `[Unreleased]` entry accurate to shipped source. · Tier 3 = dogfood doctor run + post-fix rider re-run (release-and-dogfood model, no manual round).
Next up:          · **THE ONE GATE, one discussion, roadmap written once** — queue unchanged from 2026-07-26, in force: (1) R0 shape (tightening-pass cadence · what process governs G-Proof's construction · fork checklist incl. wiring the T1 `@`-import · GF-20 spike · GF-23 verification, two rows not four · GF-70 test (seeded — narrow reading operative: matcher, not missing bash) · ADR-008 Spike S1 in a separate repo · suite-erosion defence, direction-agnostic, derived from the run · scoped breakdown + cost counter-estimate). (2) M-FORGE/M-PROOF split by changeset class — `Track:` field + `.claude/active-track`, NEVER two `## Active Session` blocks (`workflow-checkpoint.sh:82` greedy strip); R0 spikes run in a throwaway spike clone or the doc-only property collapses. (3) Fork timing — recommended at the freeze; post-freeze sync = cherry-pick-forward per bug fix only. (4) 8 Discuss rows (GF-20/23 · GF-60/61 · GF-63/77/78). (5) client/cockpit milestone candidate. (6) `/g-intake` ×3 (telemetry fix-loop · TDD+doc-pass · g-train deepening). (7) three-horizon re-scope AFTER the rebuild plan (M44/M41 stamps stay placeholders). (8) M44 detail calls + weekly platform-sync + GF-40 ADR + gate-hook clone-first checkpoint. · **INTAKE ROWS carried, (b) partially spent**: (a) `/g-patterns` derivable-counts · (b) injection-rule teeth — **Check 24 SHIPPED; remaining: generated Quick-commands + marker block for project prose, i.e. the 50-finding bare-prose backlog** (day-one list = the authoritative backlog per developer decision; also the Check-24 advisory→required promotion gate, ADR-011 follow-up 1) · (c) self-safe/at-risk state (journal/GF-73 purge vector) · (d) reviewers write their own sentinels (ADR-shaped) · (e) consumer sentinel-write permission denial · (f) user-defined voice profiles · (g) per-prompt injection > loaded-once document finding · NEW (h) security residue swept but worth a standing rule: untrusted-content gates for any model-executed check that shells out (the Check 24 lesson, generalize at rebuild). · **Advisories still live**: `project_brief.md:37` · `M44.md:3` · `test-classify-changeset.sh:11` vs `:13` · `transitional-rules.md:13` · checkpoint spent-entry nudge. · **STILL OPEN, developer's call**: what lands in v2.5 before the freeze (currently only M41) · what earns a `2.5.x` patch vs "fixed in G-Proof". · Rider in `g-docs/todo.md`: g-wiki M46 refresh.
Active context:   · `main` @ v2.4.0, `91a36b8` committed (pass-close commit follows), push pending at pass end. Both gate sentinels consumed by `post-commit-cleanup.sh` after `91a36b8`. **`/g-doctor` now ships 24 checks (16+8) — Check 24 advisory, injection-hardened; declared-local count = 1 (voice-profile block, wrapped this pass)**. ADR-010 VERIFIED, ADR-011 LANDED AND CURRENT (follow-ups 2/3/5/6 resolved in-file) — build on both, re-verify neither; the checkpoint's "a handed-off ADR needs verifying" nudge stays a spent-entry false positive (`workflow-checkpoint.sh:376`). Suite = **523 / 17 suites / green**, attested 2026-07-28 by `g-forge-dev` pre-gate — always awk-sum the run's own `Results:` lines, never hand arithmetic, never prose. **Version identity: G-Proof, first release 1.0 — never `3.0`.** Arc order PENDING the gate; M44/M41 stamps are placeholders until the post-rebuild-plan re-scope. LOCAL, NON-COMMITTED STATE (all gitignored): `.claude/settings.json` shell matcher = `Bash|PowerShell` · `.claude/hooks/workflow-checkpoint.sh` at source parity · `.claude/hooks/check-commit.sh` deliberately BEHIND source (ADR-008 hold — a decision, not drift; `/g-review`'s drift line reports it every run, this pass included) · voice profile `gian`, its CLAUDE.md section NOW wrapped in `<!-- G-Forge local-only: voice-profile -->` markers · caveman off · `.claude/review-holds` = 8 (r1 + r2 of this pass; `/g-telemetry` resets on stable). REPORT-VS-REPO CORRECTIONS (don't re-derive): 8 hook scripts + hooks.json + 6 libs, 19 agents, 38 skills, single router (ADR-007), 56 profile dirs, **24 doctor checks**, SubagentStart/Stop registered. Backlog riders: g-doctor Check-16 lib enumeration stale · `command -v` hardening · fixture sleep-leak (serial full suite ~9.6 min) · per-session counter accumulation · ADR-006 implementation · Check-24 bare-prose backlog (50 findings, see intake row b). Format note: keep this line's leading label intact, use exactly the three §I labels, replace this block **wholesale** each pass — never append a second generation, and never repeat this line's own leading label text anywhere else on it (`workflow-checkpoint.sh:82` strips with a greedy BRE through the LAST occurrence; `pre-compact.sh:74` uses awk block capture — the two consumers disagree when violated, observed live 2026-07-26).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
