...
⚠ FIRST: **/g-execute W1.7 8 in a FRESH session — Pass 3 of the approved 3-pass split (Waves 8–12: FIRST LIVE GATED COMMITS, all HQ, serial by sentinel resource): Task 14 gated commit primary (/g-review → stamp → commit; CARRIES all Pass-1/2 uncommitted edits) → Task 15 gated commit linked worktree (ADR-005 live) → Task 16 script-indirection live verify → Task 17 sentinel auto-consume on normal argv commit → Wave 12 ledger close (Tasks 19, 26–28).** THE GATE IS LIVE NOW: native pre-commit + libs in .git/hooks/ (dry-run denies via sentinel path), current check-commit.sh + libs in .claude/hooks/ (11/11 hash-match — PreToolUse already live-fired this session denying an argv `git commit` probe without sentinel). Sandbox probes MUST go via script file (PreToolUse sees argv). Plan `g-docs/plans/w1-7-gating-install-live-verify.md` · forecast `g-docs/forecasts/w1-7-gating-install-live-verify.md` · rollback baseline REFRESHED: `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-22\` (63 files byte-verified; old -21 kept). On any live-gate lockout: `--no-verify` or hook-delete (both PROVEN Task 12), restore snapshot, never debug the live gate in-session. Before M-audit W2 executes: verify ADR-007 from a fresh window. W1.5g Pass 3 (Spike S1) remains DEFERRABLE. Do NOT re-plan/re-implement W1.1–W1.6. **ADR-006 implementation stays a POST-M-audit slice.** stash@{0} confirmed superseded — droppable, developer's call.
