...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · ADR-010 VERIFIED against the repo + injection rule established, R0 planning next (2026-07-26)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **ADR-010 CAPTURED, THEN VERIFIED AGAINST THE REPO — every in-repo claim HOLDS; build on it, do not re-verify.** ADR = `g-docs/decisions/010-full-rebuild-on-current-platform.md`, status Accepted: full rebuild on the native platform, **freeze + fork** delivery (v2.5 ships, THIS repo freezes on bug-report maintenance, **forked** into a new repo (git history and tests carried — not a folder copy) and transformed into **G-Proof** (first release **1.0**) — so M44's rebrand IS the release vehicle and the 2.x line ends at the freeze). Three qualifications are part of the decision, not caveats: (1) **the commit gate is OUT of the language decision at every layer** — it decomposes into three doors, door 2 = native git hook (authoritative per ADR-004, bound to git not to any runtime, untouched by the rebuild), door 1 = the Claude Code PreToolUse hook (non-authoritative, shrinks, stays local), door 3 = CI (retired: `/g-tier light` already disables the gate as a supported operation, so GF-43 rested on a false premise; a CI *reporter* stays brief-compatible and parks against the multiplayer arc). Therefore the language surface governs **execution and review only** and the product's one differentiator carries zero rebuild risk. (2) **R3 conditional on the GF-20 spike.** (3) **Only R0 is authorized to execute** — "full rebuild" is the posture; R0's deliverable is the scoped breakdown plus a counter-estimate of the rebuild's own cost. Reversibility inverted by the delivery shape: two-way at product level (frozen v2.5 is always the fallback), one-way only inside the new repo. Premortem #1 (dogfooding deadlock, top-ranked) structurally eliminated — G-Proof is governed by frozen v2.5 installed as a consumer. · **VERIFICATION — executed, not read.** (a) ADR-004's door-2-is-authoritative reading is exact (`hooks/pre-commit:4-12` in the file's own header; ADR-004:14 and :25 with the mechanism — PreToolUse fires pre-staging so `git write-tree` there hashes the wrong tree). (b) Door 2 **IS installed**: `.git/hooks/pre-commit`, executable, `diff`-identical to source, libs at `.git/hooks/lib/` — this was the assumption flagged as carrying the most weight and it is on disk. (c) **`/g-tier light` DOES disable the gate — RUN in a scratch repo with a real hook and a real `git commit`:** light ⇒ commit LANDED at both doors; full ⇒ blocked (door 2 exit 1 + stderr deny, door 1 exit 2 + deny JSON); balanced ⇒ blocked. Both doors read the tier independently (`hooks/pre-commit:131-138`, `hooks/check-commit.sh:158-170`) and door 1's deny message advertises the switch. Re-confirmed exact: 19 agents · 38 skills · 8 hooks · 6 libs · 56 profiles · single router · 23 doctor checks · version parity 2.4.0/2.4.0 · no CI · ADR-008 Spike S1 cited correctly · installed `check-commit.sh` still deliberately behind source (ADR-008 hold) · §A7 thresholds still un-recalibrated (30/45 · 45/65). Platform-side assumptions (workflow scripts non-preview, GF-23, GF-20) remain unverified BY DESIGN — R0's job. · **TIER-1 GREEN: 523 passed, 0 failed, 17/17 suites exit 0** — and the number was wrong in every document, including my own first correction. Truth: the attested 522 was the exact sum of the suites' declared headers and was never drifted prose; `test-workflow-checkpoint.sh` had silently GAINED one assertion (declared header 80, runner-observed 81), so reality had moved to 523. An intermediate "518" was published into four files this pass off a **mental-arithmetic error** and caught by `doc-reviewer`, which refused DOCS READY and cited the committed M46 attestation records against it. All sites now read 523; the stale test header is fixed at `tests/test-workflow-checkpoint.sh:12` (re-verified by a single-suite run: `Results: 81 passed, 0 failed`). ⚠ **Premortem #2 fired before the fork, in the direction its tripwire does not watch.** The tripwire was written as "a handoff quoting a suite total LOWER than the previous one" — the real drift went **upward**, silently, because nobody re-sums. R0's regression-defence plan must derive the total from the run, and the detector must be direction-agnostic. Also: a serial full-suite run exceeds 10 minutes on this machine (fixture sleep-leak rider) — regression defence you cannot afford to run erodes quietly. · **GF-70 NOW HAS LIVE EVIDENCE — the narrow reading is the operative one.** On this Windows host `bash` IS present (`/usr/bin/bash`, GNU 5.2.37 msys) and every hook is invoked as `bash -c '...'` (`.claude/settings.json:8`). What was broken was the **matcher** (`Bash` only, while the shell tool here is PowerShell), not bash's absence. "No bash on the host" is FALSE here; "Claude Code doesn't route through bash" is the real failure mode — the materially-different finding ADR-010 says may *strengthen* the rebuild. R0's GF-70 test now designs against one confirmed data point. · **DECISION — CLAUDE.md STAYS GITIGNORED; durability comes from airtight injection, not from tracking the artifact.** The negation carve-out that sat on the intake list was implemented, reviewed live, and **reversed by the developer.** Rationale is recorded in `.gitignore`'s comment block above `/CLAUDE.md` (now at `.gitignore:42`). **THE INJECTION RULE, in force:** every load-bearing line in CLAUDE.md must be (a) inside a G-Forge marker block fed from committed plugin source, (b) an `@`-import of a committed file, or (c) explicitly declared local-only (the voice-profile import is the one sanctioned instance of (c)). Bare prose asserting facts about the tree is the defect — it is what was lost on 2026-07-10 and what let the per-suite table sit stale while the suite moved. Fix the injection, never the .gitignore. (Rejected en route: tracking the file would have published the personal voice-profile section by name to a public marketplace repo and left a dangling `@`-import in every clone.) · **T1 MOVED OUT OF HARM'S WAY — `g-docs/transitional-rules.md` is NEW and COMMITTED**, canonical source of the transitional-rules section; CLAUDE.md holds a pointer plus `@g-docs/transitional-rules.md` and no rule text. ADR-010:61 updated to match (it still described the pre-extraction design). T1's **on**-tripwire got structurally smaller (the fork carries the rule by git; only the one-line import needs wiring). ⚠ But its **off**-tripwire is now WEAKER and is recorded as **not implemented**: `/g-trim` audits `CLAUDE.md` and agent memory only (`skills/g-trim/SKILL.md:3`) and never reads `g-docs/`, so extraction moved T1 out of the one file that audit inspects. Scope-adding `/g-trim` is an R0 fork-checklist item; until then the sunset detector is an intention, not a mechanism. · **INTAKE TRIAGED — "self-safe processes / back up critical files every N milestones" (developer).** Verdict: **on-brief need, wrong mechanism as stated, splits into 3 items.** Reframing finding: **`/g-init` Step 5a ALREADY ships this rule** (`skills/g-init/SKILL.md:168` classifies CLAUDE.md/G-RULES.md/CHANGELOG.md/README.md as tracked project record; :171 tracks `.claude/hooks/`, `settings.json`, `rules/`, `agents/`; only sentinels/journal/counters are ignored) — so the hole was never a missing feature, it is this repo deviating from its own shipped boundary, defensibly for `.claude/*` and accidentally for CLAUDE.md. A backup regime for a file your own tool says to commit is a worse git beside git ("not a git replacement" = brief non-goal). BUT the dig found a genuinely at-risk set: **`.claude/journal/` (260K, 16 days — the observer record `/g-retro` synthesizes from), `.claude/context-threshold-offset` (=5, learned one compaction at a time), `.claude/telemetry-profile`** — ignored BY SHIPPED DESIGN (:177), irreplaceable, and **GF-73 already found the live destruction vector: `claude project purge` wipes sentinels + journal.** Also challenged: **"every N milestones" is the wrong clock** (the journal accrues daily and `/g-retro` consumes it per session, so an N-milestone cadence accepts up to N milestones of loss; the threatening events are session-end, compaction and purge — and `pre-compact.sh` already snapshots on exactly that logic, plus a native `SessionEnd` hook now exists). And **`g-tweak` does not exist** under that name — closest is Safe self-tune (`g-docs/ROADMAP.md` M43 area), explicitly *barred from the commit gate and enforcement sentinels*, approve-before-write; a snapshot writer is not a tweak, do not hang it there. Build constraint: **ADR-009 makes `/g-doctor` detect+diagnose only, read-only** — doctor may FLAG at-risk files, a different writer takes snapshots; do not collapse those. · **BRIEF AMENDED** (`g-docs/project_brief.md`): new non-goal at **:24** — *"Not defence against a user who breaks the tool they adopted"* — plus a dated Decisions entry retiring GF-43 on its false premise and rejecting the CI *enforcer* on non-goal grounds (reopening = amend the brief first; residual "findable switch" need routes to M43 `/g-settings`). · **`/g-update` RAN this cycle** (self-host, source = working tree): realigned `.claude/hooks/workflow-checkpoint.sh` to route ordering through `gf_semver_compare` (ADR-009), and widened the `.claude/settings.json` shell matcher `Bash` → `Bash|PowerShell` on PreToolUse + PostToolUse because on Windows the shell tool is PowerShell and those rows were dark. `check-commit.sh` HELD per the ADR-008 gating-class rule. ⚠ **COUPLING, still open:** the widened matcher now fires the held, unguarded hook on every PowerShell call too, so exposure to the abandoned-stdin stall is materially higher — the clone-first gating checkpoint is higher priority than when it was parked. · Also this cycle: modernization report intaken + ground-truthed + superseded for planning by `g-docs/audits/2026-07-rebuild-map.md` and `…/2026-07-rebuild-report.md` (R0-Decisions → R6-Reach replace the additive M-A…M-F shapes); developer review in at `…/2026-07-triage-review.md` (30 Agree / 8 Discuss / 0 Reject); caveman plugin disabled at source (`%APPDATA%\caveman\config.json` `{"defaultMode":"off"}` — it was never opted into, `getDefaultMode()` fell through to a hardcoded `full`); voice profile `gian` installed local-only (`.claude/voice-profile` + `.claude/voice-profiles/gian.md` + an `@`-import at the bottom of CLAUDE.md outside the markers, all gitignored, this repo only).
Next up:          · **COMMIT FIRST — 8 dirty files, and the changeset is now MIXED (code + doc), so BOTH gates are required:** `g-docs/decisions/010-*.md` (new) · `g-docs/transitional-rules.md` (**new**) · `.gitignore` · `g-docs/project_brief.md` · `g-docs/ROADMAP.md` (this handoff) · **`g-docs/milestones/M44.md`** (ADR-010 supersession stamp) · **`g-docs/milestones/M41.md`** (ADR-010 freeze-context stamp) · **`tests/test-workflow-checkpoint.sh`** (the stale `# Total assertions: 80` → 81 header fix — this is what makes the set mixed). Path = `/g-doc-review` **and** `/g-review` → both sentinels → commit. `CLAUDE.md` is gitignored and will NOT appear: its edits this pass (523/81 counts, T1 → pointer + import, durability note) travel by folder copy only, which is now correct by decision rather than by accident. **`/g-retro` NOT RUN** — the journal has the session; run it fresh if wanted. · **THE INJECTION RULE HAS 3 TEETH TO GROW — new gate items, none a one-liner.** CLAUDE.md is NOT yet compliant with the rule just established: ✅ compliant = `@G-RULES.md`, architecture rules, transitional rules, marker blocks, Project Tracking; ⚠ voice import = sanctioned local-only (class (c)); ❌ **Quick commands (17 suites + counts) = bare prose and DERIVABLE — the 522/523 defect class**; ❌ **Stack / Architecture / Conventions = bare prose, regenerable only by a model re-reading the repo (exactly what happened 2026-07-10)**. Therefore: (1) **generated Quick commands** — counts come from the run, not a keyboard, killing the drift class permanently instead of correcting instances; (2) **a marker block for project prose** fed from committed source so a lost CLAUDE.md is restored mechanically, not re-authored; (3) **`/g-doctor` Check 24 — flag any CLAUDE.md content that is neither marker-fed, nor an import, nor declared local-only**, which is what makes the rule enforced rather than aspirational (same wholeness class T1 already asks doctor to cover). The class already has 3 instances — 522/523 · the standing Check-16 lib-enumeration rider (4 of 6 libs; README mirrors it) · CLAUDE.md prose generally ⇒ **a `/g-patterns` row, not 3 tickets.** · **SELF-SAFE / at-risk-state items for the gate** (R0/G-Proof material — v2.5 is frozen after M41, so adding capability to a frozen line contradicts the delivery shape): (a) `/g-doctor` durability check — every file G-Forge depends on classified committed | regenerable | at-risk | local-only; (b) at-risk snapshot writer for journal + learned calibration, triggered on session-end/pre-compact/purge-risk, **not** a milestone counter, and NOT inside doctor (ADR-009 read-only); (c) scope-add `/g-trim` to read `g-docs/transitional-rules.md` — currently T1's sunset detector does not exist. · **THEN R0 — "the HOW".** Only authorized body of work, own shape unplanned: the tightening-pass cadence · what process governs G-Proof's construction (full-tier 2.5 explicitly NOT the assumed answer — plan→execute→review→gate protects a stable product from regression; a continuous transform wants a different rhythm) · the fork checklist (wire the T1 import; the rule travels by git) · GF-20 spike · GF-23 verification (cheapest test, biggest leverage — gates `code-reviewer` DIES and `architecture-enforcer`'s conditional) · GF-70 test (**seeded with live evidence, see above**) · whether ADR-008 Spike S1 still binds once the rebuild is a separate repo · the suite-erosion defence (**derive the count from the run, direction-agnostic**) · the scoped breakdown + cost counter-estimate. · **STILL OPEN, developer's call, neither blocking:** what lands in v2.5 before the freeze (currently only M41) · what earns a `2.5.x` patch vs "fixed in G-Proof" (needed before the announcement, or the first bug report decides it by default). · **THE GATE still owes** (one discussion, roadmap written once): (1) rebuild shapes R0…R6 + rebuild-map verdicts under the confirmed full-rebuild mandate; (2) the 8 Discuss rows — GF-20/23 transform shapes · GF-60/61 watch-vs-prototype · GF-63/77/78 posture; (3) G-Forge client/cockpit milestone candidate (intaken thin-first: deep-link launchers + board view, after rebuild core, likely paired with M44); (4) `/g-intake` ×3 — telemetry fix-loop (M39 spec; metric recalibration + alarm-fatigue note) · TDD + documentation-pass layer · g-train deepening; (4b) **user-defined voice profiles** (`/g-voice` hard-rejects any value outside `dev|mid|eli5` at `skills/g-voice/SKILL.md:20`; shape = accept any name resolving to `.claude/voice-profiles/<name>.md`, three built-ins as defaults — note this repo's local bridge works only because CLAUDE.md is gitignored here, and a consumer has no equivalent hiding place, which is exactly why the feature is needed); (4c) FINDING worth a note or an ADR — **a persona re-injected on EVERY prompt (caveman, via UserPromptSubmit) beat a document loaded ONCE (CLAUDE.md `@`-import)**, observed live with the injection still firing; bears on how much behaviour `workflow-checkpoint.sh`'s per-prompt injection actually drives and whether G-Forge should rely on injection frequency at all; (5) `/g-patterns` §G Currency-rule proposal **+ the derivable-counts row above**; (6) three-horizon re-scope — **RUNS AFTER the rebuild plan** per developer 2026-07-26 (M26/M35/M29/M33/M43 absorption candidates); (7) M44 pull-forward detail calls (direction DECIDED: rebrand lands right after the rebuild; the three detail calls — done-def rewrite, M41 prereq, ride-alongs — settle at the gate) + the weekly platform-sync process (proposed, not built) + GF-40 ADR (invocation × scheduling, unwritten and independently gating) + the gate-hook clone-first checkpoint (raised in priority by the matcher coupling above). · **RETIRED, do not re-propose:** the `.gitignore` negation carve-out for CLAUDE.md (implemented and reversed by the developer this pass — see the injection rule instead) · friend's board ranking (artifact no longer awaited) · GF-43 CI enforcer (false premise + two non-goals) · GF-46 as an open question (DECIDED: full rebuild) · M44 as a competing milestone (it is the rebuild's release vehicle). · Rider in `g-docs/todo.md`: g-wiki M46 refresh.
Active context:   · main @ v2.4.0, origin-synced through c0e121e, **nothing committed this session.** 8 dirty files, changeset **MIXED** (see Next up) — both `/g-doc-review` and `/g-review` sentinels required before commit. **ADR-010 is the anchor document and is VERIFIED against the repo — build on it, do not re-verify.** Real suite total = **523 across 17 suites, green** (not 522, not 518 — and one intermediate figure this pass came from mental arithmetic, so re-sum from the run's own `Results:` lines, never from prose). **INJECTION RULE IN FORCE** (`.gitignore:42` comment block): every load-bearing CLAUDE.md line is marker-fed from committed source, an `@`-import of a committed file, or declared local-only; bare prose asserting facts about the tree is the defect — fix the injection, never the .gitignore. Arc order still PENDING the gate — do not plan against the old sequence, and note the sequence from M45 onward is dead anyway (2.x ends at the v2.5 freeze). LOCAL, NON-COMMITTED STATE a fresh session inherits silently (all gitignored, none of it in git): `.claude/settings.json` shell matcher = `Bash|PowerShell` · `.claude/hooks/workflow-checkpoint.sh` at source parity · `.claude/hooks/check-commit.sh` deliberately BEHIND source (ADR-008 hold — do not "fix" it casually, it is a decision) · `.claude/voice-profile` = `gian` + `.claude/voice-profiles/gian.md` + a voice `@`-import at the bottom of CLAUDE.md outside the markers · caveman off via `%APPDATA%\caveman\config.json`. REPORT-VS-REPO CORRECTIONS baked into triage (don't re-derive): 8 hook scripts + hooks.json + 6 libs, 19 agents, 38 skills, single router (ADR-007), 56 profile dirs, 23 doctor checks, SubagentStart/Stop registered. Backlog riders: g-doctor Check-16 lib enumeration stale (4 of 6 libs; README "4 lib scripts" mirrors skill — fix together) · `command -v` hardening · fixture sleep-leak (a serial full-suite run now exceeds 10 min) · per-session counter accumulation (`.claude/session-prompt-count.<uuid>` files observed accruing) · ADR-006 implementation. Format note: keep this line's leading label intact and use exactly the three §I labels (done-this-pass, next-up, active-context), and replace this block **wholesale** each pass — never append a second generation under a numbered or carried-over label variant (that shape produced live stale-residue findings on 2026-07-26). **Never write the active-context label text a second time on this line:** `workflow-checkpoint.sh:82` strips with a greedy BRE (`sed 's/.*<label>[[:space:]]*//'`), which cuts through the LAST occurrence on the line, so a repeat here makes the per-prompt banner display the tail of this note instead of the project state — observed live on 2026-07-26. The constraint is once **per line**, not once per block; `pre-compact.sh:74` captures the block by awk and is unaffected, so the two consumers disagree when this is violated.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
