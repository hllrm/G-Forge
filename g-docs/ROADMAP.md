...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | **W2 Pass 3 COMPLETE 2026-07-22** (Waves 3–4: sweep + conformance + all attests GREEN) @ v2.2.1 (v2.3.0 at FULL M-audit close)
⚠ FIRST: **/g-resume → /g-execute 5** on the W2 plan (`g-docs/plans/m-audit-w2-shim-retirement-conformance.md` — Pass 4 = Wave 5: task 16 g-skill-validate walkthrough on 3 samples [g-status/g-doctor/g-adr, attested PASS recording] HQ-inline → Wave 6: task 23 ledger rows + CHANGELOG [Unreleased] entries, LAST → **/g-review → THE gated commit** — first commit carrying ALL W2 edits, Waves 1–4, ~69 dirty entries expected incl. 38 shim deletions). → W3 → **release pass: v2.3.0 at FULL M-audit close** with README status strip + CHANGELOG `[2.3.0]` cut. THE GATE IS LIVE (installed gating pair predates heredoc fix — source-newer drift on commit-detect.sh is EXPECTED and deferred per ADR-008 gating class; drift suite attested 12/12 with that carve-out). Rollback baseline: `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-22\`. On lockout: `--no-verify`/hook-delete, restore snapshot, never debug live in-session. Do NOT re-plan Waves 1–4. **ADR-006 implementation stays POST-M-audit.** stash@{0} superseded — droppable.
