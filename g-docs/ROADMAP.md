...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | M50 SCOPED · M47 SWARM CLOSED · SCOPED REVIEW PROVEN · 2026-08-17
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **M47 CLOSE SWARM DISCHARGED** — /g-telemetry, /g-align, /g-wiki, /g-doctor all run; the obligation had been open since M47 shipped 2026-08-12. · **M50 — Eval-Chain Integrity created and folded into v2.5**, sequenced THIRD (M47 → M48 → M50 → M45); ADR-012 amended a second time; build order verified identical across five surfaces. Scope = the five telemetry-instrument defects, the agent contract map, and the firing audit. · **M43 gained Wave 4 (governance cadence)** — health passes gate on the `full` tier keyed on RUN-RECENCY, never on a pass issuing a blocking verdict, so /g-align's advisory contract survives; needs its own ADR before build. · **Audit remainder (todo row 7) FULLY DISPOSED** — every item has an owning milestone; todo row 1 closed by the wiki pass. · n8n portable-filename contract in the g-patterns skill + §I; slugify expression handed over for the n8n side. · Hook banners de-emojied (10 four-byte emoji, two hooks, four assertions re-anchored). · Committed `5ba563d`, **NOT YET PUSHED**.
Next up:          · **FIRST TASK: /g-adr for the derive-don't-type boundary** — code consumers vs prose. Decided 2026-08-17, currently surviving only as one bullet inside M50's scope, so it dies when M50 closes. Deliberately deferred to a clean window per §C (high-branching deliberation poisons the context it runs in). · **Push `5ba563d`.** · `/g-plan` M48 (carries suite-runtime + suite-ordering fixes and the audit-7 rider added today). · M43 W4 governance-cadence ADR — before build, not now: the design (which passes, what windows, how it degrades) is undecided. · `/g-trim` — has never completed here, so `.claude/last-trim` has never existed and its weekly nudge has fired on every prompt indefinitely. · CARRIED: n8n slugify on the n8n side · ROADMAP status markers deliberately NOT de-emojied (four skills read them) · `commands/g-forge.md:8` hardcodes an env-specific plugin-cache path (audit-4 N4, unscoped) · inbox trust-boundary ADR · M38 delivery decision · intake rows (a)-(h) · task 6 M45 rider · task 7 remainder. · **Tighten the README 2.5 statement and make it genuine** (`README.md:24-30` freeze copy, `:44-48` "What's coming in 2.5"). Two concrete defects, both introduced by the roadmap moving under it: (a) `:46` pitches the review-gate cost fix as a *promise* ("one enormous pass that costs a fortune") when there is now a **measured** result — 11 tool calls / 79k tokens / 1 round vs ~3h + ~130k killed — so the claim can be stated as evidence rather than aspiration; (b) the section names M47 planning and M45 review lightening but **omits M50 entirely**, though M50 is now sequenced third and carries the self-governance instruments. Also cut length: it is the longest prose block in the README and the freeze argument survives compression. Note `:777` (milestone list) and `:778` (freeze/G-Proof line) are already current as of 2026-08-17 — do not re-edit those.
Active context:   · **SCOPED REVIEW GATE PROVEN ON THIS REPO — the pass's headline result.** 11 tool calls / 79,475 tokens / 3m54s / ONE round / 1 major + 1 minor, both genuine, against a ~3h + ~130k-token unscoped run that was killed before finishing and a six-round `/g-resume` Step 0. **Scoping did not cost recall.** The levers were exclusion, not file-list precision: forbid suite re-runs (attested green as given), forbid architecture re-derivation, drop three reviewer axes with zero surface. Third independent datapoint after Alter-G (reproduced twice) and the keyline field report — carry all three into M45's design ADR. · **The pass's methodological error: scope was written from AUTHORSHIP, not DEPENDENCY.** `/g-blast-radius` was skipped; it later showed 6 suites exercising the changed hooks where the hand-scope named 1, and the only silent-breakage vector (a skill grepping the banner for old markers, uncovered by any test) was checked AFTER dispatch. Run blast-radius BEFORE dispatch — it is M45's stated design. · **A mechanical check verifies the claim it tests, not the claim you care about** — build-order strings verified clean by grep while the ordinal words beside them ('sequenced second' under a build order putting M50 third) became the review's one major. Same class: /g-doctor Check 20's by-name .gitignore grep reported three FALSE failures because this repo ignores `.claude/` wholesale; `git check-ignore` is the authoritative test. · **Gitignored is not deleted** — two audit findings written off as unrecoverable were recovered by one `ls` of `g-docs/agent-output/audit/`. · **ADR-004: sentinels bind to the STAGED tree** — stage, then stamp `commit_sentinel_ts=<write-tree> commit_sentinel_head=<HEAD> commit_sentinel_worktree=<toplevel>`, then commit as a separate call; prose sentinels are unparseable and the gate denies correctly. The commit-message heredoc failed live, reproducing todo row 10 — use the Write tool. · LOCAL STATE: review-holds = 25 · voice `gian` · telemetry `cautious` · /g-doctor 16/16 required passed · suite 564/18 green on a quiet sequential run · inbox verdict files: enumerate with `ls g-docs/inbox/adversarial/`, never trust a written count. · Amber context gate ran from exchange 15 to close, knowingly, to buy the review datapoint. · Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass, and never repeat this line's own leading label text anywhere else on it (`workflow-checkpoint.sh` strips with a greedy BRE through the LAST occurrence; `pre-compact.sh` uses awk block capture — the two disagree when violated, observed live 2026-07-26 and 2026-08-15).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
