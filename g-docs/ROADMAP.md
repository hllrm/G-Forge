...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · REBUILD frame locked, v2 report written, gate next (2026-07-25)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · Modernization intake COMPLETE. Report saved verbatim → `g-docs/audits/2026-07-modernization-report.md`. §0.2 ground truth run (CLI 2.1.218 — clears every §5 version gate; changelog runs to 2.1.220). §0.3 live-doc verify via agent: report holds EXCEPT (a) subagent nesting default-on ENDED at v2.1.216 — now opt-in via CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH, no "spawn classifier" exists → GF-21 depth-0 constraint STANDS on default config, README:342 needs reword not retraction; (b) native checkpointing does NOT restore subagent edits (except foreground context:fork) → GF-31 wave recovery must be git-snapshot, not /rewind. Triage table → `g-docs/audits/2026-07-modernization-triage.md`: report rows 14 ADOPT / 9 PARTIAL / 3 ALREADY-DONE (GF-03/11/13) / 4 DEFER / 0 REJECT. Coverage sweep (full llms.txt vs report appendix) added GF-70…78 — material: GF-70 w18 Windows-without-Git-Bash runs PowerShell as shell tool = 8 bash hooks silently dead (feature-detect + README requirement); GF-76 v2.1.219 Opus 5 default + 1M context = A7 thresholds + A1 tier table stale; also GF-71 Tool(param:value) permission matching as gate defense-in-depth, GF-73 `claude project purge` wipes sentinels/journal, 15 weekly digests unaudited (w13–w23, w25, w27–w29). Artifact published for developer review: https://claude.ai/code/artifact/0f4a3bcc-2453-4d75-a73d-4c67edb36de6 (filterable triage board, v3 carries rebuild frame). DEVELOPER REFRAME (mid-pass, now governing): this is a REBUILD — "if G-Forge were built today on 2.1.220, what would it be?" Unit of analysis = component, not feature; native COMBOS can kill whole components (worked example: code-lead → gate adjudicator over delegated review, own diff-reading dies); massive change explicitly sanctioned. Produced accordingly: `g-docs/audits/2026-07-rebuild-map.md` (all 19 agents + 38 skills + 8 hooks: SURVIVES/SHRINKS/TRANSFORMS/DIES + cascade) and `g-docs/audits/2026-07-rebuild-report.md` (v2 report, SUPERSEDES the external report for planning; re-cut milestone shapes R0-Decisions → R6-Reach replace additive M-A…M-F). Weekly platform sync proposed as standing process (intake candidate, not built).
Next up:          · DEVELOPER REVIEW IS IN (g-docs/audits/2026-07-triage-review.md): 30 Agree / 8 Discuss / 0 Reject; **GF-46 DECIDED — FULL REBUILD confirmed by developer in direct Q&A 2026-07-25** (over shell-only and optional-JS-tier; ADR to capture via /g-adr, no longer an open question); GF-13 resolved off-agenda (polish rides R1). NEXT SESSION: fresh window, /g-resume, then (a) /g-adr for GF-46 full-rebuild + GF-40 invocation×scheduling split, (b) THE ONE GATE, now SEVEN inputs (one discussion, roadmap written once): (1) rebuild shapes R0…R6 + rebuild-map verdicts under the confirmed full-rebuild mandate; (2) 8 Discuss rows — agenda finalized in the review file (GF-20/23 transform shapes · GF-60/61 watch-vs-prototype · GF-63/77/78 posture); (3) NEW: G-Forge client/cockpit milestone candidate (developer idea, intaken thin-first: deep-link launchers + board view, after rebuild core, likely paired with M44/G-Proof); (4) /g-intake ×3 — telemetry fix-loop (M39 spec; metric recalibration + alarm-fatigue note) · TDD + documentation-pass layer · g-train deepening; (5) /g-patterns §G Currency-rule proposal; (6) three-horizon re-scope (M26/M35/M29/M33/M43 absorption candidates); (7) M44 pull-forward + weekly platform sync process. Friend's board ranking folds in if arrived (artifact add745a4). UNCOMMITTED: g-docs/audits/ ×5 files + this handoff — doc-only commit on developer nod. Rider in todo.md: g-wiki M46 refresh.
Active context:   · main @ v2.4.0, origin-synced except uncommitted g-docs/audits/ ×5 (report · triage · rebuild-map · rebuild-report · triage-review) + this handoff. REPORT-VS-REPO CORRECTIONS baked into triage (don't re-derive): actual tree 8 hook scripts + hooks.json + 6 libs, 19 agents, 38 skills, single router (ADR-007), 56 profile dirs, /g-doctor 23 checks, SubagentStart/Stop registered. Arc order PENDING the gate — do not plan against the old sequence. Backlog riders: g-doctor Check-16 lib enumeration stale (4 of 6 libs; README "4 lib scripts" mirrors skill, fix together) · command -v hardening · fixture sleep-leak · per-session counter accumulation · ADR-006 implementation. Installed check-commit.sh pre-guard BY DESIGN (ADR-008). Format note: keep this line's leading label intact and replace this block wholesale each pass — workflow-checkpoint.sh greps for that label (must appear exactly once in the block: greedy strip shows the tail after the LAST occurrence); prior-pass residue served a week-stale banner until 2026-07-23.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
