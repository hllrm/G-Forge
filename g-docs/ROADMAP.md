...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | W1.3 REVIEWED MERGE READY + **ADR-006 Accepted** (optimistic wave concurrency / collision absorption) · sentinel written, W1.3 commit pending @ v2.2.1 (v2.2.2 pending milestone close)
⚠ FIRST (fresh session): **verify ADR-006 against the actual repo state before building on it** (clean-slate check — `g-docs/decisions/006-optimistic-wave-concurrency-collisions-absorbed.md`; confirm the Decision's claims about hooks/observe.sh + agent-lifecycle.sh surfaces, ADR-004/005 coherence, and that no wave code contradicts it). THEN **commit W1.3** (six worktree-integrated hooks + this pass's doc/record edits — `.claude/g-forge-approved` sentinel is written, gate open), then **`/g-plan` W1.4** (install wiring: `/g-init`/`/g-update` learn the new hook files + `/g-doctor` Check 16 extension) → W1.5 → W1.6 → W1.7. Do NOT re-run `/g-review` on W1.3 or re-plan/re-implement W1.1–W1.3. **ADR-006 implementation (baseline sweep + flight recorder + wave-close integrity + NEEDS_GLOBAL + solo-wave + frozen 3-verb deny) is a POST-M-audit milestone slice — do not wedge into W1.** **4 W1.3 review minors to fold downstream:** (W1.4) post-commit-cleanup lacks check-commit.sh's sed command-field-extraction tier — harmless, ADR-004 content-binding covers it; (W1.6) observe.sh sed not escaped-quote-aware; (W1.5) W4 guard-idiom variance across the six hooks; (W1.5) W5 — extract workflow-checkpoint's hand-rolled stamp parser to `hooks/lib/` so the sentinel format gets ONE reader (strongest advisory). Report: `g-docs/agent-output/review/code-lead-2026-07-16.md`.
...
- W1 (P0): ADR-004 (sentinel↔tree binding) + ADR-005 (worktree enforcement) implementation + finding #21 fold-in — 37 tasks / 8 waves, split into budget-scoped sub-parts (each sized to fit a session's `/g-plan` context-budget gate; sequenced 1→7, run `/g-plan` on each in order):
  ...
  - **W1.4 — Install wiring + drift detection:** `/g-init`/`/g-update` learn new hook files; `/g-doctor` Check 16 extended (tasks 20+21, 22, 19). Depends on: W1.2
  - **W1.5 — Foundation + gate tests:** tests for W1.1 + W1.2 (tasks 23, 24, 25, 26); fold in the W1.1 review minors (4) + W1.2 review minors (g-review SKILL Step 6 literal-HEAD wording; architecture-rule native-git-hook class note → or W2 #18). **NEW (2026-07-15, developer):** generalize `/g-review` Step 1 — if the project defines a local test-runner agent (convention: `.claude/agents/<name>-dev.md`), HQ delegates the suite run to it and receives attested runner output; else inline as today. Dogfooded by the repo-local, never-shipped `g-forge-dev` agent + `g-dev/fixtures/` harness (scaffolded 2026-07-15 — W1.2's sandbox fixtures promoted there; W1.5's new tests should land as g-dev fixtures where they exercise gate behavior end-to-end). Depends on: W1.2
  ...
  - **W1.7 — Suite run + live verification + ledger close:** full suite green, real gated commit through primary tree, real gated commit through a linked worktree, M-audit ledger sign-off (tasks 34, 35, 36, 37; HQ-executed, not delegated). Depends on: W1.5, W1.6
...
**Version:** v3.2.0 (minor — the register is a new capability; renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand)
...
**Re-prioritization:** promoted to the **next buildable milestone** (v2.3.0 at the time of that decision; now v3.2.0 after the M42 + M41 insertions and the M41→G-Proof v3.0.0 rebrand), ahead of the deferred M26 — M26 is spike-gated with nothing depending on it, while M29 is buildable now and strategically central (governance scaled to teams, per M24). Its Phase A doubles as the **de-risking spike for the whole arc** — it answers "is convention enough?" before M30–M32 commit. (M25 stays a parallel compute-gated track.)
...
**Version:** v3.5.0 (minor — Phase A shipped un-versioned; ships when Phase B lands. Follows M37 per the version plan; shared digests build on the memory substrate. Renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand.)
...
**Version:** v3.3.0 (minor — new capability; renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand)
...
**Status:** ⬜ Not started
**Version:** v3.4.0 (minor — the salience substrate + its cross-system wiring; renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand)
...
**Status:** ⬜ Not started
**Version:** v3.6.0 (minor — new skill; renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand)
...
**Status:** ⬜ Not started
**Version:** v3.7.0 (minor — new skill; Phase A ships it, B/C fold in as they land; renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand)
...
**Status:** ⬜ Not started
**Version:** v3.8.0 (minor — new recognized folder class + classifier arm + doctor advisory + intake questions + optional ADR field; renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand)
...
**Sequencing note:** slotted at the tail (now v3.8.0 — renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand) originally to avoid renumbering the planned M29→M39 lane (a rationale since overtaken by the M41 renumber, position unchanged). **Wave 1 is a pull-forward candidate** — the reference-only mis-gate is a live enforcement fail-open, thematically M-audit's own territory, and could ship as a `v2.2.x` patch ahead of the arc if the developer wants the gate honest sooner.
...
### M41 — G-Proof Rebrand + /g-release + /g-changelog (gated release pipeline)
**Status:** ⬜ Not started
**Version:** v3.0.0 (**semver major** — the product rebrand **G-Forge → G-Proof** is a breaking change for consumers: `/plugin install g-forge` → `g-proof`. Also carries new release commands + skills + a `/g-doctor` version-consistency check. **PULLED FORWARD 2026-07-15 (developer)** from the tail to immediately after M-audit — releases happen constantly, the mid-milestone-bump slip already fired once, and the milestone accreted `/g-changelog` + the README pass. **Rebrand folded in + bumped to v3.0.0** in a later session; everything downstream renumbers into the 3.x line. Full rebrand scope lives in `g-docs/milestones/M41.md` — that file is the source of truth; the waves below predate the rebrand fold-in and cover only the release pipeline.)
**Goal:** **Rebrand G-Forge → G-Proof** (semver major) and, in the same milestone, make cutting a release a **single gated step** instead of a manual, multi-file, error-prone ritual — so the rename ships behind real release machinery, not a hand-edited bump. Full rebrand scope (rename, README rewrite, graphics, field comms) + per-wave done conditions live in `g-docs/milestones/M41.md` — that file is the source of truth for `/g-plan`; the summary below defers to it. Distribution is straight off `main` (no tags, no CI) — the version field in `plugin.json` **is** the "latest available" signal that `/g-update` and the daily `workflow-checkpoint.sh` check advertise to every installed project — so a wrong or premature bump ships immediately. `/g-release` owns that bump with preconditions and consistency.
...
**Scope (waved — full task breakdown + done conditions in `g-docs/milestones/M41.md`; waves match that file):**
- **Wave 1 — Rename & manifest:** repo `hllrm/g-forge` → `hllrm/g-proof` (GitHub auto-redirects old URLs); `plugin.json` + `marketplace.json` name/display-name/description + version → **3.0.0**; sweep internal `g-forge` → `g-proof` (CLAUDE.md, skill frontmatter, hook headers) — **historical retros + dated ADRs left as-written** (rename globset excludes them); CHANGELOG heading + v3.0.0 anchor.
...
- **Wave 3 — README rewrite (from scratch, persuasion-ordered):** ~250–300 lines (from ~700), reference→persuasion order; tagline, producer's-seal analogy, **gate GIF** (commit blocked → `/g-review` → MERGE READY → commit passes; fallback animated-SVG/mermaid/text), before/after table, 5-minute install, FAQ, roadmap table. Reuses Wave 2's consistency check for counts. (The lighter `/g-review` Step-6 close-out README-currency mechanism — optional, behavior-change-gated — is the *reusable* pass for future milestones; M41's own README is a one-time full rewrite.)
- **Wave 4 — Field communication:** migration notes + announcement for known installs (`hllrm/G-Cash`, an external consumer, marketplace listing) — repo renamed, plugin name changed, **run `/g-update` to resync**; flag Confluence `109314050` / Drive refs for later cleanup. Done = consumers notified + confirmed able to `/g-update`; no broken links.

**Explicitly out of scope:** publishing pipelines/CI, signing, changelog generation **from raw commits** (generation from the curated ledger/review record is Wave 0; `git log` is never a source — superseding the original blanket exclusion when `/g-changelog` was folded in, 2026-07-15), auto-deciding the semver bump (the developer states major/minor/patch; `/g-release` enforces consistency, not the decision), auto-applied README rewrites (always drafted + nodded).

**Depends on:** — (independent; composes with `/g-roadmap`'s milestone close and finding #20's "green run with evidence").

**Sequencing note:** pulled forward (2026-07-15, developer approval; now **v3.0.0** with the G-Proof rebrand folded in) — sequenced immediately after M-audit-2026-07 closes, before M42. The original "tailed to avoid renumbering" trade lost once the milestone accreted `/g-changelog` + the README pass and v2.2.2 came into sight.
...
- *Rename churn / broken clones* (med) → GitHub auto-redirects old URLs; no force-push; field installs resync via `/g-update`. Historical retros + dated ADRs left as-written (rename globset excludes them).
- *Downstream fork breakage* (med) — G-Cash, [redacted], the alveria fork carry hardcoded `g-forge` refs → Wave 4 migration notes + a tested `/g-update` path; done condition includes "consumer can resync," not just "announced."
- *Version-jump culture shock* (low) — v2.x → v3.0.0 reads as major features → changelog + announcement state plainly: v3.0.0 = rebrand (semver major on the plugin-name break), same enforcement model, same feature set.
- *Gate GIF can't be captured* (low) → fallback to animated SVG / mermaid / detailed text sequence; less visceral, still accurate.
...
**Status:** ⬜ Not started
**Version:** v3.1.0 (minor — new planning capability + gate; renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand)
...
**Sequencing note:** ID tailed at M42, but **sequenced early — after M41 (which the 2026-07-15 pull-forward slotted directly behind M-audit), before M29** — field-validated by the first real consumer's first smoke test, cheap (skill prose, no enforcement code), and it protects every project *birth*: the blind spot compounds worst at kickoff. Takes **v3.1.0** (renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand; see version plan).
...
**Version:** v3.9.0 (minor — new skill + new operative variable; renumbered into the 3.x line by the M41→G-Proof v3.0.0 rebrand. **Parallel-friendly / pull-forward eligible** — independent of the multiplayer arc, touches only g-init/g-execute skill prose + a new skill; slots into any gap on developer request, like M36.)
...
v0.8.1 → v0.9.0 (M8) → v0.10.0 (M9) → v0.11.0 (M10) → v0.12.0 (M11)
       ...
       → v2.2.2 (M-audit-2026-07 — Forge Integrity) → **v3.0.0 (M41 — G-Proof rebrand + /g-release + /g-changelog + README; semver major, plugin-name break)** → **v3.1.0 (M42 — Planning Cold-Start Integrity; sequenced early despite tail ID — G-Cash field report)** → v3.2.0 (M29 — multiplayer phase one) → v3.3.0 (M35 — Memory Forge) → **v3.4.0 (M37 — Salience Layer propagation; M36 approach/ADR is design-only, gates M37, slots early)** → **v3.5.0 (M33 B–D — the Roundtable)** → M34 (cross-session dependency tracking + pull/push orchestration, arc spine) → M30–M32 mechanics reconcile against M34 → **v3.6.0 (M38 — G-Report)** → **v3.7.0 (M39 — G-tweak)** → **v3.8.0 (M40 — Reference Convention; Wave 1 pull-forward candidate to a v2.2.x patch)** → **v3.9.0 (M43 — Operator Controls: /g-settings + inspection cadence; parallel-friendly, pull-forward eligible)** · M26 (Provable Wave Dispatch) deferred to its own minor when its spike clears · M25 benchmark ships its number when run
```
