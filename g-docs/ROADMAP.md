...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 RELEASED · SUITE 564/18 GREEN · TWO SESSIONS RECONCILED · 2026-08-16
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **FIRST GITHUB RELEASE EVER CUT — v2.4.0**, lightweight tag `v2.4.0` → `ee4cc6b1aa94a9e0992ebf91e845b670a993dd16`, verified live via `git ls-remote --tags origin`; body is the 2.4.0 CHANGELOG section. Backfilling the other 53 released versions was deliberately declined as noise (watcher notifications, fictional dates). Repo description set. · **G-RULES §D corrected to match: "Git tags: not used" was FALSE the moment that tag pushed** — §D now states tags are used from v2.4.0 onward for GitHub releases only, CHANGELOG heading still authoritative, pre-2.4.0 deliberately un-tagged; the *Release commit sequence* gained **step 8** (tag + cut release), which previously ended at step 7 and would have shipped every future release untagged. Both the shipped rule and its `.claude/rules/` mirror updated. · **`/g-resume` gained Step 0 — origin sync before re-hydration** (`skills/g-resume/SKILL.md`), the read side of the §A7 seam no longer trusting a possibly-stale clone. Sub-steps 0a–0g: existence gate · upstream-first ref resolution (config before string-splitting `@{u}`) · bounded non-interactive fetch · post-fetch re-check · total four-row classification · gated local `git merge --ff-only` (never `git pull` — no second network call at the first prompt) · **0g Record axis**, a separate briefing line, because `Freshness:` speaks only for the current branch and a feature-branch session level with its own upstream was reading `synced` while the handoff on mainline had moved on. 14 Freshness literals, closed set, verified equal to the Step 3 template both directions by a method that cannot false-green. **Six review rounds: 16 → 19 → 11 → 10 → 9 → 2 findings, every one closed.** · **Suite timing bounds consolidated into a new tracked `tests/lib/timing-bounds.sh`** — `GF_HOOK_STDIN_GUARD_MS=65000` (2× the 31.9s worst observed) and `GF_LIB_READ_WINDOW_MS=6000` (2× 2876ms). Root finding was duplication, not the numbers: the hook-guard bound was widened in one suite and not its twin, and that twin went red next run. A cross-referencing comment was tried and rejected for a single definition. Zero duplicated timing literals remain in `tests/`. · **The four workflow-checkpoint emoji failures are SOLVED, mechanism named:** Windows `wchar_t` is 16-bit, MSYS `mbrtowc` cannot represent above U+FFFF, GNU grep 3.0 rejects the sequence — 4-byte emoji (🪑 🔴 🟡 📋) never matched, 3-byte (⚠ ✓) always did. Fixed at the helper: `check_match` greps under `LC_ALL=C`. Sound across all 50 call sites (every pattern is literal text plus `.*`; no bracket expression, no POSIX class, no `-i`). **No hook output changed.** · Deleted the parked synthetic fixture `g-docs/patterns/latest_old.md` (self-declaring, outside the directory's two-name model, queued for deletion in the prior retro). · Archived todo row 8 to `todo-done.md`; its prescribed fix shipped here.
Next up:          · **Decide the emoji question properly** — `✓`/`⚠` earn their place as CLI status markers; 🪑 🔴 🟡 📋 🎯 🔄 are decoration that cost four red assertions and a Windows diagnosis. Removing them means moving every assertion that reads the banner. Deliberately NOT done tonight. · **n8n disclosure is a live decision, not a settled one:** README's Evidence section links the adversarial inbox while leaving the vendor unnamed, but verdict files carry `source: n8n adversarial audit` and 9 commit subjects name it — one `ls` away. Name it or scrub it; the half-state is the worst of both. · n8n filename sanitization (slugify at source: `[A-Za-z0-9._-]`, no colons/slashes/spaces) — still hard-fails `git merge` on Windows every run · the repeat-fire retry loop · M49 scope decision on evidence · **the M47 close swarm still owed** (`/g-telemetry` · `/g-align` · `/g-wiki` refresh · `/g-doctor` odd-count) · `/g-plan` M48 (now carrying the suite-runtime and suite-ordering fixes) · delete the consumed verdict files once weighed (enumerate the directory, never work from a count) · re-author the three deferred pattern fixes · inbox trust-boundary ADR · M38 delivery decision · field-reports registration · intake rows (a)–(h) · task 6 M45 rider · task 7 remainder.
Active context:   · **TWO CLAUDE SESSIONS RAN THIS TREE CONCURRENTLY AND HAVE BEEN RECONCILED — read this before trusting any earlier record.** Session A shipped the v2.4.0 release, the README pass and the suite attestation; session B shipped `/g-resume` Step 0. Neither clobbered the other and the merge is additive, but **each attributed the other's edits to its own agents**: the prior handoff's headline accusation, that `g-forge-dev` mutated two test files out of scope in a §C violation, was WRONG — those edits were session B's, made deliberately with measurements. That accusation is retracted here. The genuine agent finding stands and is unrelated: **confabulation occurrence 4** (todo row 9), a runner claiming `565 passed, 5 failed` over a 17-row table summing to 537, dropping a green 22-assertion suite and inventing a number to fill the hole. HQ-sums-the-table caught it, as it has all four times, and has prevented it none of them. · **SUITE = 564 / 18 / GREEN**, summed from a per-suite table with exactly one authoritative result per suite. Beware double-counting when suites are run in overlapping batches — a naive sum across two overlapping run logs produced a phantom 805 this pass and was caught before it reached the record. · **Concurrency lesson worth keeping: two sessions on one working tree cannot both attest a clean suite** — their runs contend for CPU and the timing assertions measure the contention, not the code. Three of this pass's five original failures were bound problems the contention exposed rather than caused. · Review: code-lead gated the combined tree, MERGE READY conditional on suite reconcile plus two doc-record blockers, both now closed (this handoff was one of them — it previously announced `SUITE RED · AN AGENT MUTATED THE SUITE`). README independently verified clean against disk: 19 agents · 38 skills = 38 router tokens · 12 ADRs · 56 profile dirs · 7 hooks + 6 libs · casing matches `git remote -v` · all 5 Evidence links resolve. · MIXED changeset — needs BOTH `.claude/g-forge-approved` and `.claude/g-forge-docs-approved`. §D note: the `/g-resume` change is a feature edit landing on `main` without a branch, accepted knowingly under deadline. · LOCAL STATE: review-holds = 24 · voice `gian` · telemetry `cautious` · inbox holds REJECT verdicts, `ls g-docs/inbox/adversarial/` for the live count, never trust a number written here. · Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass, and never repeat this line's own leading label text anywhere else on it (`workflow-checkpoint.sh` strips with a greedy BRE through the LAST occurrence; `pre-compact.sh` uses awk block capture — the two disagree when violated, observed live 2026-07-26 and 2026-08-15).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**Depends on:** — (sequenced **fourth** in the 2.5 build order per [ADR-012](decisions/012-g-forge-2.5-final-release-scope.md), moved from third by the 2026-08-17 M50 amendment: after M48's hardening of the current pipeline **and after M50**, whose agent contract map is a required input to this milestone's design ADR — M45 must decide whether the 11 reviewer agents get scoped Write grants or HQ writes their records (todo row 6), and M50 produces the evidence for that call. Before M38/M40/M43/M41. *The 2026-07-15 "release machinery first" ordering is retired — M41 now cuts the release last.*) Independent of M36/M37, both fork-bound in `g-docs/g-proof-roadmap.md` (ships flat-depth; the depth selector arrives, if ever, with G-Proof's salience layer).
...
- **Wave 4 — Governance cadence: the health passes become gating, not hopeful** *(added 2026-08-17, developer — "these passes must be mandatory and gating if the full g-forge thing is up, or setup in the upcoming settings")*. New variable `.claude/governance-cadence`, tier-defaulted: **`full` = gating · `balanced` = nudge-only (today's behaviour) · `light` = silent**, overridable per-pass via `/g-settings`. Covers `/g-align`, `/g-telemetry`, `/g-trim`, `/g-doctor` — the passes that report on G-Forge's own health.
  ...
  - **Per-pass cadence decision, made explicitly rather than by default** — the four layers this milestone must cover, with today's measured state: **eval** (`/g-align` ✓ stamped · `/g-telemetry` no stamp · `/g-doctor` no stamp · `/g-blast-radius` **2 runs total** despite being a named core instrument — now structurally required by M45 and M50's §B checks, so the fix is upstream) · **intelligence** (`/g-patterns` no cadence · `/g-forecast` event-driven at plan approval, correctly not clock-driven · **`/g-identity` has NEVER run — no `g-docs/identity.md` exists**, a shipped capability that is dead in practice: wire it to a cadence or record it as on-demand, but decide) · **self-repair** (`/g-update` event-driven ✓ · `/g-trim` broken as above) · **gates** (`/g-review`, `/g-doc-review` already hard — unchanged).
...
- **Suite wall-clock is a review-gate cost, not just a test concern.** Three compounding causes measured 2026-08-16 on this repo: roughly 9 minutes of deliberate waiting on abandoned-stdin guard tests (`test-class-split-invariant.sh:121` looping `< <(sleep 300)` over six hooks at `GUARD_WINDOW_MS=65000`, plus `test-check-commit.sh:316,357` and `test-stdin-read.sh:80`); MSYS fork overhead across 564 assertions; and 18 independent suites run serially with the two heaviest (`test-workflow-checkpoint.sh` 81, `test-worktree-resolve.sh` 42) last, so the run always feels hung exactly when it is nearly done. Fixes in order of cost: reorder or parallelise the suites, then make the guard windows env-injectable so tests prove the timeout at 2s while production keeps 65s. **Never shrink the constants themselves** — the same architecture note records `GUARD_WINDOW_MS` being widened 8000→20000 because real MSYS overhead breached a tighter bound. Related: todo task 7 already carries the `< <(sleep 300)` sleeper-reaping rider
...
**Status:** ⬜ Not started (folded into 2.5 by developer decision 2026-08-17 at `/g-intake` triage — amends the ADR-012 milestone list a second time)
**Version:** v2.5.0 (rides the freeze release)
...
- **Agent-count consumers derived, never typed.** `/g-telemetry` Step 5b hardcodes 18 agent names against **19** in `agents/` — `doc-reviewer` is invisible to coverage permanently; its own coverage table template carries only 17 rows. Done condition is a **test that fails when `agents/` diverges from any consumer**, not a corrected list
  - **Scope boundary — "derive, don't type" applies to CODE CONSUMERS ONLY, never to prose** (developer, 2026-08-17). A consumer that types a list goes *blind*: `/g-doctor` Check 16's typed 7-hook list means an 8th hook escapes the very drift detector meant to catch it. A **document** that types a count is merely occasionally wrong — and the "fix" of writing *"read the directory for the current set"* instead is worse documentation, not better: it trades a useful fact that can go stale for a useless one that cannot. Caught live this session, in a `/g-wiki` edit that replaced "12 ADRs (001–012)" with a pointer and was reverted. **For prose the rule is the inverse: keep the concrete number, and add a mechanical currency check that fails when it drifts** — the shape the README's verified-against-disk counts already use. Any audit item in this milestone that touches documentation (e.g. §I's "three hook scripts" line) takes the prose rule, not the consumer rule.
...
- **Agent contract map:** frontmatter tool grants vs what each body instructs, across all 19 agents; the dispatch graph across the 17 skills that dispatch; nesting constraints (`review-orchestrator` must run as root session agent). Feeds M45's design ADR, which must decide the same question for its 11 reviewer agents (todo row 6). **Two known instances recovered from `g-docs/agent-output/audit/audit-4-structure.md` on 2026-08-17** — W3: `review-orchestrator.md:22` holds `Agent(…doc-writer)` and dispatches it to *write* docs mid-review, contradicting G-RULES §G's validate-don't-generate split and "review agents output findings, never fixes" · W4: five agents (`architecture-enforcer`, `code-lead`, `code-reviewer`, `doc-reviewer`, `security-auditor`) pin `model: opus` with no in-file justification — §A1 sanctions pinned per-agent tiers, so the defect is the missing WHY, not the pin
...
**Depends on:** M48 (sequenced **third** in the 2.5 build order — M47 ✅, then M48's hardening, then M50, then M45). **Explicitly excludes — owned elsewhere, verified 2026-08-17 against both milestone entries:** the `/g-blast-radius` in-memory producer change (`skills/g-blast-radius/SKILL.md:111`) and review-scoping-by-changed-files → **M45**; the "don't route a deterministic suite run through an agent" fix and the suite wall-clock work → **M48**. Scoping this milestone from the developer's phrasing alone would have duplicated all three.
**Feeds:** M45's reviewer record-write design ADR (todo row 6).

**Premortem:**
- *Scope collides with M45/M48 and work is done twice* (**high** — nearly happened during this milestone's own intake) → exclusion boundary written into `Depends on:` above, re-verified against both entries before any `/g-plan`.
