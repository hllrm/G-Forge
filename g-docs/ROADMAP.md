...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | synced to `9688e95` · W1.1 shipped (shared foundations) · ADR-004/005 verified live · review-flow follow-up resolved @ v2.2.1 (v2.2.2 pending milestone close)
⚠ FIRST (fresh session): run W1.2 — **commit gate + native pre-commit hook** (tasks 3+4+5, 6+7+8 in `g-docs/plans/w1-1-shared-foundations.md`'s sibling sub-plans). `/g-plan` this next: swap `hooks/check-commit.sh`'s detection/worktree-integration onto `hooks/lib/commit-detect.sh` + `hooks/lib/worktree-resolve.sh` (both shipped in `9688e95`), and build the new native `pre-commit` hook (tree/HEAD compare via the `commit_sentinel_ts`/`commit_sentinel_head`/`commit_sentinel_worktree` stamp `/g-review` now writes, first-commit special case, classifier reuse). W1.1's foundations are done and verified — do not re-plan or re-implement them.
...
- W1 (P0): ADR-004 (sentinel↔tree binding) + ADR-005 (worktree enforcement) implementation + finding #21 fold-in — 37 tasks / 8 waves, split into budget-scoped sub-parts (each sized to fit a session's `/g-plan` context-budget gate; sequenced 1→7, run `/g-plan` on each in order):
  ...
  - **W1.2 — Commit gate + native pre-commit hook:** `check-commit.sh` detection/worktree swap, new native `pre-commit` hook (tree/HEAD compare, first-commit case, classifier reuse) (tasks 3+4+5, 6+7+8). Depends on: W1.1
  ...
  - **W1.5 — Foundation + gate tests:** tests for W1.1 + W1.2 (tasks 23, 24, 25, 26). Depends on: W1.2
