...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | M48 SPLIT a–e · M48a WAVE DONE, REVIEW PENDING · 2026-08-19
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · Prior-pass items (ADR-013 verify · first `/g-trim` · README 2.5 rewrite) committed and pushed `b70f524`. · **M48 split into M48a–M48e** at the `/g-plan` budget gate (developer call; sub-milestone detail now in the ROADMAP M48 entry — the durable record; order fixed a→e). · **M48a Wave 1 complete, 4/4 DONE via `/g-afk`**: required fix-closure sweep (Step 2b) + round-3 note (Step 2c) in `skills/g-doc-review/SKILL.md` · volatile-fact heuristic (lens 5, WARNING default, Currency escalation) in `agents/doc-reviewer.md` · falsifiability rule in `rules/g-rules/H-testing.md` Tier 1 · `tests/run-all.sh` (glob-derived set, subset-checked heaviest-first hint, summed totals; its two proof runs both 564/0/18 identical to serial — but NO wall-clock gain: reordering can't shrink a serial sum, parallelism deliberately off; the real lever is M48b/c's fast guard windows — caveat recorded in the plan Progress notes). · Forecasts written: `g-docs/forecasts/m48-review-pipeline-hardening.md` (family) + `m48a-doc-gate-teeth-and-runner.md`. · **Session closed at the context gate — the developer corrected §A7: unsafe past 25% of the window USED (not 25% remaining); saved to auto-memory** — so M48a's review was cut before code-lead dispatch and hands off below.
Next up:          · **FIRST: `/g-review` the uncommitted M48a changeset** (4 modified: `skills/g-doc-review/SKILL.md` · `agents/doc-reviewer.md` · `rules/g-rules/H-testing.md` · `g-docs/ROADMAP.md`; 3 new: `tests/run-all.sh` + 2 forecast files) — suite runs fresh and DIRECT (`bash tests/run-all.sh`, never via agent; it must post-date the wave's edits since `test-review-severity.sh` greps `agents/doc-reviewer.md`); installed-copy drift pre-verified clean 2026-08-19 (hooks/*.sh + lib byte-identical; `pre-commit` lives in `.git/hooks/` and `hooks.json` is the manifest — both out of that check's scope); scope the code-lead dispatch by exclusion (named files, attested baseline off-limits, no architecture re-derivation). On MERGE READY: mixed changeset ⇒ BOTH sentinels, identical stamp, then commit per ADR-004, push, mark M48a ✅. · **Then `/g-plan` M48b** (see ROADMAP M48 entry). · **AFK residue: decide the `permissions` block added to `.claude/settings.json` 2026-08-19** — the deny-list is harmless to keep; the `allow` list (`Bash(*)` etc.) auto-approves everything for future sessions and should probably be stripped now AFK is over. · **§A7 correction as a doc task**: shipped `rules/g-rules/A-session.md` §A7 + `hooks/workflow-checkpoint.sh` amber wording still say "remaining below 25%" — rewrite to 25%-USED per the developer's correction (fold into an M48 sub or a standalone doc commit). · M43 W4 governance-cadence ADR — before M43's build (slot 6). · CARRIED: n8n slugify on the n8n side · ROADMAP status emoji deliberately kept · `commands/g-forge.md:8` env-specific cache path · inbox trust-boundary ADR · M38 delivery decision · intake rows (a)-(h) · task 6 M45 rider · task 7 remainder · `CLAUDE.md:32` date stamp (local, ADR-013: derivable or drop) · doc-reviewer memory stale-count entry (manual curation).
Active context:   · **Context gate corrected by the developer 2026-08-19: unsafe past 25% of the window USED** — act on usage, not remaining; auto-memory `context-threshold-25-percent-usage` holds it; shipped rule text still stale (doc task in Next up). · **ADR-013 governs doc edits**: consumers derive their lists; documents keep concrete numbers, pinned or omitted; no file:line/count claims not verified the same turn. · M48a plan + wave records: `g-docs/plans/m48a-doc-gate-teeth-and-runner.md` (Progress: wave complete, review pending) + `g-docs/agent-output/wave-1-m48a/` (4 reports incl. run-all's two pasted proof runs). · M50 ground truth unchanged from 2026-08-18 pass (telemetry TABLE 17 vs 19; `g-specialize` 48 typed vs 56 dirs, intended set UNSTATED — settle in `/g-plan` M50). · ADR-004: sentinels bind to the STAGED tree — stage, then stamp `commit_sentinel_ts=<write-tree> commit_sentinel_head=<HEAD> commit_sentinel_worktree=<toplevel>`, then commit as a separate call; commit messages via a Write-tool file, never a heredoc. · LOCAL STATE: review-holds 25 · voice `gian` · telemetry `cautious` · suite 564/18 GREEN attested 2026-08-19 POST-wave-edits (direct HQ run, table HQ-summed; record: `g-docs/agent-output/wave-1-m48a/post-wave-suite-attestation.md` — valid unless the tree changes before review) · last-trim 2026-08-18. · Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass, and never repeat this line's own leading label text anywhere else on it (`workflow-checkpoint.sh` strips with a greedy BRE through the LAST occurrence; `pre-compact.sh` uses awk block capture — the two disagree when violated).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**Status:** ⬜ Not started — **split into M48a–M48e on 2026-08-19** (developer decision at `/g-plan`'s budget gate: the monolithic plan estimated ~74 exchanges against a ~33-exchange session budget; split along the wave-dependency structure so each sub fits one session with no mid-plan handoff, each closing through its own review + commit. Execution order is fixed a→e — b's lib overrides must exist before c wires them into fixtures. All subs ride v2.5.0; the split is sequencing only, no scope change. Sub-milestone task detail below is the durable record — the per-sub plan files in `g-docs/plans/` are gitignored scratch.)

**M48a — Doc-gate teeth & suite runner** ⬜ · grep-the-literal-fact sweep + round-3 consolidation note as a required `/g-doc-review` step (doc-reviewer executes the sweep itself, in Step 2, under its own record-scoped `Write` grant, recording evidence in its per-run record at `g-docs/agent-output/review/doc-reviewer-[YYYY-MM-DD]-[request-slug]-r[N].md`; Step 2b is HQ's check that the record carries it, not the sweep itself; a surviving stale copy or a missing sweep → DOCS HOLD) · doc-reviewer volatile-fact heuristic (in-flight process counts in a reviewed doc are a smell; ADR-013 remedy is pin-with-a-test-first or omit; pointer language to the owning record is this contract's own addition) · falsifiability comment rule in `rules/g-rules/H-testing.md` (applies in projects with an executable test suite; shipped source only) · `tests/run-all.sh` (derives suite set from the `tests/test-*.sh` glob per ADR-013, front-loads the two suites that would otherwise run last in glob order so the big suites don't tail the run, parallelism deliberately off, must prove identical totals vs serial).

**M48b — Lib overrides & first test fixes** ⬜ · env-injectable timeout override in `hooks/lib/stdin-read.sh` (unset = byte-identical; production call sites untouched) · fast test-time guard-window constant in `tests/lib/timing-bounds.sh` (additive, WHY comment, never replaces `GF_HOOK_STDIN_GUARD_MS`) · audit-7 F4: `test-g-doctor-drift.sh` derives the cksum snippet from shipped `skills/g-doctor/SKILL.md` instead of a hand-copied mimic · audit-7 H3+H9: grep-pin the g-init settings.json template matchers and `hooks/hooks.json` = `{"hooks": {}}` in `test-lib-install-completeness.sh`.

**M48c — Code-gate teeth & new suites** ⬜ · grep-the-literal-fact sweep + round-3 note as a required `/g-review` step (recorded in code-lead's review record) · wire the fast override into the abandoned-stdin fixtures (`test-class-split-invariant.sh` six-hook loop, `test-check-commit.sh` cases 23/25; guard-deleted probe must still go RED; re-sum counts from Results lines) · audit-7 H4: version-agreement suite (`plugin.json` vs `marketplace.json`) · audit-7 H5: router-token ↔ `skills/` dir parity suite (derive-and-compare both sets at runtime, never a typed list).
...
- **Agent-count consumers derived, never typed** — governed by [ADR-013](decisions/013-derive-in-consumers-keep-counts-in-prose.md) (two rules: consumers derive from the directory; documents keep concrete numbers). Live defect: `/g-telemetry`'s coverage table types 17 agent names against 19 in `agents/` (`doc-reviewer`, `feature-implementer` uncountable), and the committed 2026-08-17 report shipped blind to `doc-reviewer`. Same class, latent: `/g-doctor`'s typed hook lists, `/g-specialize`'s typed stack lists (its frontmatter list's only consumer should be a `[ -d profiles/<arg> ]` test — delete the list). Done condition is a **test that fails when any typed list diverges from its directory**, never a corrected list.
  - **v2.5 scope fence:** fix the known consumers only; parity assertions land in their own suite file so a red parity suite can't destabilise the 18 green ones.
