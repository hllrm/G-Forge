...
⚠ FIRST: **/g-plan W1.7 in a FRESH session — gating clone-first checkpoint + live verify: install the gating pair (check-commit.sh + native pre-commit) via the ADR-008 clone-first path, then FIRST REAL RUN of the stamped-sentinel + native pre-commit flow. Fold into W1.7's live-verify checklist: (a) script-indirection sentinel bypass (script-file commits dodge BOTH argv sites — gate and post-commit-cleanup; the native hook closes this, verify precisely); (b) the W1.6 route-forward minors on the W1.6i ledger row (5 headerless suites for the #20 cross-check; stale session-start:33-34 comment; --exclude-dir=lib breadth; bare `full` pattern workflow-checkpoint:193; CLAUDE.md ".claude/ git-ignored" claim vs tracked g-forge-dev.md; suite wall-clock shared-fixture idea).** Before W2 executes: verify ADR-007 from a fresh window. W1.5g Pass 3 (Waves 6–7, Spike S1 probes+report) remains DEFERRABLE — cache-state restore mandatory; plan `g-docs/plans/w1-5g-self-host-integrity.md` · rollback baseline `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-21\` (durable, byte-verified). Standing cautions until W1.7 lands: installed gate is v2.2.1-era (#21-class false-positives — script-file commits, path-free messages; fired again live this pass on an HQ probe string); sentinel auto-consume broken via script indirection — clear by hand post-commit; do NOT run stock /g-update (self-host mechanism is the install path). Do NOT re-plan/re-implement W1.1–W1.6. **ADR-006 implementation stays a POST-M-audit slice.** stash@{0} confirmed superseded — droppable, developer's call.
