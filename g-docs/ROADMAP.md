...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | W1.5a–W1.5g SHIPPED (all reviewed MERGE READY, committed) @ v2.2.1 (v2.3.0 pending milestone close — upgraded from v2.2.2, developer 2026-07-18)
⚠ FIRST: **W1.6 next (/g-plan in a fresh session — ⚠ flagged oversized, expect split). Before W2 executes: verify ADR-007 from a fresh window. W1.5g Pass 3 (Waves 6–7, Spike S1 probes+report) remains DEFERRABLE — slot when convenient, cache-state restore mandatory.** Pass 1 (Waves 1–3) COMPLETED 2026-07-21 — see LATEST. Pass 3 = Waves 6–7 (Spike S1 probes+report), deferrable. Plan `g-docs/plans/w1-5g-self-host-integrity.md` (gitignored local — Progress table current: waves 1–3 complete) · forecast `g-docs/forecasts/w1-5g-self-host-integrity.md` 90% High with the standing calibration caveat. Execution cautions still live: do NOT run stock /g-update (Wave 4 IS the install, via the new mechanism); rollback baseline = `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-21\` (durable, byte-verified, restore command in `g-docs/agent-output/wave-w15g-1/durable-snapshot.md`); gating pair (check-commit.sh + native pre-commit) explicitly NOT installed — W1.7; #21-class: install commands from script files; on overreach full-file diff, never spot-revert. ADR-008 VERIFIED 2026-07-20 clean-slate (11/11 claim groups hold; note: the 2026-07-19 snapshot sits in session-scoped OS temp — task 12 re-snapshots durable BEFORE install); ADR/ledger/roadmap/retro docs COMMITTED `7479882` (DOCS READY 0b/2w, live-handoff currency warning fixed inline pre-commit; doc sentinel auto-consumed — 2nd consecutive live consumption). Plan-review corrections already folded into the W1.5g bullet: 6 non-gating hooks not 7 (check-commit.sh is gating, W1.7 only), profile agents added to install set, three-class agent canonical mapping (advisory-only for template-instantiated — reviewer to confirm). Do NOT run stock /g-update before Wave 4's install ships (stale-cache footgun live) → then W1.6 — W1.5f SHIPPED 2026-07-19 (guard normalization + #22 pull-forward fix, MERGE READY 0c/0M/3m, attested 210/210+35/35+3/3; NEW minors from W1.5f → W1.6/W2: agent-lifecycle node-tier maps JSON-null agent_type to "null" vs "unknown" (theoretical); worktree-suite retired-token scan enumerates 4 lib files instead of excluding hooks/lib/ as a dir; observe-suite quote-safety fixture puts adversarial chars on line 2 which head -n1 discards — json_escape hostile-input coverage is manual-only) (standing rule: new minors route to W1.6/W2, never back into a slice) → W1.6 (tests for W1.3+W1.4; observe.sh sed escaped-quote item; carried minors: W1.5a newline-boundary + glued-pathspec-fidelity tests; W1.5b shadowed `.md` dir-arm fixtures + invariant grep missing `hooks/pre-commit`; NEW from W1.5d: case-(b) malformed-partial-stamp advisory-delta test + wave-agent doc-writer overreach structural note; NEW from W1.5e: g-review Step 6 `commit_sentinel_ts` run-on bullet split (minor) + W2 look at commit-detect false-positives on non-commit commands *quoting* commit phrases (heredoc report write denied live — #21 residual class); ⚠ oversized, expect split) → W1.7 (gating clone-first checkpoint + live verify incl. FIRST REAL RUN of the stamped-sentinel + native pre-commit path — gating installs ONLY; the non-gating local install [6 hooks + 4 libs + 10 rules + profile agents] moved to W1.5g via the self-host-aware mechanism per ADR-008 — do NOT run stock /g-update locally before that mechanism ships). Do NOT re-plan/re-implement W1.1–W1.5d. **ADR-006 implementation stays a POST-M-audit slice.** **ADR-007 recorded 2026-07-19 (one command per skill — retire shims): verify it against the repo from a fresh window before W2 executes finding #19; W1.5e does not build on it.** stash@{0} confirmed superseded — droppable, developer's call.
LATEST (2026-07-21c): **W1.5g Pass 2 (Waves 4–5) COMPLETE — FIRST LIVE SELF-HOST INSTALL SUCCEEDED; slice reviewed MERGE READY + committed.** /g-resume → /g-execute 4: Wave 4 (g-forge-dev, HQ-supervised) installed the non-gating set from the working tree per ADR-008 — 21 files (6 hooks + 4 libs + 10 g-rules flat-renamed + claude-plugin-architect) byte-identical to source, HQ-verified independently; gating pair untouched (check-commit.sh sha unchanged `43fb0921…`, no native pre-commit created — W1.7 owns both). Wave 5: live extended-Check-16 verify PASS (20 hash-match + 1 expected template-instantiated advisory, 0 Fail) — the Tier-3 evidence per the plan's DoD. /g-review on Pass 1+2: attested 245/245 across 11 suites (g-forge-dev runner, canonical invocation — no session-limit kill this time), code-lead **MERGE READY 0c/0M/1m** (minor: dead scaffold block in the invariant suite lines ~30–38 + unused var → W1.6; template-advisory design note confirmed coherent; drift line correctly visibility-only: the 2 "drifted" files ARE the deferred gating pair). /g-doc-review **DOCS READY 0b/3w** — w1 (missing CHANGELOG entry) fixed inline pre-commit (ADR-008 self-host entry added under Unreleased); w2+w3 (g-doctor frontmatter under-scopes Check 16; g-review Step 1 wording implies broader Check-16 scope than hooks+libs) → W1.6 advisory. Both sentinels stamped identical (ADR-004). The install went live mid-session: this session's own hooks now run the W1.5f-fixed code (#22 agent attribution goes live for the NEXT session's journal). NEW → W1.6: code-lead minor + 2 doc-clarity warnings above. **NEXT: see FIRST (W1.6 → W1.7; Pass 3 deferrable).**
