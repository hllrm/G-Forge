...
**Version:** v2.4.0 (minor — the register is a new capability; shifted +1 when M42 took v2.3.0)
...
**Version:** v2.7.0 (minor — Phase A shipped un-versioned; ships when Phase B lands. Follows M37 per the version plan; shared digests build on the memory substrate. This field previously said v2.5.0 while the version plan said v2.6.0 — realigned to the plan during the M42 resequence.)
...
**Version:** v2.5.0 (minor — new capability; shifted +1 by M42)
...
**Status:** ⬜ Not started
**Version:** v2.6.0 (minor — the salience substrate + its cross-system wiring; shifted +1 by M42)
...
**Status:** ⬜ Not started
**Version:** v2.8.0 (minor — new skill; shifted +1 by M42)
...
**Status:** ⬜ Not started
**Version:** v2.9.0 (minor — new skill; Phase A ships it, B/C fold in as they land; shifted +1 by M42)
...
**Status:** ⬜ Not started
**Version:** v2.10.0 (minor — new recognized folder class + classifier arm + doctor advisory + intake questions + optional ADR field; shifted +1 by M42)
...
**Sequencing note:** slotted at the tail (v2.10.0 after the M42 shift) to avoid renumbering the planned M29→M39 lane. **Wave 1 is a pull-forward candidate** — the reference-only mis-gate is a live enforcement fail-open, thematically M-audit's own territory, and could ship as a `v2.2.x` patch ahead of the arc if the developer wants the gate honest sooner.
...
**Status:** ⬜ Not started
**Version:** v2.11.0 (minor — new command + skill; `/g-doctor` gains a version-consistency check; shifted +1 by M42)
...
**Sequencing note:** tailed at v2.11.0 (after the M42 shift) to avoid renumbering the arc, but a **strong pull-forward candidate** — it directly prevents the mid-milestone-bump slip that just happened, and every release until it lands repeats the manual ritual.
...
**Status:** ⬜ Not started
**Version:** v2.3.0 (minor — new planning capability + gate)
...
**Sequencing note:** ID tailed at M42, but **sequenced immediately after M-audit closes, before M29** — field-validated by the first real consumer's first smoke test, cheap (skill prose, no enforcement code), and it protects every project *birth*: the blind spot compounds worst at kickoff. Takes **v2.3.0**; downstream minors shift +1 (see version plan).
...
v0.8.1 → v0.9.0 (M8) → v0.10.0 (M9) → v0.11.0 (M10) → v0.12.0 (M11)
       ...
       → v2.2.2 (M-audit-2026-07 — Forge Integrity) → **v2.3.0 (M42 — Planning Cold-Start Integrity; sequenced here despite tail ID — G-Cash field report)** → v2.4.0 (M29 — multiplayer phase one) → v2.5.0 (M35 — Memory Forge) → **v2.6.0 (M37 — Salience Layer propagation; M36 approach/ADR is design-only, gates M37, slots early)** → **v2.7.0 (M33 B–D — the Roundtable)** → M34 (cross-session dependency tracking + pull/push orchestration, arc spine) → M30–M32 mechanics reconcile against M34 → **v2.8.0 (M38 — G-Report)** → **v2.9.0 (M39 — G-tweak)** → **v2.10.0 (M40 — Reference Convention; Wave 1 pull-forward candidate to a v2.2.x patch)** → **v2.11.0 (M41 — /g-release; strong pull-forward candidate)** · M26 (Provable Wave Dispatch) deferred to its own minor when its spike clears · M25 benchmark ships its number when run
```
