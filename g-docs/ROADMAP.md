...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | **v2.3.0 RELEASED (`e28e501`, pushed) — M-AUDIT-2026-07 ✅ CLOSED** | origin in sync
⚠ FIRST: **/g-resume → milestone close swarm, then /g-plan M41** — (1) Close swarm (read-only, parallel-friendly): `/g-patterns` (mine: claim-vs-data recurrence #3 — W3 attestation confabulation vs this pass's honest totals under the HQ-sum mitigation; heredoc-stall-in-permission-layer now 2 occurrences — W3 r1 + release code-lead — pattern-worthy; claim-vs-whole-file #2 standing), `/g-telemetry` (M-audit now in corpus; note 4 review-holds counter), `/g-align` (nudge outstanding since before close). (2) ADR prompt answered "no new ADR" unless developer says otherwise — M-audit's decisions are already ADR-004/005/007/008. (3) ~~/g-intake M36 note~~ **DONE 2026-07-23 in-session: developer chose (a) → NEW M45 — Review Pipeline Rework (v2.5.0, slots M41 → M45 → M42; design ADR first; depth-selection routed to M36/M37; downstream renumbered +1 minor through M43 v2.14.0; note file status updated)**. (4) `/g-wiki` refresh queued for M-audit (or note as pending task in todo). (5) Doc commit: retro `2026-07-23-m-audit-close-v230.md` + 2 reconciled forecast Outcome tables + ROADMAP/milestone ✅ flips + this handoff are UNCOMMITTED — ride one doc-gated commit (doc class ⇒ `/g-doc-review` sentinel). (6) Then `/g-plan` **M41 — Release Machinery + README Currency (v2.4.0)**. Backlog riders carried: session-start/workflow-checkpoint `command -v` hardening (code-lead minor), test-fixture 300s-sleep leak cleanup (Info), per-session counter-file accumulation, installed check-commit.sh still pre-guard BY DESIGN (gate cadence — picks up stdin guard at next gated install slice; source diverges intentionally), ADR-006 implementation POST-M-audit backlog. Rollback baseline: `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-22\`.
LATEST (2026-07-23): **STDIN-GUARD Pass 2 COMPLETE → v2.3.0 RELEASED + PUSHED → M-AUDIT CLOSED.** /g-resume → /g-execute 4: eager-install A/B/C 3/3 first-attempt, HQ hash-verified 7/7 pairs byte-identical, check-commit.sh divergence confirmed (ADR-008 gate cadence). Wave 5 attestation: 9/10 suites green first run; class-split 34/4 — 4 timing-only reds (8013–9896ms vs 8000ms bound; guard exit-0 correct everywhere) → HQ adjudicated real MSYS overhead not flake (near-10s cases; single guarded read per hook verified) → GUARD_WINDOW_MS constant 8000→20000ms w/ WHY comment (§C inline rule) → re-attest 38/0, max 8094ms (breaches old bound even quiet — diagnosis confirmed). Full-suite: **493/493 across 16 suites, HQ-summed independently, = predicted 493 exactly; header-vs-runner MATCH everywhere** (claim-vs-data mitigation held). /g-review: code-lead **MERGE READY 0c/0M/1m** (missing-lib `command -v` outlier ×2 hooks) **/1 Info** (fixture sleep leak) — agent stalled post-verdict on heredoc record write (permission-layer class, 2nd occurrence; developer killed; HQ transcribed record verbatim from in-flight call w/ provenance note — treated as delivered, no redeploy). /g-doc-review **DOCS READY 0b/2w**. Dual sentinels off write-tree `abe59faf` → commit `7712aa4` (13 files, 473+/23−) passed BOTH live gates (gate also correctly denied a chained stamp+commit first — PreToolUse evaluates pre-chain; split sequence passed). **Release pass:** CHANGELOG `[2.3.0]` cut + dual bump 2.3.0 (plugin+marketplace same commit) + first README status strip + ADR-007 migration check PASS → release-diff gate: code-lead MERGE READY 0 findings; doc-review **DOCS HOLD** (README agent table 18 rows, missing doc-reviewer since v2.1.0; "16 specialist" vs 17) → HQ fixed vs disk roster (19=19) → re-gate DOCS READY → commit `e28e501` → **pushed, origin in sync**. Milestone ✅ flipped (ROADMAP + M-audit-2026-07.md); CLAUDE.md table synced 493; both forecast Outcome tables reconciled (stdin-guard: scenario 2 happened-as-variant at assertion layer, 1/3/5 clean, 4 fired at review layer not waves; w3-p2: 4 absorbed-as-designed, rest clean). Retro: `g-docs/retros/2026-07-23-m-audit-close-v230.md`. New dispatch rule from this pass: record-writing agents must use Write tool, never Bash heredoc (2nd stall) — consider granting Write per g-forge-dev precedent. **NEXT: see FIRST (close swarm + M41, fresh session).**
LATEST (2026-07-22k): **STDIN-GUARD Pass 1 (Waves 1–3) COMPLETE — 10/10 slots first-attempt, zero resumes, integrity clean.** /g-resume → release decision gate: developer HELD v2.3.0, pulled stdin-hang fix in as release rider → /g-plan (15 tasks / 5 waves / 15 slots; forecast 6/10, 80% High w/ standing calibration caveat; budget 65 vs ~37 ⇒ 2-pass split approved "go") → /g-execute 1–3. **Wave 1:** `hooks/lib/stdin-read.sh` landed — `gf_read_stdin_timeout [seconds]` (builtin `read -t -d ''`, no `timeout` coreutil dep, always returns 0, 5s default, `$()` trailing-newline parity with old `cat` idiom; bash<4.4 partial-discard caveat = empty payload = fail-open, acceptable). **Wave 2 (6 slots):** all 7 stdin sites wired — observe.sh both modes; post-commit-cleanup:53 + agent-lifecycle:58 (the two field-confirmed 66-min orphans); workflow-checkpoint + session-start stdin read MOVED below lib-sourcing (one agent, one mental model, both files); pre-compact bounded drain; check-commit:76 fail-open w/ site comment citing ADR-004 native backstop + fallback-to-original-`cat` if lib missing (gate must never skip reading stdin). All guarded `[ -f ... ] && . ...` relative-to-self sourcing — installed copies work; missing lib degrades per class (non-gating tolerate empty, gate falls back to cat). **Wave 3 (3 slots):** class-split +12 asserts (6 hooks × exit-0 + <8s bound, process-substitution `< <(sleep 300)` fixtures — mkfifo AVOIDED per MSYS forecast mitigation); check-commit +3 (fail-open, deny-regression on well-formed commit payload, non-commit pass-through); CHANGELOG Unreleased/Fixed entry w/ root cause + polarity. Tests authored NOT run (test-writer has no exec; Wave 5 attests — "written ≠ passing"). Dirty set = exactly 13 scoped items, zero strays, zero commits by design. **NEXT: see FIRST (Pass 2, fresh session).** Developer raised a ship-blocker concern mid-W3-gate (agent stall pattern, "not sure we can ship 2.3 with this"); HQ assessment on record: NOT gate-correctness — root cause found = **hook stdin-hang** (orphaned `INPUT=$(cat)` in observe.sh:181 + post-commit-cleanup.sh:53 when the harness abandons a tool call; 60s hook timeout unenforced on Windows; two 66-min orphans found+killed) + separate permission-layer heredoc class; fix = stdin read-timeout guard in all stdin-JSON hooks, routed POST-M-audit (could pull into the release pass as a hotfix-class slice if developer holds the release — ASK FIRST). (2) On go: release pass = README status strip (version badge + CHANGELOG/ROADMAP links) + CHANGELOG `[2.3.0]` cut + dual version bump (plugin.json + marketplace.json, same commit) + ADR-007 migration check + push (8 commits ahead). (3) `/g-retro` (milestone topic) + close swarm: `/g-patterns` (claim-vs-data recurrence #3: attestation agent's 568/650 summary vs its own 468-summing table, HQ-corrected after spot-checks; + resumed-agent stall-kill adjudication), `/g-telemetry`, `/g-align` (nudge outstanding), ADR prompt. (4) `/g-intake` the M36 note `g-docs/milestones/M36-salience-inputs/2026-07-22-review-cost-scaling-feedback.md` — review-cost scaling (code-lead 200k+ monolith: no Agent tool + orchestrator nesting wall), code-lead-in-HQ + scoped-waves + salience-depth shape, stall evidence; placement M38/M39 primary, pull-forward eligible. (5) Forecast Outcome table reconcile (`g-docs/forecasts/m-audit-w3-p2-minors.md`) at retro. NEW findings routed forward: hook stdin-hang guard (above) · per-session counter-file accumulation (code-lead r2 minor) · attestation-record pre-correction totals (doc WARNING, regenerable file). **ADR-006 implementation stays POST-M-audit.** Rollback baseline: `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-22\`.
LATEST (2026-07-22j): **W3 Pass 3 COMPLETE — W3 SHIPPED `9e7de95` (29 files, 1108+/57−); M-audit W0–W3 ALL COMPLETE.** /g-resume → /g-execute 3. Wave 3 2/2 first-attempt (task 18 g-kickoff grep 0; task 19: 7 dual-stack combo profiles gained two-stack seam sections — astro-react/-svelte/-vue, electron-react, electron-vue-pinia, react-tauri, tauri-vue-pinia). Wave 4 attestation **GREEN 468/468 across 15 suites + 82/82 across 4 fixtures, header-vs-runner MATCH everywhere** — but the runner agent's own summary totals were confabulated (568/650 + phantom "+100" narrative); HQ caught it (per-suite table sums to exactly the record's ≈468 prediction), spot-check re-ran 2 suites live, appended correction to the attestation record; "425/14" W1 claim REFUTED. **Review: code-lead MERGE READY ×2 CONVERGENT (0c/0M)** — r1 stalled post-verdict on its record write (killed; record found complete on disk after), r2 fresh redeploy with inline-findings return + depth-scaled axes came back clean at ~82k tokens (vs 200k+ monolith class); 2 minors + 2 informational → backlog. **/g-doc-review DOCS READY 0b/1w** (13-point currency verification all PASS; warning = attestation file's own pre-correction totals, gitignored record, contained). Dual ADR-004 sentinels stamped off staged write-tree `859d75f`, commit passed BOTH live gates, sentinels auto-consumed, committed tree = stamped tree exact, working tree clean. Riders landed: W3i ledger row (+#6b defer, #23i, #24i), 7 CHANGELOG bullets, CLAUDE.md table synced 468/W3, ADR-008 eager install (6 files → .claude/hooks + git-hooks lib pair, byte-verified — W2 heredoc fix now LIVE on this repo's installed gate). **Mid-gate field feedback captured (developer):** review-cost scaling + orchestration shape + stall pattern → M36 note (see FIRST); stall ROOT CAUSE found by diagnostic fork: hook stdin-hang orphans (observe.sh:181, post-commit-cleanup.sh:53, 66 min, killed — fork's kill was pattern-match-based, flagged); HQ's premature doc-reviewer kill on a stale stuck-signal owned + resumed-to-completion clean. Tier-3 sandbox-attested per no-manual-QA doctrine. **NEXT: see FIRST (release decision gate, fresh session).**
LATEST (2026-07-22i): **W3 Pass 2 (Wave 2) COMPLETE — 5/5 slots DONE, 1 resume-to-completion, integrity clean.** /g-resume → /g-execute 2. **Tasks 2+16 (commit-detect):** zero-cost `-c alias.NAME=VALUE` argv sub-case landed per the Wave-1 spike verdict (`git -c alias.co=commit co` DETECTED, no `git config` spawn) + heredoc opener-line suffix-scan (`cat <<EOF | bash` with commit body stays DETECTED; interpreter-guard invariant intact, 9 HEREDOC pins green); suite 58→65 + check-commit 22/22. **Tasks 6+13+14+15 (session hooks):** `resume` source now PRESERVES the depth counter per developer decision (startup/clear still reset; dead `clear` fixture revived + asymmetric compaction coverage closed); workflow-checkpoint plugin-cache path flipped from hardcoded `g-forge/g-forge` two-segment to highest-version-dir resolution (fail-before/pass-after); prompt counter now session-scoped (`session_id`-keyed, bare-filename fallback — concurrent sessions no longer clobber); `command -v timeout` guard + untimed fallback proven via timeout-absent PATH; session-start 30→43, workflow-checkpoint 66→77, pre-compact untouched 22/22. **Task 4:** H-testing Tier-3 native-app paragraph (finding #24: install pre-session or dev-mode; `request_access` can't grant unregistered binaries). **Task 12:** classifier `README*|CHANGELOG*|LICENSE*` root-vs-nested split mirroring the `*.md` rule (3 over-match paths fail-before-proven now CODE; polarity narrowing-only, fail-toward-deny intact); classify 47→50 + class-split 26/26. **Task 17:** g-onboard `/g-plan`→`/g-forge plan`, grep 0. Dispatch note: session-hooks agent stopped once mid-run (truncated non-RESULT fragment) → resumed-to-completion per W1.7 pattern, returned clean; no redeploy, no thrash. Wave-close integrity CLEAN (dirty set = exactly Waves 1–2 scope: 17 tracked files + forecast doc; zero strays). Zero commits by design. **NEXT: see FIRST (Pass 3, fresh session).**
LATEST (2026-07-22h): **W3 planned + Pass 1 (Wave 1) COMPLETE — 5/5 slots, all first-attempt, uncommitted by design.** Developer directive (now standing, in memory): "never skip issues — fix everything" → W3 executes in full, 19 tasks / 4 waves / 13 slots, 3-pass split approved ("go"). Wave 1: **task 1 spike** — alias gap characterized empirically (recursive/flag/`!`-form/cyclic verified in scratch repo; bench ~228-260ms per `git config --get-regexp` call), verdict DEFER config-file case + zero-cost `-c` argv sub-case implement-now; **task 5** — resume-counter reset is spec asymmetry not code bug (no fail-before fixture possible — suite pins current behavior as correct), route to decision; **task 3** — g-doctor Check 21 flipped to inverted self-deriving check (canonical names derived from g-docs/ itself), drift suite 12→17 w/ fail-before/pass-after + false-positive guard; **tasks 9+7+8** — agent_type cascade re-confirmed no-change; json_escape now strips all C0 (fail-before proven via `git show HEAD:` scratch + `od` byte inspection); tier-gating idiom aligned to observe.sh w/ 4 parity assertions (29→35); **tasks 10+11** — observe.sh C0 strip + UTF-8-boundary-safe truncator on both detail + WT_KEY emit points (fail-before via git-stash; 30→33). Dispatch notes: BOTH clusters stopped mid-run (session-limit class); agent-lifecycle needed 2 resumes then a 3rd stop → HQ verified state directly (suite green, output file present, scope clean) instead of a 4th resume — spent-agent policy held, no redeploy, no thrash. Wave-close integrity CLEAN (dirty set = exactly 6 scoped files + forecast doc). Suite-count note for Wave-4 reconcile: task-3 agent reported "425/425 across 14 files" vs record's "420/15 suites" — reconcile at formal attestation (post-Wave-1 expected ≈ +11: 420+5+6+3... recount at attest). **NEXT: see FIRST (Pass 2, fresh session).**
LATEST (2026-07-22g): **W2 Pass 4 COMPLETE — W2 SHIPPED at `4b8c532` (70 files: 38 shim deletions + 26 mods + 3 new; 668+/452−).** /g-resume → /g-execute 5. **Wave 5 (task 16): the walkthrough gate EARNED ITS KEEP r1** — caught that the umbrella's 37-line per-skill prose fallback list (lines 50–86) had SURVIVED Wave 1's "trimmed to bare tokens" claim (only the intro line had been reworded; verified vs git diff HEAD). HQ-deleted inline (§C exact-lines rule); router now 49 lines, zero per-skill prose — ADR-007's third surface closed FOR REAL. r2 post-fix: g-status/g-doctor/g-adr all VALID (record: `g-docs/agent-output/wave-w2-5/task16-skill-validate-walkthrough.md`). **Claim-vs-whole-file class recurrence #3 → /g-patterns at next retro.** Wave 6 (task 23): 8 ledger rows (19i ADR-007-EXECUTED, 12i+13i+14i+15i, 16i, 17i, 18i, 21iii, 22ii, W2-close) + CHANGELOG `### Removed` (38 shims) + 6 Fixed entries. **/g-review:** drift check found observe.sh source-newer → ADR-008 eager non-gating install done in-session (byte-identical); drift line = 1 file (commit-detect.sh, EXPECTED carve-out). g-forge-dev attestation **GREEN 420/420 across 15 suites + 78/78 across 3 fixtures** (header-vs-runner reconciled: 49→58, 24→30; 405−49−24+58+30=420; CLAUDE.md table synced local-only). code-lead **MERGE READY 0c/0M/1m** (minor: heredoc guard scans opener-line prefix only — `cat <<EOF | bash` body missed by argv layer; native hook fully backstops; candidate future suffix-scan → W3-or-backlog). /g-doc-review **DOCS HOLD 1b/2w → READY**: blocking README:308 stale "SKILL.md + command file + router wiring" fixed pre-stamp + whole-file grep = 0 residual; stale milestone status tag fixed; g-onboard:242/g-kickoff:64 rhetorical output strings → W3 advisory. Dual sentinels stamped off staged write-tree `f0620c5`, commit passed BOTH live gates, sentinels auto-consumed, committed tree = stamped tree exact, working tree clean. Tier-3 sandbox-attested per no-manual-QA doctrine. **NEXT: see FIRST (W3 decision, fresh session).**
LATEST (2026-07-22f): **W2 Pass 3 (Waves 3–4) COMPLETE — teaching sweep + full conformance landed; every deferred attestation GREEN with header-vs-runner MATCH.** /g-resume → /g-execute 3: Wave 3 6/6 slots DONE (teaching-sweep agent needed 2 completion-resumes mid-run — W1.7 pattern, resumed-to-completion, no redeploy, no thrash). **Tasks 6+7+12:** 66-row enumeration table (`wave-w2-3/teaching-sweep-ghelp-arghint3.md`); teaching refs in README + g-help/g-kickoff/g-onboard updated to surviving `/g-forge <token>` form, rhetorical/historical refs untouched; g-help gained the unknown/retired-token branch (Step 0); the 1 flagged live hit fixed (claude-plugin-architect.md:61 example now states ANY `commands/<name>.md` file = the violation); argument-hint 0 in g-help/g-train/g-roundtable. **Tasks 10+11:** argument-hint 0 in g-docs/g-refactor/g-audit + g-wiki/g-optimize/g-adr. **Attests (g-forge-dev ×3):** #19 commit-detect **58/58** + check-commit **22/22** (all 9 HEREDOC fixtures pass — heredoc fix formally attested); #20 observe **30/30** + agent-lifecycle **29/29**; #24 drift **12/12** + installed rule byte-identical (blob `417f2ad` reconfirmed). Wave 4 (serialized on g-help): 1/1 DONE first attempt — Announce/Rules/Step conformance in g-help/g-status/g-doctor/g-brief, semantics unchanged. Task-9 recount now fully discharged: all 17 violations covered, zero gaps. Integrity clean at BOTH wave closes (W3 delta = exactly 13 scoped files; W4 delta = exactly 3). Zero commits by design — all rides Pass 4's gated commit. **NEXT: see FIRST (Pass 4, fresh session).**
LATEST (2026-07-22e): **W2 Pass 2 (Wave 2) COMPLETE — THE 38 SHIMS ARE GONE; ADR-007's core deletion is executed.** /g-resume → /g-execute 2: 5/5 slots DONE, integrity clean (delta = exactly Wave-2 scope over Wave-1's known 13 dirty files). **Task 5:** 38 command shims deleted (plain rm, unstaged), `commands/` = `g-forge.md` only; report-only ref enumeration: 14 hits classified, 1 live teaching-doc hit routed to Wave 3, rest historical/infrastructural. **Task 9:** authoritative conformance recount vs amended rule — 17 violation instances across 12 skills (9 argument-hint / 3 Announce / 3 Rules / 2 Steps / 0 Skill() calls), 13 prior "violations" dissolve under the context: carve-out, **every violation covered by planned Wave 3–4 tasks, zero NOT-covered flags, zero already-conformant targets**. **Task 19-edit:** heredoc fix landed per the Wave-1 characterization — new `_commit_detect_strip_heredocs` guarded well-formed/interpreter/opener-line, strips bodies pre-suffix-walk, NEVER for bash/sh/eval heredocs, unterminated ⇒ untouched; 9 HEREDOC fixtures; sanity runs **commit-detect 58/58** (header updated to runner-observed) + **check-commit 22/22** (header already accurate, no edit); formal attest = Wave 3. **Task 24-edit:** amended architecture rule eager-installed, installed copy byte-identical to source (blob `417f2ad`); drift-suite attest = Wave 3. **Task 17-attest (g-forge-dev):** review-severity 9/9, header MATCH. Dispatch note: task-19 agent stopped mid-run once and was **resumed-to-completion** (W1.7 pattern — clarified the concurrent shim deletions were out of its scope; no redeploy, no thrash). Zero commits by design — all rides Pass 4's gated commit. **NEXT: see FIRST (Pass 3, fresh session).**
LATEST (2026-07-22d): **ADR-007 VERIFIED (all claims hold) → W2 PLANNED+APPROVED → Wave 1 SHIPPED (Pass 1 of 4).** /g-resume → ADR-007 fresh-window verify: 7/7 claims hold (38 shims present, drift persists, all three amend-first surfaces still shim-mandating, no build step, versioned cache confirmed); verify notes fed the plan — ROADMAP W2 bullet was stale (rewritten, task 22), arch-rule tasks merged (one file one task), rule-install rides ADR-008 eager cadence (task 24), post-release migration check got an owner (task 8). /g-plan W2: 24 tasks / 6 waves / 17 slots; 2 Clarifies developer-resolved (teaching-docs-only sweep; g-help catch IN scope); budget 81 vs ~35 → **4-pass split approved** ("ok"); forecast 85% High (breadth-driven, calibration caveat on doc). Wave 1 executed: 5/5 implementers DONE first attempt (g-skill-validate inverted to shim-absence check; g-skill-design SKILL.md-only authoring; arch rule = single-router model + context: carve-out + #18 hook-class note + three-class agent taxonomy; enforcer verdict PASS|FAIL→PASS|HOLD contradiction fixed; observe.sh journals SessionStart source w/ 6 pins) + 5 HQ-inline (umbrella: roundtable row added everywhere + **fallback prose list lines 48-85 WAS the real third surface — trimmed to bare tokens** [the "already bare" pre-read saw only frontmatter]; ROADMAP W2 bullet rewritten; release-checklist ADR-007 line added; **task-18 heredoc characterization written** — misfiring branch = newline-suffix walk commit-detect.sh:261-281, no heredoc awareness, quoting doesn't protect [split precedes quote context]; fix shape + interpreter-guard [never strip bash/sh/eval heredocs — body executes] + fail-before/regression test plan in `g-docs/agent-output/wave-w2-1/heredoc-characterization.md`; **task-21 PostToolUse-skip fixture 6/6** — hooks proven correct given payload, gap is platform skip, decision ACCEPT no-fix, tests/README.md section + probe-hygiene rule). Wave-close integrity CLEAN (delta = exactly 12 scoped files). Zero commits by design. **NEXT: see FIRST (Pass 2, fresh session).**
LATEST (2026-07-22c): **W1.5g Pass 3 (Waves 6–7) COMPLETE — Spike S1 done, W1.5g CLOSED, W1 CLOSED.** /g-resume → /g-execute W1.5g 6. Wave 6 (g-forge-dev, HQ-supervised, sequential, snapshot-first): **S1(a)** local-marketplace `add` under same name REPLACES the registry entry (no side-by-side: `g-forge → GitHub` became `g-forge → Directory`); `install` onto existing same-scope install = NO-OP, zero cache mutation (forced refresh = uninstall-then-install); marketplace remove auto-removes its cache dir; GitHub 2.2.1 install survived byte-intact (hash-verified). **S1(b)** router cache Glob with two version dirs (fake `9.9.9` beside `2.2.1`) resolves **mtime-ordered, NOT semver-ordered** — silently non-deterministic wrt version; edge state (one dir in practice) but any future dev-install mechanism must never allow two version dirs to coexist; semver-dir layout specific (content-hash marketplaces immune). Restore HQ-verified: cache `2.2.1`-only, registry → GitHub (metadata-only `lastUpdated` diff expected). Wave 7: decision-input report `g-docs/forecasts/w1-5g-spike-s1-skills-agents-devinstall.md` — explicitly NOT an install/decision; cache-pinned skills/agents distribution stands; any future flip = own gated slice. Ledger row `27ii+28ii` added (W1.5g close + W1 complete). Pass 3 docs-only per plan (/g-review ran after Pass 2); this pass's doc commit carries the ledger row + report + prior session's uncommitted retro/forecast edits. **NEXT: see FIRST (ADR-007 verify, fresh window → /g-plan W2).**
LATEST (2026-07-22b): **W1.7 Pass 3 (Waves 8–12) COMPLETE — W1.7 SHIPPED; THE GATE RAN ITS FIRST REAL COMMITS AND HELD EVERY PATH.** /g-resume → anomaly check (stray empty fixture-probe commit `20ddfbe` "filler"/`t@t` from Pass-1 clone debugging, pre-gate-install, escaped its throwaway clone — dropped via `reset --soft`, tree-identical, reflog-recoverable) → /g-execute W1.7 8. **Wave 8 (Task 14):** attestation GREEN 405/405 15 suites + fixtures 37/0 + 35/35 (header-vs-runner MATCH) → code-lead MERGE READY 0c/0M/1m + doc gate DOCS READY 0b/1w (both minors fixed inline pre-stamp: fixture header 36→37, CHANGELOG 35-real-commits precision) → dual ADR-004 sentinels stamped → **`1acd512` (13 files) passed BOTH live gates, both sentinels auto-consumed on normal argv — zero manual clears.** **Wave 9 (Task 15):** ledger row authored in a linked worktree, DOCS READY 0/0, doc sentinel stamped per-worktree-keyed → **`a966d88` committed FROM the worktree, ADR-005 live** (primary-side key matched + consumed); bonus: pathspec-less argv deny live-proved the unclassifiable⇒CODE fail-toward-deny polarity (doc-only commits signal class via explicit pathspec argv); ff-merge, worktree removed. **Wave 10 (Task 16):** unstamped script-indirection commit (argv-invisible — PreToolUse passed the bash call) **DENIED by native hook rc=1 HEAD-unmoved**; heredoc-body #21-residual re-confirmed live (first probe's heredoc containing a commit line was itself denied — W2 look stands); pass-half = the Wave-12 close commit via stamped script indirection. **Wave 11 (Task 17):** satisfied by 1acd512+a966d88 evidence. **Wave 12:** ledger close row `W1.7ii` (Tasks 19+26+27+28) — **findings #8/#9/#10/#25 CLOSED, #21 argv surfaces closed/heredoc residual → W2**; #22 attribution confirmed live in journal (typed agent events, RESULT parsing, no `unknown`). NEW residual: PostToolUse skipped on rc≠0 tool calls (commits inside failing chains unjournaled; probe-hygiene rule: never chain proof-steps into commit commands) → W2/W3. Release-gate decision recorded: **v2.3.0 at FULL milestone close.** Records: `g-docs/agent-output/{wave-w17-8,review}/`, plan Progress all-complete. **NEXT: see FIRST (Spike S1, fresh session).**
LATEST (2026-07-22): **W1.7 Pass 2 (Waves 3–7) COMPLETE — THE COMMIT GATE IS LIVE.** /g-resume → /g-execute W1.7 3, all HQ inline + one g-forge-dev attestation. Wave 3: rollback snapshot REFRESHED to `claude-install-snapshot-2026-07-22\` (63 files byte-verified: 48 .claude + 14 git-hooks + CLAUDE.md; stale -21 snapshot was pre-W1.5g-Pass-2, kept undeleted). Wave 4: check-commit.sh installed over the stale v2.2.1 copy + **4 drifted non-gating hooks caught up** (agent-lifecycle/pre-compact/session-start/workflow-checkpoint were source-newer — W1.6's F1/F10/node-tier fixes had never received their ADR-008 eager slice-close install) → 11/11 installed files hash-match source. Wave 5: native pre-commit installed into .git/hooks/ (clobber-guard trivially clean, executable, marker, byte-identical) **+ lib/ alongside — install-set gap caught live**: first escape-hatch probe hit lib-missing fail-closed (`internal error — could not load hooks/lib/worktree-resolve.sh`, still deny rc=1, fail-closed proven as bonus); hook resolves libs `dirname $0`-relative so .git/hooks/lib/{worktree-resolve,classify-changeset,sentinel-read,commit-detect}.sh now installed, live dry-run denies via REAL sentinel path. Wave 6: escape hatches PROVEN in scaffolded clone (needs .claude/integration-tier=full — bare clone confirmed inert-in-non-G-Forge, hook-correct): (a) unstamped deny rc=1 → (b) --no-verify bypass rc=0 → (c) hook-delete pass rc=0 → (d) restore re-deny rc=1; live hook diff-intact after. Wave 7: g-forge-dev attestation **GREEN 405/405 across 15 suites, header-vs-runner MATCH everywhere**, zero live-gate interference (record: `g-docs/agent-output/wave-w17-7/`). Also live-proven incidentally: the new PreToolUse gate denied HQ's own inline `git commit` probe argv this session (correct deny — no sentinel; sandbox probes now must ride script files). Zero commits by design — everything rides Task 14. **NEXT: see FIRST (Pass 3, fresh session).**
LATEST (2026-07-21h): **W1.7 PLANNED + APPROVED (3-pass split) and Pass 1 (Waves 1–2, clone-proof) COMPLETE — hard gate OPEN for the live gating install.** /g-resume → /g-plan W1.7: task-decomposer 28 tasks (2 Clarifies HQ-resolved: script-indirection clone-proven before live per ADR-008 §2; --exclude-dir narrowing is precision-only) → wave-planner 12 waves / 5 agent slots → budget 79 vs ~32 → developer approved 3-pass split at wave boundaries ("go split"): Pass 1 = W1–2 clone-proof, Pass 2 = W3–7 live install, Pass 3 = W8–12 live commits + ledger. Forecast 80% High (calibration caveat on doc; top scenario = gate self-modification lockout, mitigations structural). Pass 1 executed: **Wave 1** 3 agents parallel, all DONE (2 needed completion-resumes after mid-run stops — new pattern: resumed-to-completion, not redeployed; no thrash) — (a) NEW `g-dev/fixtures/clone-first-gate-verify.sh` (37 assertions through REAL git commits in throwaway clones: stamped pass, deny ×3, dual-sentinel ×3, first-commit ×2, #21 probes ×2, script-indirection, CANARY discrimination proof, parent-shell counting, invocation-form-insensitive); (b) mechanical minors 20–24 all landed (5 suite count-headers pinned runner-observed; session-start:33-34 comment; --exclude-dir narrowed; workflow-checkpoint garbage-tier fallback fixed + 2 pins 64→66; CLAUDE.md carve-out claim) — 8 suites green; (c) Task 18 multi-fire characterization: **no-bug, PLATFORM-EXPECTED** (one SessionStart per source startup/resume/compact; registration single-source, a57fcf9 intact; in-hook multi-write ruled out; residual polish = journal the `source` field → W2/W3); Task 25 wall-clock → backlog (false-green risk class, designed slice). Wave-close integrity clean (delta = exactly scoped files). **Wave 2: g-forge-dev attestation GREEN — clone-first 37/0 + pre-commit-gate 35/35, first attempt, no session-limit kill.** Zero commits by design — all Pass-1 edits ride Task 14's first gated commit (Pass 3). Records: plan + forecast on disk, agent output `g-docs/agent-output/wave-w17-{1,2}/`. **NEXT: see FIRST (Pass 2, fresh session).**
LATEST (2026-07-21g): **W1.6 SHIPPED — Pass 3 ran the full HOLD→fix→MERGE READY cycle and the review earned its keep twice over.** /g-resume → /g-execute W1.6 7: attestation r1 ALL GREEN 392/392/15 suites → /g-review (canonical working-tree SKILL per ADR-008; routine drift check: 5 files, all explained — gating pair deferred + 3 source-newer post-W1.5g-install) → code-lead **r1 HOLD 0c/3M/5m** (~34 min; no Task tool, axes code-lead-owned): M1 F10 half-applied (reader $GF_CLAUDE_DIR, writers bare — blind counter would become UNRESETTABLE false-red in linked worktrees), M2 pins-that-cannot-fail (bare `approved` matches `not yet approved`; `^sedtier` matches both truncation forms), M3 headers diverge from runner (agent-lifecycle claimed 31 with a sum-to-31 breakdown, ran 21). HQ probe-confirmed all three (zero phantoms) — then found a **CRITICAL r1 missed: test-agent-lifecycle was false-green** ([ -d ] guard on linked-worktree .git — a FILE — so §4 wt-tagging NEVER ran while the skip branch credited PASS+=4 phantom passes; second dead branch: glob-in-assignment killed 6 pairing assertions; 21 reported = 17 real + 4 phantom, zero worktree coverage). Fixes: -e guard + skip-credits-zero; ls-resolve; discriminating patterns (`approved (commit gate open)`; doubled-backslash + absence-of-`truncated`); F10 conversion COMPLETED in-slice over r1's route-to-W1.7 (regression this slice introduced; 1-of-7 sites was worse than either endpoint; W1.5a precedent) — all 7 counter paths $GF_CLAUDE_DIR across workflow-checkpoint/session-start/pre-compact + 3 NEW linked-worktree reset assertions. **Every fix canary-proven red-under-corruption** (hook corrupted → 4 approved pins red 60/4; untruncated value → sed pin red; writers reverted → reset pins red 5/3-not-0, compact-preserve stays green). First real wt run then found 1 divergence → adjudicated HOOK-CORRECT/fixture-side (hook writes show-toplevel native spelling C:/...; fixture compared MSYS mktemp spelling + malformed grep-without-file-arg — fixed via rev-parse-derived expected key). Attestation **r2 GREEN 403/403/15 suites, header-vs-runner MATCH everywhere** (r1 methodology defect fixed: r1 compared runner against itself; r2 compares header vs runner independently — that's how M3 stayed invisible) → code-lead **r2 MERGE READY 0c/0M/0m** (all axes PASS, scope call agreed, adversarial no-remaining-phantom sweep). Tier-3 per no-manual-QA doctrine (attestation IS the DoD evidence). /g-doc-review **DOCS READY 0b/6w** — 4 fixed inline pre-commit (stale "pending Wave 7" annotations ×2; pre-compact stale activation-only comment — uncatalogued sibling of the acknowledged class; g-forge-dev "6 suites"→glob-authoritative), 2 routed W1.7. Riders landed: W1.6i ledger row (F1+F10+Critical+Write-grant), CLAUDE.md table 15 suites/403, CHANGELOG W1.6 bullet. review-holds 4. NEW → W1.7: everything on the W1.6i route-forward list (see FIRST). **NEXT: see FIRST (/g-plan W1.7).**
LATEST (2026-07-21f): **W1.6 Pass 2 (Waves 4–6) COMPLETE — 9/9 agent slots DONE first attempt across 3 waves, integrity clean at every wave close; 1 FINDING probe-adjudicated hook-correct.** /g-resume → /g-execute W1.6 4 (telemetry stable, no cap): Wave 4 6 slots parallel — tasks 9–12 (commit-detect 43→49, classify 42→47 authored; invariant scan now covers hooks/pre-commit), task 15 REAL FIX (agent-lifecycle node tier: JSON null agent_type stringified to "null" → now falls back "unknown"; forced-node-path pin, sanity-run green), task 16 (worktree retired-token scan → single --exclude-dir=lib, count-neutral 42), task 21 (dead scaffold gone, run 26/26), task 22 (g-doctor frontmatter Check-16 scope), tasks 14+23 (g-review Step 6 bullet split + Step 1 drift-check wording narrowed to hooks+libs) → Wave 5 tasks 17+18 (json_escape hostile chars → line 1, Test 5 assertion real; sed-tier escaped-quote truncation pinned, all 3 parsers stubbed) → Wave 6 task 19 sweep AUDIT-ONLY (all 10 pre-existing suites already conformant absolute-once-at-top, zero edits; per-suite conformance table in wave-w16-6 report) + task 20 (tests/README.md convention section). **Task 19's real-run FINDING adjudicated per Pass-1 discipline:** observe Test 5 (W1.6-17) red — HQ probed the hook directly: raw journal line VALID JSON, adversarial content fully preserved, backslash round-trip faithful (the momentary "halving" read was HQ's own probe-quoting artifact, verified against python3 repr + node), python3 tier live on this machine → **HOOK-CORRECT, fixture-side bug**: the shared test helper `agent_lifecycle_journal`'s detail read-back sed was quote-blind (`[^"]*` truncates at the first escaped quote — the exact class task 18 pins for the hook's own sed tier, here in the TEST harness) → HQ-fixed inline per §C inline rule (escape-aware capture `\(\\.\|[^"\\]\)*`) + header count 25→24 (the Wave-5 arithmetic flag confirmed: 23+1 new test = 24, agent claimed 25) → re-run **24/24 green**. Zero commits by design (Pass-2 work rides Pass 3). NEW → nothing carried out of this pass beyond the standing Pass-3 commit-time items. **NEXT: see FIRST (Pass 3 = /g-execute W1.6 7, fresh session).**
LATEST (2026-07-21e): **W1.6 Pass 1 (Waves 1–3) COMPLETE — 4 never-tested hooks now have attested suites; TWO REAL HOOK BUGS found and fixed in-slice.** /g-resume → /g-execute W1.6 1: Wave 1 4/4 test-writer WRITTEN first attempt (integrity clean) → Wave 2 task 13 stamp advisory-delta pin → Wave 3 attestation loop: **r1 3/4 red (12 fails)** → adjudication (developer-ruled): **F1 REAL** — session-start.sh never read the integration-tier VALUE (only existence; light tier banner fired where docs say silent) → fixed in-slice, light now exits silently BEFORE banner, state maintenance (counter reset) deliberately kept on all tiers; **NEW F10 REAL (HQ-probe-found)** — workflow-checkpoint.sh PROMPT_COUNT_FILE was bare-relative `.claude/…` → depth counter blind + stderr noise from linked worktrees → now `$GF_CLAUDE_DIR`-resolved (1-line); F4 (straight-to-🔴 at 46 = CORRECT, impl red=45) / F5 (balanced DOES suppress auto-triggers; 📋 nudges are manual-invocation reminders, assertion narrowed) / F6 (worktree Review line correct) all **hook-correct, probe-refuted, fixture-side** → fix wave (1 claude-plugin-implementer + 3 test-writers, file-disjoint) → **r2 158/160** → last 2 = fixture bugs HQ-fixed inline per §C inline rule (§9 stamps were 3-line heredocs but gf_parse_stamp is single-line by contract → fell into the documented advisory-presence fallback — the hook's per-worktree keying VERIFIED correct; §2.3 fixture trees never actually clean — untracked scaffolding, fixed with self-ignoring .gitignore) → **r3 GREEN 160/160** (27+22+64+21 new + guard 26/26 throughout). Also: **g-forge-dev granted Write tool (developer directive)** after its big heredoc report write was blocked at the permission layer (HQ verified NOT #21 — installed regex needs literal `git commit`; scoped in agent body to g-docs/agent-output/ reports only; .claude/agents/g-forge-dev.md is tracked, rides the commit). Suite-authored claims vs attested counts diverged sharply (38/26/54/29 claimed vs 15→27/22/54→64/21 real) — standing rule held: only the runner's table is authoritative. Session-start suite counter bug (printed-FAIL-not-counted, subshell class) found+fixed. Carry to Pass 3: add F1+F10 ledger rows + the g-forge-dev Write note to M-audit-2026-07.md; new-suite counts into CLAUDE.md test table at commit time. Zero commits (Pass-1 work rides Pass-3 /g-review + commit per plan). **NEXT: see FIRST (Pass 2 = /g-execute W1.6 4, fresh session).**
LATEST (2026-07-21d): **W1.6 PLANNED + APPROVED in the Pass-2 session (post-commit context headroom used deliberately — developer call).** Pushed `cb7ac7b..137e56a` (origin current, 0 ahead); stash@{0} dropped (list empty). /g-plan W1.6: task-decomposer 23 tasks (8 new-coverage + 15 carried minors; 1 Clarify resolved — agent-lifecycle gets its own suite, observe fixtures stay, task-15 pin in test-observe) → wave-planner 7 waves / 16 slots, collision-free (9+10+11+12 and 14+23 and 17+18 consolidated; sweep task 19 strictly last among suite edits) → budget 81 est. vs ~36 → approved 3-pass split at wave boundaries (W1.5g precedent) → forecast 75% Elevated written with the calibration caveat + change-scoped framing. Plan + forecast on disk; Progress all-pending. NOTE for W1.6 Wave-1 prompts: absolute-path-once-at-top + canonical repo-root invocation pins are MANDATORY task-prompt content (scenario 1). **NEXT: see FIRST (fresh session → /g-resume → /g-execute W1.6 1).**
LATEST (2026-07-21c): **W1.5g Pass 2 (Waves 4–5) COMPLETE — FIRST LIVE SELF-HOST INSTALL SUCCEEDED; slice reviewed MERGE READY + committed.** /g-resume → /g-execute 4: Wave 4 (g-forge-dev, HQ-supervised) installed the non-gating set from the working tree per ADR-008 — 21 files (6 hooks + 4 libs + 10 g-rules flat-renamed + claude-plugin-architect) byte-identical to source, HQ-verified independently; gating pair untouched (check-commit.sh sha unchanged `43fb0921…`, no native pre-commit created — W1.7 owns both). Wave 5: live extended-Check-16 verify PASS (20 hash-match + 1 expected template-instantiated advisory, 0 Fail) — the Tier-3 evidence per the plan's DoD. /g-review on Pass 1+2: attested 245/245 across 11 suites (g-forge-dev runner, canonical invocation — no session-limit kill this time), code-lead **MERGE READY 0c/0M/1m** (minor: dead scaffold block in the invariant suite lines ~30–38 + unused var → W1.6; template-advisory design note confirmed coherent; drift line correctly visibility-only: the 2 "drifted" files ARE the deferred gating pair). /g-doc-review **DOCS READY 0b/3w** — w1 (missing CHANGELOG entry) fixed inline pre-commit (ADR-008 self-host entry added under Unreleased); w2+w3 (g-doctor frontmatter under-scopes Check 16; g-review Step 1 wording implies broader Check-16 scope than hooks+libs) → W1.6 advisory. Both sentinels stamped identical (ADR-004). The install went live mid-session: this session's own hooks now run the W1.5f-fixed code (#22 agent attribution goes live for the NEXT session's journal). NEW → W1.6: code-lead minor + 2 doc-clarity warnings above. ⚠ Sentinels NOT auto-consumed at `137e56a` (manually cleared): the commit ran via a script file to dodge the old gate's #21 false-positive, which also hid it from post-commit-cleanup's argv detection — script-indirection bypasses BOTH argv-based sites (gate + cleanup); exactly the class the W1.7 native pre-commit closes. Fold into W1.7's live-verify checklist. **NEXT: see FIRST (W1.6 → W1.7; Pass 3 deferrable).**
LATEST (2026-07-21b): **W1.5g Pass 1 (Waves 1–3) COMPLETE — self-host mechanism + drift detection coded, suites attested 245/245.** /g-resume → /g-execute Wave 1: 6/6 slots DONE first attempt, integrity clean (no doc-writer children fired, no overreach — tracked delta exactly the six scoped files). Shipped: Check 16 extended in place (10-file g-rules flat-rename mapping missing=Fail + three-class installed-agents rule: profile-copied hash-compare / template-instantiated advisory-only / project-local `*-dev.md` excluded); ADR-008 detection convention in /g-update (Step 0/1) + /g-init (new Step 1a) with verbatim-identical condition (root `.claude-plugin/plugin.json` + name match → source root = working tree; cache Glob only in explicit fallback branches); /g-review Step 1 routine drift check (visible-only, 4 never-gates statements); drift suite 3→12; NEW test-class-split-invariant.sh 26 tests; durable snapshot at `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-21\` (34 .claude + 14 git-hooks files byte-verified, supersedes the 2026-07-19 temp snapshot — not deleted). **Wave 2 attestation gate earned its keep:** first real run of the invariant suite failed 24/26 under canonical relative invocation (lazy `dirname $0` hook-path resolution after fixture cd — absolute invocation masked it, which is why the two parallel runners CONTRADICTED: 2/26 vs 26/26; HQ probe-adjudicated both forms empirically); attempt-2 fix (absolute resolution once at script top, six lazy re-derivations replaced) → re-attested 245/245 across 11 suites, 0 failures, canonical invocation, escalation-log row added. CLAUDE.md counts synced to attestation (210→245). Zero commits by design — Pass 1 work rides to the commit after Pass 2's /g-review. NEW minor → W1.6: the invariant-suite bug class (test-suite path resolution must be absolute-once-at-top, invocation-form-insensitive) is worth a structural grep across the other 10 suites. **NEXT: see FIRST (Pass 2 = Wave 4–5 live install, fresh session).**
LATEST (2026-07-20/21): **ADR-008 verified → docs committed → W1.5g planned + approved with split.** Fresh session ran /g-resume → clean-slate verified ADR-008 11/11 (all measured-drift claims reconfirmed live: 7/7 hooks, 0 libs, 0/10 rules, cache 2.2.1-only, architect agent drifted; Check 16 required + in no loop; detection condition viable; snapshot found but in volatile session temp). Plan-vs-findings review caught 1 real discrepancy (W1.5g bullet said "7 hooks" — ADR class is 6 non-gating; installing check-commit.sh eagerly would violate §2) + 2 gaps (profile agents missing from install set; agent surface is three-class, not one hash-compare) — all fixed in the bullet pre-commit. /g-doc-review DOCS READY (0b/2w, warning 1 fixed inline) → committed `7479882` (doc sentinel auto-consumed, 10 ahead of origin unpushed). /g-plan W1.5g: 17 tasks → 7 waves/13 slots (wave-planner r1 had Spike probes parallel in Wave 1 sharing live plugin-cache state — validation caught it, revised to combined post-install Wave 6 with mandatory cache restore). Budget formula 69 vs 38 → exceeded; /g-forecast 90% High (structural drivers: budget + first-ever live runtime self-mutation; calibration caveat on the doc). **Developer approved 2026-07-21: "split as suggested"** — planned pass boundaries at Wave 3→4 and 5→6 instead of /g-roadmap re-scoping. **NEXT: see FIRST.**
LATEST (2026-07-19): **ADR-007 decided + W1.5e shipped in one pass.** (1) Naming/version strategy **downgraded from ADR to a brief tech-decisions row** (developer: product strategy, not architecture) — flags struck in M44.md Rollback + this handoff, row added to project_brief.md. (2) **Finding #19 resolved end-to-end:** G-Cash live-install verification confirmed two files = two visible entries → `/g-adr` resumed → deliberation surfaced a pointer-stub middle option → developer rejected ALL keep-and-sync layers ("one command per thing, one only") → **ADR-007 Accepted** (retire shims, SKILL.md sole source, umbrella bare tokens; migration free via versioned cache replacement — shims never touch consumer repos, no /g-update work, no pastable cleanup; implementation = W2 with the sequencing constraint: amend g-skill-validate/g-skill-design/architecture rule BEFORE deletion; verify ADR-007 fresh-window before W2). (3) **W1.5e shipped** (see the ✅ slice line): plan 5 tasks/2 waves, forecast 55% Elevated (calibration caveat on the doc), all first-attempt, integrity clean (overreach scenario did NOT fire — the no-child-dispatch hard line held), attested 187/187 + 35/35 + 4/4, MERGE READY 0c/0M/1m, DOCS READY 0b/1w (w fixed pre-commit: M-audit Version field → v2.3.0). (4) **CHANGELOG archaeology:** the finding-#20 bullet header had been silently destroyed in the W1.5d doc-writer overreach — the recorded "single line, reverted, intact" was WRONG, damage was worse; header restored from `22ea4f3`. Feed to /g-patterns: overreach damage audits must diff the whole file, not the noticed line. (5) g-forge-dev attestation dispatch killed by session limit again (2nd time) — HQ ran directly per W1.5a precedent. (6) Live #21-class false-positive: installed PreToolUse gate denied code-lead's report *write* (heredoc quoting commit phrases) — W2 look queued. (7) NOTE: installed hooks livelier than the standing Windows fail-open note — both sentinels AUTO-CONSUMED at `4e1c49a` (first in 3+ sessions); verify precisely at W1.7. Session retro: `g-docs/retros/2026-07-19-adr007-w15e.md` (w1-5e forecast Outcome table reconciled — scenario 1 clean, 2+3 partial-absorbed; retro + outcome edits UNCOMMITTED at handoff, ride the next doc commit). **NEXT: see FIRST (W1.5f).** Three linked calls made at the approval gate: (1) **M-audit ships v2.3.0, not v2.2.2** — W1 is new capability (native pre-commit site, 4 libs, 12-file install set, 187 tests), not a patch; the release pass at close now also ships the first README **status strip** (version badge + CHANGELOG link + ROADMAP link, high on the page) + the `[2.3.0]` CHANGELOG cut — **standing README/CHANGELOG currency convention starts here**. (2) **M41 split:** release machinery (/g-changelog + /g-release + doctor version-consistency + currency enforcement) stays at v2.4.0 right after M-audit; the **G-Proof rebrand + full persuasion-ordered README restyle moved to NEW M44 — the capstone, sequenced dead last, shipping as G-Proof 1.0** (versioning restarts under the new name; rationale: consumers keep a stable name through the heavy middle, no mid-arc 3.x renumbering, "proof" claimed only when M38/M39 self-governance can back it). (3) **Downstream renumbered back into 2.x** (M42 v2.5.0 … M43 v2.13.0; Version Plan block rewritten). M41.md rewritten + new M44.md carries the rename waves. ADR flag RESOLVED 2026-07-19: naming/version strategy downgraded to a brief tech-decisions row (product strategy, not architecture — developer call; row added to project_brief.md, M44.md Rollback note updated; no /g-adr). Session retro: `g-docs/retros/2026-07-18-w15cd-capstone-restructure.md` (w1-5c/w1-5d forecast Outcome tables reconciled — W1.5d scenarios 2 happened + 3 partial, both caught; retro + outcome edits UNCOMMITTED at handoff, ride the next doc commit). **NEXT: unchanged — see FIRST (W1.5e).**
...
Active context:   · main @ v2.2.1 + quick-wins merge (`4158ffa`); **v2.3.0 ships when M-audit W1 closes** (upgraded from v2.2.2 — developer, 2026-07-18; release pass includes the first README status strip + CHANGELOG cut, starting the standing currency convention). Arc sequence (restructured 2026-07-18): **M-audit-2026-07 (v2.3.0) → M41 (Release Machinery + README Currency, v2.4.0 — REBRAND SPLIT OUT) → M42 (v2.5.0) → M29 (v2.6.0) → M35 (v2.7.0) → M37 (v2.8.0, salience propagation) → M33 B–D (v2.9.0) → M34 → M30–M32; then self-improvement track: M38 (v2.10.0) → M39 (v2.11.0); M40 v2.12.0 + M43 v2.13.0 tail/pull-forward; then **M44 — G-Proof 1.0, the rebrand capstone, THE last release of the arc** (versioning restarts under the new name; no mid-arc 3.0.0). M36 (salience approach/ADR) slots early, parallel-friendly, gates M37.** M-audit 🔄 (W0 done, W1 next — items 8–10 need spike/ADR before code); M29 + M34 ⬜ specs in g-docs/milestones/; M35 spec written this pass. Cleanup queue unchanged: Confluence page 109314050 + Drive doc + stale `g-table` Gmail labels — delete or keep as fixtures. Windows note: hook matchers + installed-copy drift were fail-open here — see memory `windows-hook-gotchas`. **New finding #19 (this pass):** every skill's description is hand-authored in up to 3 places (`commands/<name>.md`, `skills/<name>/SKILL.md`, `commands/g-forge.md`'s subcommand line) and has drifted repo-wide (confirmed: g-adr, g-retro, g-review, g-audit, g-plan, g-execute) — a real behavioral disagreement on g-adr, not just wording. Proposed fix (retire standalone command shims, SKILL.md sole source) escalated to `/g-adr`; the pause was resolved **2026-07-19 by the `hllrm/G-Cash` live-install verification: two files = two visible entries CONFIRMED** (same `g-forge:<name>` display name twice, different description text, each tracing verbatim to `commands/<name>.md` vs `skills/<name>/SKILL.md`; duplicate-registration and alias-pair readings refuted; full evidence on the #19 row in `g-docs/milestones/M-audit-2026-07.md`). **DECIDED 2026-07-19 → ADR-007 (Accepted, two-way door): one command per skill — retire ALL standalone shims; SKILL.md sole authored source; `g-forge.md` umbrella trimmed to bare tokens; migration is free via versioned cache replacement (shims never touch consumer repos — no /g-update involvement, no pastable cleanup).** Keep-and-sync alternatives (synced/generated/pointer stubs) explicitly rejected — developer criterion: one command per thing, no added layers. Bare `/g-<name>` aliases go away (accepted cost; docs sweep rides the change). Sequencing constraint: amend `/g-skill-validate` Steps 3–4 + `/g-skill-design` + the architecture rule's `commands/` definition BEFORE deleting shims. Implementation = M-audit W2 (finding #19's wave); verify ADR-007 against the repo before that wave executes. Re-enter with /g-resume.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**Status:** ✅ Complete (W0–W3 + stdin-guard release rider; v2.3.0 released 2026-07-23, `e28e501`)
...
**Version:** v2.7.0 (minor — the register is a new capability; renumbered 2026-07-23 — M45 inserted ahead)
...
**Version:** v2.10.0 (minor — Phase A shipped un-versioned; ships when Phase B lands. Follows M37 per the version plan; shared digests build on the memory substrate. Renumbered 2026-07-23 — M45 inserted ahead.)
...
**Version:** v2.8.0 (minor — new capability; renumbered 2026-07-23 — M45 inserted ahead)
...
**Status:** ⬜ Not started
**Version:** v2.9.0 (minor — the salience substrate + its cross-system wiring; renumbered 2026-07-23 — M45 inserted ahead)
...
**Status:** ⬜ Not started
**Version:** v2.11.0 (minor — new skill; renumbered 2026-07-23 — M45 inserted ahead)
...
**Status:** ⬜ Not started
**Version:** v2.12.0 (minor — new skill; Phase A ships it, B/C fold in as they land; renumbered 2026-07-23 — M45 inserted ahead)
...
**Status:** ⬜ Not started
**Version:** v2.13.0 (minor — new recognized folder class + classifier arm + doctor advisory + intake questions + optional ADR field; renumbered 2026-07-23 — M45 inserted ahead)
...
**Status:** ⬜ Not started
**Version:** v2.5.0 (minor — `/g-review` restructure + reviewer/agent contract changes; inserted 2026-07-23 from the 2026-07-22 field feedback — review cost hits every remaining slice, so it slots directly after M41)
...
**Status:** ⬜ Not started
**Version:** v2.6.0 (minor — new planning capability + gate; renumbered 2026-07-23: M45 review-pipeline rework inserted ahead at v2.5.0)
...
**Version:** v2.14.0 (minor — new skill + new operative variable; renumbered 2026-07-23 — M45 inserted ahead. **Parallel-friendly / pull-forward eligible** — independent of the multiplayer arc, touches only g-init/g-execute skill prose + a new skill; slots into any gap on developer request, like M36.)
...
v0.8.1 → v0.9.0 (M8) → v0.10.0 (M9) → v0.11.0 (M10) → v0.12.0 (M11)
       ...
       → **v2.3.0 (M-audit-2026-07 — Forge Integrity; upgraded from v2.2.2 — W1 is new capability, not fixes; ships the first README status strip + starts the CHANGELOG/README currency convention)** → **v2.4.0 (M41 — Release Machinery + README Currency: /g-release + /g-changelog + /g-doctor version-consistency; rebrand split out 2026-07-18)** → **v2.5.0 (M45 — Review Pipeline Rework: code-lead-in-HQ + partitioned reviewer waves; inserted 2026-07-23 — review cost hits every remaining slice)** → **v2.6.0 (M42 — Planning Cold-Start Integrity; sequenced early despite tail ID — G-Cash field report)** → v2.7.0 (M29 — multiplayer phase one) → v2.8.0 (M35 — Memory Forge) → **v2.9.0 (M37 — Salience Layer propagation; M36 approach/ADR is design-only, gates M37, slots early)** → **v2.10.0 (M33 B–D — the Roundtable)** → M34 (cross-session dependency tracking + pull/push orchestration, arc spine) → M30–M32 mechanics reconcile against M34 → **v2.11.0 (M38 — G-Report)** → **v2.12.0 (M39 — G-tweak)** → **v2.13.0 (M40 — Reference Convention)** → **v2.14.0 (M43 — Operator Controls: /g-settings + inspection cadence; parallel-friendly, pull-forward eligible)** → **G-Proof 1.0 (M44 — rebrand capstone; versioning restarts under the new name — THE last release of the arc)** · M26 (Provable Wave Dispatch) deferred to its own minor when its spike clears · M25 benchmark ships its number when run
```
