...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · AUDIT WAVES A+B SHIPPED (tasks 3–4 closed) · 2026-08-11
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **AUDIT WAVE A + WAVE B REMEDIATION MERGED** (`29e4a98` on main, pushed): 18 audit findings closed — Wave A doc/enum (g-specialize architect table: 4 wrong paths fixed **+ 4 rows the review found missing**, flask/pygame/xamarin/frontend-data-flow — table now bijective with `profiles/`; telemetry roster 19 both sites; README two-layer gate bypass; §I/§J heading letters; ADR-009/010 back-stamps; severity-contract rule rewritten FROM test-review-severity's pins incl. security-auditor's deliberate native Critical/High/Medium/Low; rust-architect name; M28 renumber; M1/M2 links) + Wave B code (check-commit bounded shim replaces blocking `cat`, KNOWN FAIL-OPEN documented in-file; post-commit-cleanup gains shim — old `INPUT=""` degrade left sentinels uncleared = gate stuck open — plus explicit `exit 0` per ADR-008; 5 shim bodies byte-identical honoring `${1:-5}`; g-plan reads session-keyed counter AND derives red threshold 45−offset floor 25, was stale literal 40; g-afk training-mode block with g-train's canonical string; g-doctor **Check 25** governing-integration-tier guard, worktree-aware — doctor is now **25 checks (16+9)**; STDIN_GUARD_WINDOW_MS=20000 on both abandoned-stdin cases; g-init gitignore globs the keyed counter, closing audit-5 F-6's counter leg; CHANGELOG entries for every shipped-surface fix — 10 Wave A + 8 Wave B conditions = the 18). Gate: **4 code-lead rounds + parallel cautious code-reviewer**, HOLDs r1–r3 all fixed (recurring self-inflicted class: count half-sweeps — cure that worked was deriving sentences from ground truth before writing), r4 MERGE READY; one reviewer Major refuted with hash evidence. Tasks 3–4 archived to `todo-done.md`.
Next up:          · **/g-plan M47** (first 2.5 milestone; M48's plan folds in todo task 5 test-teeth riders). · **CARRIED:** M38 delivery decision at its plan gate · announcement block order at release · g-wiki refresh (task 1, widened) · field-reports/ §I-table registration at M38 · intake rows (a)–(h) unchanged · task 6 M45 rider (reviewer record-write contract) · task 7 remainder (+2 new riders: g-specialize triple stack-list derive refactor · sleeper reaping).
Active context:   · `main` @ v2.4.0, clean after merge `29e4a98` + this close-out commit — both pushed, never carried. Suite = **564 / 18 suites / green attested 2026-08-11 twice** (g-forge-dev runner, totals summed from per-suite table both times) — regression signal only, not coverage (audit verdict stands). `/g-doctor` **25 checks (16+9)** as of this pass. **ADR-010 VERIFIED + back-stamped · ADR-011 CURRENT · ADR-012 CURRENT — build on all three, re-verify none**; checkpoint's ADR-verify nudge stays a spent-entry false positive (`workflow-checkpoint.sh:391`). **Version identity: 2.5.0 last G-Forge release; G-Proof ships 1.0, never 3.0.** ROADMAP authoritative; fork-bound content only in gitignored `g-proof-roadmap.md`. LOCAL, NON-COMMITTED STATE (all gitignored): `.claude/hooks/` + `.claude/rules/` re-synced with source THIS PASS (2026-08-11, incl. architecture-claude-plugin + I/J g-rules) · settings matcher `Bash|PowerShell` · voice profile `gian` · caveman plugin ACTIVE (full) · audit reports in `g-docs/agent-output/audit/` + 4 review records in `g-docs/agent-output/review/code-lead-2026-08-11*.md`. REPORT-VS-REPO CORRECTIONS (don't re-derive): 8 hook scripts + hooks.json + 6 libs, 19 agents, 38 skills, single router (ADR-007), 56 profile dirs, 25 doctor checks, SubagentStart/Stop registered. Backlog riders: fixture sleep-leak (serial suite ~10 min) · ADR-006 implementation · Check-24 bare-prose backlog. Format note: keep this line's leading label intact, use exactly the three §I labels, replace this block **wholesale** each pass — never append a second generation, and never repeat this line's own leading label text anywhere else on it (`workflow-checkpoint.sh:97` strips with a greedy BRE through the LAST occurrence; `pre-compact.sh:74` uses awk block capture — the two consumers disagree when violated, observed live 2026-07-26).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
