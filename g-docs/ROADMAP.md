...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | W1.2 shipped (commit gate + native pre-commit hook) · MERGE READY r2 · sandbox-proven @ v2.2.1 (v2.2.2 pending milestone close)
⚠ FIRST (fresh session): `/g-plan` **W1.3 — remaining hook worktree integrations** (`post-commit-cleanup.sh`, `observe.sh`, `pre-compact.sh`, `session-start.sh`, `workflow-checkpoint.sh`, `agent-lifecycle.sh` onto `hooks/lib/worktree-resolve.sh`; tasks 11–18). ⚠ flagged oversized — expect the context-budget gate to split it. W1.1+W1.2 foundations are done and reviewed — do not re-plan or re-implement them. W1.4 (install wiring + drift) and W1.5 (tests) are unblocked too if W1.3 splits badly.
...
- W1 (P0): ADR-004 (sentinel↔tree binding) + ADR-005 (worktree enforcement) implementation + finding #21 fold-in — 37 tasks / 8 waves, split into budget-scoped sub-parts (each sized to fit a session's `/g-plan` context-budget gate; sequenced 1→7, run `/g-plan` on each in order):
  ...
  - **W1.3 — Remaining hook worktree integrations:** `post-commit-cleanup.sh`, `observe.sh`, `pre-compact.sh`, `session-start.sh`, `workflow-checkpoint.sh`, `agent-lifecycle.sh` (tasks 11+12, 13+14, 15, 16, 17, 18). Depends on: W1.1. ⚠ oversized estimate — expect `/g-plan` to split further
...
**Version:** v2.5.0 (minor — the register is a new capability; shifted +1 by M42, +1 again by the M41 pull-forward)
...
**Version:** v2.8.0 (minor — Phase A shipped un-versioned; ships when Phase B lands. Follows M37 per the version plan; shared digests build on the memory substrate. This field previously said v2.5.0 while the version plan said v2.6.0 — realigned during the M42 resequence; +1 again by the M41 pull-forward.)
...
**Version:** v2.6.0 (minor — new capability; shifted +1 by M42, +1 by the M41 pull-forward)
...
**Status:** ⬜ Not started
**Version:** v2.7.0 (minor — the salience substrate + its cross-system wiring; shifted +1 by M42, +1 by the M41 pull-forward)
...
**Status:** ⬜ Not started
**Version:** v2.9.0 (minor — new skill; shifted +1 by M42, +1 by the M41 pull-forward)
...
**Status:** ⬜ Not started
**Version:** v2.10.0 (minor — new skill; Phase A ships it, B/C fold in as they land; shifted +1 by M42, +1 by the M41 pull-forward)
...
**Status:** ⬜ Not started
**Version:** v2.11.0 (minor — new recognized folder class + classifier arm + doctor advisory + intake questions + optional ADR field; shifted +1 by M42, +1 by the M41 pull-forward)
...
### M41 — /g-release + /g-changelog (gated release pipeline)
**Status:** ⬜ Not started
**Version:** v2.3.0 (minor — new commands + skills; `/g-doctor` gains a version-consistency check. **PULLED FORWARD 2026-07-15 (developer):** from tailed v2.11.0 to immediately after M-audit — releases happen constantly, the mid-milestone-bump slip already fired once, and the milestone accreted `/g-changelog` + the README currency pass; downstream minors shifted +1.)
...
**Scope (waved):**
- **Wave 0 — /g-changelog: compile, don't hand-write (added 2026-07-15, developer):**
  - New skill that **drafts** CHANGELOG `[Unreleased]` entries from the durable record — milestone-ledger rows, review verdicts (MERGE READY summaries), plan done-conditions — never from raw `git log` (distillation risk: ledger rows are already human-curated signal; commit messages are not). Runs at wave/milestone close or on demand; **draft + human nod** before anything is written (suggest-not-act). Keep-a-Changelog buckets inferred from the row type (fix/feat/docs), developer corrects.
  - Amends this milestone's former out-of-scope line: the CHANGELOG stays human-**curated** but stops being human-*remembered* — generation is drafted, curation is the nod. (Field motivation: every wave close to date relied on a session hand-writing entries; W1.2's entries were hand-written the same day this was scoped.)
- **Wave 0b — README currency pass at milestone close (added 2026-07-15, developer):**
  - `/g-review` Step 6 close-out swarm gains an **optional** README pass: counts verified mechanically (the Wave-1 `/g-doctor` consistency check below), narrative sections drafted by doc-writer **only when the milestone changed user-facing behavior**, applied on developer nod. Optional-by-default — internal-only milestones skip silently. (Retro evidence: m10-m14 pre-shipping sweep — stale README on 3 of 5 milestones; recommended exactly this close-time cross-check.)
...
**Sequencing note:** pulled forward to v2.3.0 (2026-07-15, developer approval) — sequenced immediately after M-audit-2026-07 closes, before M42. The original "tailed to avoid renumbering" trade lost once the milestone accreted `/g-changelog` + the README pass and v2.2.2 came into sight.
...
**Status:** ⬜ Not started
**Version:** v2.4.0 (minor — new planning capability + gate; shifted +1 by the M41 pull-forward)
...
**Sequencing note:** ID tailed at M42, but **sequenced early — after M41 (which the 2026-07-15 pull-forward slotted directly behind M-audit), before M29** — field-validated by the first real consumer's first smoke test, cheap (skill prose, no enforcement code), and it protects every project *birth*: the blind spot compounds worst at kickoff. Takes **v2.4.0** (was v2.3.0 until the M41 pull-forward); downstream minors shift +1 (see version plan).
...
**Version:** v2.12.0 (minor — new skill + new operative variable; tailed deliberately to avoid a third same-day renumber. **Parallel-friendly / pull-forward eligible** — independent of the multiplayer arc, touches only g-init/g-execute skill prose + a new skill; slots into any gap on developer request, like M36.)
...
v0.8.1 → v0.9.0 (M8) → v0.10.0 (M9) → v0.11.0 (M10) → v0.12.0 (M11)
       ...
       → v2.2.2 (M-audit-2026-07 — Forge Integrity) → **v2.3.0 (M41 — /g-release + /g-changelog + README pass; pulled forward 2026-07-15)** → **v2.4.0 (M42 — Planning Cold-Start Integrity; sequenced early despite tail ID — G-Cash field report)** → v2.5.0 (M29 — multiplayer phase one) → v2.6.0 (M35 — Memory Forge) → **v2.7.0 (M37 — Salience Layer propagation; M36 approach/ADR is design-only, gates M37, slots early)** → **v2.8.0 (M33 B–D — the Roundtable)** → M34 (cross-session dependency tracking + pull/push orchestration, arc spine) → M30–M32 mechanics reconcile against M34 → **v2.9.0 (M38 — G-Report)** → **v2.10.0 (M39 — G-tweak)** → **v2.11.0 (M40 — Reference Convention; Wave 1 pull-forward candidate to a v2.2.x patch)** → **v2.12.0 (M43 — Operator Controls: /g-settings + inspection cadence; parallel-friendly, pull-forward eligible)** · M26 (Provable Wave Dispatch) deferred to its own minor when its spike clears · M25 benchmark ships its number when run
```
