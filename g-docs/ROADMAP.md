...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | **W2 SHIPPED 2026-07-22 — commit `4b8c532`** (all 24 tasks; MERGE READY + DOCS READY; dual sentinels auto-consumed) @ v2.2.1 (v2.3.0 at FULL M-audit close)
⚠ FIRST: **/g-resume → decide W3** (`M-audit-2026-07.md` "Wave 3 — P2: Minors" is DEFERRABLE by design — execute only if worth it): either `/g-plan W3` (P2 minors: #6b git-alias resolution spike-or-scope, #23 doctor stray-doc allowlist, #24 H-testing native-app doc gap, session-start/observe/classifier polish items, combo-profile layer maps) or **skip straight to the release pass — v2.3.0 at FULL M-audit close**: README status strip (version badge + CHANGELOG link + ROADMAP link, high on page) + CHANGELOG `[2.3.0]` cut + version bump in BOTH `.claude-plugin/plugin.json` + `marketplace.json` (same commit) + post-release migration check (ADR-007 risk owner — first consumer update must show only new-version entries, no shim survivors). Developer call on W3 execute-vs-defer. 6 commits ahead of origin UNPUSHED — push at release pass or on developer nod. THE GATE IS LIVE (commit-detect.sh source-newer drift EXPECTED — gating-class deferral per ADR-008; its clone-first install rides the next gating checkpoint). Rollback baseline: `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-22\`. On lockout: `--no-verify`/hook-delete, restore snapshot, never debug live in-session. **ADR-006 implementation stays POST-M-audit.** stash@{0} superseded — droppable.
