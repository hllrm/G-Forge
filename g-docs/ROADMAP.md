...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | **W1 CLOSED 2026-07-22** (W1.1–W1.7 + W1.5a–g + W1.6 all shipped) @ v2.2.1 (v2.3.0 at FULL M-audit close)
⚠ FIRST: **ADR-007 fresh-window verify → /g-plan W2** (finding #19 shim retirement — sequencing constraint: amend g-skill-validate/g-skill-design/architecture rule BEFORE deleting shims; plus the W2-queued residuals on the W1.7ii ledger row: #21 heredoc-content false-positive look, journal `source`-field polish, PostToolUse-on-error characterization) → W3 → **release pass: v2.3.0 at FULL M-audit close (W1+W2+W3 — developer 2026-07-22; one milestone, one version)** with README status strip + CHANGELOG `[2.3.0]` cut. THE GATE IS LIVE AND BATTLE-PROVEN (see LATEST 2026-07-22b). Rollback baseline: `D:\SW_Projects\G-Forge-snapshots\claude-install-snapshot-2026-07-22\`. On lockout: `--no-verify`/hook-delete, restore snapshot, never debug live in-session. Do NOT re-plan anything in W1. **ADR-006 implementation stays POST-M-audit.** stash@{0} superseded — droppable.
