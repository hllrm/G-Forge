...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | W1.5a SHIPPED (reviewed MERGE READY r2, committed this pass) @ v2.2.1 (v2.2.2 pending milestone close)
⚠ FIRST: **`/g-plan` W1.5b** — continue the W1.5 split in order (W1.5b→f; slices in the M-audit section; standing rule: new minors route to W1.6/W2, never back into a slice) → W1.6 (tests for W1.3+W1.4; observe.sh sed escaped-quote item; newline-boundary + glued-pathspec-fidelity tests from W1.5a; ⚠ oversized, expect split) → W1.7 (live verify incl. FIRST REAL RUN of the stamped-sentinel + native pre-commit path — note W1.3/W1.4/W1.5a commits passed via the installed PreToolUse gate; the native hook is still not installed LOCALLY until /g-update runs here, which is W1.7 territory). Do NOT re-plan/re-implement W1.1–W1.5a. **ADR-006 implementation stays a POST-M-audit slice.** stash@{0} confirmed superseded (older snapshot of 5 W1.3 hook files) — droppable, developer's call.
...
- W1 (P0): ADR-004 (sentinel↔tree binding) + ADR-005 (worktree enforcement) implementation + finding #21 fold-in — 37 tasks / 8 waves, split into budget-scoped sub-parts (each sized to fit a session's `/g-plan` context-budget gate; sequenced 1→7, run `/g-plan` on each in order):
  ...
  - **W1.5 — Foundation + gate tests — SPLIT 2026-07-17 into W1.5a–f** (decomposed to 25 tasks / ~84 est. exchanges, far over one session's budget; approved split below — each slice is its own `/g-plan` run, sized ≤26 est. exchanges; the fail-before → fix → attest sandwich stays intact inside each slice; every test-writer suite is followed by a `g-forge-dev` attestation task per finding #20; standing rule: minors found during W1.5x reviews route to W1.6/W2, never back into a W1.5 slice):
    ...
    - **W1.5c — pre-commit gate fixtures** (~19): extend `g-dev/fixtures/pre-commit-gate-verify.sh` with doc-only-class pass/deny, conflicted-index write-tree-failure deny, ambiguous-worktree-resolution deny; attested run. Depends on: —
    - **W1.5d — sentinel-read extraction + install propagation** (~26, riskiest slice — deliberately alone): fail-before `tests/test-sentinel-read.sh` → extract the ADR-004 stamp parser from `workflow-checkpoint.sh` + `pre-commit` into shared `hooks/lib/sentinel-read.sh` (single reader — W1.3's strongest advisory) → pass-after attest; propagate the new lib into g-init copy table / g-update realign table / g-doctor Check 16 / README (11-file set → 12; W1.4-forecast scenario 2 mitigation folded in as an explicit task). Depends on: —
