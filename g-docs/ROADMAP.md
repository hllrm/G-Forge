...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · ADR-010 VERIFIED + committed b61191a · G-Proof 1.0 identity settled (2026-07-26)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **COMMITTED `b61191a`** (8 files, 251 insertions, tree clean, **unpushed**) through both gates: `/g-review` MERGE READY (independent full suite re-run — 9.6 min, 17/17 exit 0, **523 passed / 0 failed**) and `/g-doc-review` DOCS READY **on pass 5** (passes 1–4 held on 22 blocking findings, all fixed). Retro written: `g-docs/retros/2026-07-26-adr-010-verify.md`. · **ADR-010 VERIFIED AGAINST THE REPO — every in-repo claim holds. Build on it; do NOT re-verify.** The three load-bearing ones: (1) ADR-004's door-2-is-authoritative reading is exact (`hooks/pre-commit:4-12` in the file's own header; `ADR-004:14`/`:25` with the mechanism — PreToolUse fires pre-staging so `git write-tree` there hashes the wrong tree); (2) door 2 **is installed** at `.git/hooks/pre-commit`, executable, `diff`-identical to source, libs present — this was the assumption flagged as carrying the most weight; (3) **`/g-tier light` DOES disable the gate — EXECUTED**, scratch repo, real hook, real `git commit`, no sentinel: light ⇒ commit landed at both doors, full ⇒ blocked (door 2 exit 1 + stderr deny, door 1 exit 2 + deny JSON), balanced ⇒ blocked. Both doors read the tier independently (`hooks/pre-commit:131-138`, `hooks/check-commit.sh:158-170`) and door 1's deny message advertises the switch. **GF-43's retirement premise confirmed at the authoritative site.** Re-confirmed exact: 19 agents · 38 skills · 8 hooks · 6 libs · 56 profiles · single router · 23 doctor checks · parity 2.4.0/2.4.0 · no CI · ADR-008 Spike S1 cited correctly · installed `check-commit.sh` still deliberately behind source (ADR-008 hold) · §A7 thresholds still un-recalibrated (30/45 · 45/65). Platform-side assumptions (workflow scripts non-preview, GF-23, GF-20) remain unverified BY DESIGN — R0's job. · **VERSION IDENTITY SETTLED — G-Proof, first release 1.0. Never `3.0`.** ADR-010 had asserted "G-Proof 3.0" as *decided* while `g-docs/project_brief.md:54`'s standing 2026-07-18/19 row said 1.0 and explicitly forbade a `3.0.0` — and the 3.0 framing left T1's "sunset at G-Proof 1.0" unable to ever fire. Developer chose option A: the brief row is **reaffirmed, not amended**; what ADR-010 retires from the M44 plan is its **gating**, not its version identity. Renamed across ADR-010 + `transitional-rules.md`; **supersession stamps added** to the ROADMAP's M44 section, `g-docs/milestones/M44.md`, `g-docs/milestones/M41.md`, and the brief's naming row. · **T1 EXTRACTED — `g-docs/transitional-rules.md` is NEW and COMMITTED**, canonical; `CLAUDE.md` holds a pointer + `@`-import and no rule text. On-tripwire got structurally smaller (the fork carries the rule by git; only the import needs wiring). ⚠ **Off-tripwire recorded as NOT IMPLEMENTED:** `/g-trim`'s audit *subjects* are `CLAUDE.md` + agent memory (`skills/g-trim/SKILL.md:3`), so extraction moved T1 out of the subject set — it already reads `g-docs/project_brief.md` at `:16`, so a scope-add extends an existing path. R0 fork-checklist item. · **CLAUDE.md STAYS GITIGNORED — decided.** The negation carve-out was implemented, reviewed live, and **reversed by the developer.** Replaced by **THE INJECTION RULE**, recorded in `.gitignore`'s comment block above `/CLAUDE.md`: every load-bearing line must be (a) marker-fed from committed plugin source, (b) an `@`-import of a committed file, or (c) explicitly declared local-only (the voice-profile import is the one sanctioned (c)). **Bare prose asserting facts about the tree is the defect — fix the injection, never the .gitignore.** (Rejected en route: tracking the file would have published the personal voice-profile section by name to a public marketplace repo and left a dangling `@`-import in every clone.) · **SUITE TOTAL IS 523**, 17 suites, green. The attested **522 was correct and never "drift"** — it was the exact sum of `CLAUDE.md`'s per-suite table AS IT THEN STOOD, which agreed with its own headline (an earlier claim that the headline sat 4 above its table was itself a hand-arithmetic error; so was a "517" table sum, and so was calling 522 the sum of the *declared headers* — two suites declare none and one declares twice); `test-workflow-checkpoint.sh` had silently GAINED one assertion (header 80, runner 81), now fixed at `tests/test-workflow-checkpoint.sh:12` and re-verified by a single-suite run. An intermediate **"518" I published to four files came from mental arithmetic** and was refuted by `doc-reviewer` against the committed M46 attestation records. ⚠ **Premortem #2 fired before the fork, in the direction its tripwire does not watch** — the count moved **UP** silently; the tripwire watches only for a falling total. R0's regression defence must derive the total from the run and be **direction-agnostic**. · **CODE GATE FOUND A HOLE MY FIX DOESN'T COVER:** `tests/test-g-doctor-drift.sh` and `tests/test-worktree-resolve.sh` have **no `# Total assertions:` header at all** — 59 of 523 assertions with no committed detector, and their counts live only in gitignored `CLAUDE.md`. Also `tests/test-classify-changeset.sh:11` vs `:13` carry **two conflicting declarations (47 then 50)**; 50 is operative, but a first-match detector reads 47. · **GF-70 HAS LIVE EVIDENCE — the narrow reading is operative.** `bash` IS present on this host (`/usr/bin/bash`, GNU 5.2.37 msys) and every hook is invoked `bash -c '...'` (`.claude/settings.json:8`); what was broken was the **matcher** (`Bash` only, while the shell tool is PowerShell). "No bash on the host" is FALSE here — "Claude Code doesn't route through bash" is the real failure mode, the materially-different finding ADR-010 says may *strengthen* the rebuild.
Next up:          · **PUSH `b61191a` + this session-close commit.** · ⚠ **THEN FIRST TASK: write ADR-011 — the injection rule** (`/g-adr`). Flagged BLOCKING by `doc-reviewer`: it is a project-wide pattern that REPLACED an implemented-then-reversed approach, which G-RULES §G says must have an ADR, and the precedent is squarely in-class (ADR-007 retired command shims, ADR-008 set the working-tree split cadence). Deliberately deferred to a fresh window rather than written at the tail of a long session — §C makes finalizing a consequential ADR its own reset trigger. **The rationale is preserved meanwhile in `.gitignore`'s comment block above `/CLAUDE.md`**, including the rejected negation-carve-out alternative and its two costs (publishing the personal voice-profile reference to a public repo; a dangling `@`-import in every clone). If ADR-011 is judged unnecessary, record an explicit triage-downgrade in the style of `project_brief.md:54` — silence does not close it. · **THE ONE GATE, one discussion, roadmap written once.** (1) **R0 shape** — the tightening-pass cadence · what process governs G-Proof's construction (full-tier 2.5 explicitly NOT the assumed answer) · the fork checklist (wire the T1 `@`-import; the rule itself travels by git) · GF-20 spike · GF-23 verification (cheapest test, biggest leverage — gates `code-reviewer` DIES + `architecture-enforcer`'s conditional, **two rows not four**) · GF-70 test (seeded, see above) · whether ADR-008 Spike S1 still binds in a separate repo · the suite-erosion defence · scoped breakdown + cost counter-estimate. (2) **M-FORGE / M-PROOF SPLIT — developer intent clarified: plan G-Proof 1.0 *while implementing* v2.5.** Refined shape: the two tracks separate by **changeset class**, not by a new namespace — M-proof planning emits only `g-docs/` (doc-class), M-forge emits code, and `gf_classify_changeset` already routes them to different gates, so planning structurally cannot destabilise v2.5. **Two constraints:** (a) R0's spikes are NOT docs — they must run in a **throwaway spike clone** (a lab, never synced back) or the doc-only property collapses; (b) the real collision is the two shared records — one `g-docs/ROADMAP.md` and one handoff — which wants a **`Track:` field + `.claude/active-track`** (`/g-resume forge|proof`), **never two `## Active Session` blocks** (`workflow-checkpoint.sh:82` takes the first match with a greedy strip and would silently serve one forever). (3) **FORK TIMING — recommended: fork at the freeze, not early** (developer asked; not yet decided, and deciding early amends ADR-010's delivery shape). A continuously-synced fork pays merge tax scaling with how much has been transformed — by R2–R3 you merge v2.5 changes into a tree whose target component is deleted. A frozen parent is the cheapest thing to fork from. Post-freeze the one legitimate sync path is **cherry-pick-forward per bug fix**, never a standing merge — and answering "what earns a `2.5.x` patch vs 'fixed in G-Proof'" *is* answering how much forward-porting you're signing up for. (4) 8 Discuss rows (GF-20/23 transform shapes · GF-60/61 watch-vs-prototype · GF-63/77/78 posture). (5) client/cockpit milestone candidate. (6) `/g-intake` ×3 — telemetry fix-loop · TDD+doc-pass layer · g-train deepening. (7) three-horizon re-scope — **RUNS AFTER the rebuild plan**; M44/M41 stamps are placeholders until it does. (8) M44 detail calls (done-def rewrite · whether M41 stays a prereq · ride-alongs) + weekly platform-sync process + GF-40 ADR (invocation × scheduling, unwritten, independently gating) + the gate-hook clone-first checkpoint (raised in priority by the PowerShell matcher coupling). · **NEW INTAKE ROWS FROM THIS PASS:** (a) **`/g-patterns` derivable-counts row — **four concrete instances plus one category**: the 522/523 header, the two headerless suites, `test-classify-changeset.sh`'s dual declaration, the Check-16 lib enumeration (4 of 6; README mirrors it), and CLAUDE.md prose generally. One row, not five tickets. (b) **The injection rule's three teeth** — generated Quick-commands (counts from the run, not a keyboard) · a marker block for project prose so a lost CLAUDE.md is restored mechanically · **`/g-doctor` Check 24** flagging any CLAUDE.md content that is neither marker-fed, nor an import, nor declared local-only. (c) **Self-safe / at-risk state** (triaged this pass): `/g-init` Step 5a already ships the tracked-vs-ignored rule (`skills/g-init/SKILL.md:168`), so CLAUDE.md was this repo deviating from its own boundary — but `.claude/journal/` (the `/g-retro` source), `.claude/context-threshold-offset` and `.claude/telemetry-profile` are ignored **by shipped design** (`:177`), irreplaceable, and **GF-73 found the live destruction vector (`claude project purge`)**. Trigger must be session-end/pre-compact/purge-risk, **not** an N-milestone counter (the journal accrues daily); writer must NOT be `/g-doctor` (ADR-009 read-only); `g-tweak` does not exist — Safe self-tune is barred from the gate and sentinels, so don't hang it there. (d) **Reviewers should write their own sentinels** (developer principle — the verdict-holder holds the key). Half-implementable today: `code-lead` has Write, **`doc-reviewer` has none** and `/g-doc-review` Step 4 puts the write in the skill. Changes who holds the gate key ⇒ ADR-shaped. Note ADR-004's tree-binding already prevents stamping a *different* tree than reviewed, so this is a refinement, not a defect. (e) **Consumer-facing: the gate's own sentinel write can be denied by the auto-mode permission classifier.** It succeeded on retry here, but on a consumer machine that denial leaves `/g-review` unable to close after a legitimate MERGE READY — indistinguishable from G-Forge being broken. `/g-init` should install an explicit allow-rule alongside the hooks. (f) **user-defined voice profiles** (`/g-voice` hard-rejects anything outside `dev|mid|eli5` at `skills/g-voice/SKILL.md:20`; the local bridge works here only because CLAUDE.md is gitignored — a consumer has no equivalent hiding place, which is the point). (g) **FINDING** — a persona re-injected on EVERY prompt beat a document loaded ONCE; bears on how much `workflow-checkpoint.sh`'s per-prompt injection actually drives. · **FIVE ADVISORY WARNINGS remain live** (they were left unfixed at `b61191a` because editing after the verdict would have staled the ADR-004 tree stamp; a sixth — `ADR-010:88`'s "exact sum of declared headers" — was corrected in the session-close pass and is no longer outstanding): `project_brief.md:37` tranche row unmarked (covered by `:68`'s blanket) · `M44.md:3` stamp over-warns vs its own re-cut `:13`/`:88`/`:104` · `test-classify-changeset.sh:11` vs `:13` dual count · `transitional-rules.md:13` "copied" gloss vs git-carried fork · **the checkpoint's "verify ADR" nudge (`workflow-checkpoint.sh:376`) fires on spent entries** — matches at `ROADMAP:212`, `:406`, **and this advisory row itself**, so cleaning the two spent entries will not silence it. A fresh session is therefore told an ADR needs verifying while this handoff says ADR-010 is verified — **ignore that nudge unless a NEW ADR is genuinely pending** — the hook emits fixed text (`workflow-checkpoint.sh:377`) and never names one, so the nudge cannot tell you which entry matched. · **STILL OPEN, developer's call:** what lands in v2.5 before the freeze (currently only M41) · what earns a `2.5.x` patch vs "fixed in G-Proof". · Rider in `g-docs/todo.md`: g-wiki M46 refresh.
Active context:   · `main` @ v2.4.0, **clean tree, `b61191a` committed and UNPUSHED**. Both gate sentinels were consumed by `post-commit-cleanup.sh` (confirms the widened PowerShell PostToolUse matcher fires). **ADR-010 is the anchor and is VERIFIED — build on it, do not re-verify; if the checkpoint nudges "a handed-off ADR needs verifying", that is a spent-entry false positive, see Next up.** Real suite total = **523 / 17 suites / green** — always re-sum with `awk` from the run's own `Results:` lines, never by hand and never from prose (a hand-sum error published "518" to four files this pass). **INJECTION RULE IN FORCE** (`.gitignore` comment block above `/CLAUDE.md`): marker-fed from committed source, `@`-import of a committed file, or declared local-only; bare prose asserting facts about the tree is the defect. **Version identity: G-Proof, first release 1.0 — never `3.0`.** Arc order PENDING the gate and dead from M45 onward; M44/M41 carry ADR-010 supersession stamps that are placeholders until the post-rebuild-plan re-scope. LOCAL, NON-COMMITTED STATE a fresh session inherits silently (all gitignored): `.claude/settings.json` shell matcher = `Bash|PowerShell` · `.claude/hooks/workflow-checkpoint.sh` at source parity · `.claude/hooks/check-commit.sh` deliberately BEHIND source (ADR-008 hold — do not "fix" it casually, it is a decision) · `.claude/voice-profile` = `gian` + `.claude/voice-profiles/gian.md` + a voice `@`-import at the bottom of CLAUDE.md outside the markers · caveman off via `%APPDATA%\caveman\config.json`. REPORT-VS-REPO CORRECTIONS baked into triage (don't re-derive): 8 hook scripts + hooks.json + 6 libs, 19 agents, 38 skills, single router (ADR-007), 56 profile dirs, 23 doctor checks, SubagentStart/Stop registered. Backlog riders: g-doctor Check-16 lib enumeration stale · `command -v` hardening · fixture sleep-leak (a serial full-suite run takes ~9.6 min) · per-session counter accumulation · ADR-006 implementation. Format note: keep this line's leading label intact, use exactly the three §I labels (done-this-pass, next-up, active-context), and replace this block **wholesale** each pass — never append a second generation under a numbered or carried-over label variant. **Never repeat the active-context label text on this line:** `workflow-checkpoint.sh:82` strips with a greedy BRE, cutting through the LAST occurrence on the line, so a repeat makes the per-prompt banner serve this note instead of project state (observed live 2026-07-26). Constraint is once **per line**; `pre-compact.sh:74` uses awk block capture and is unaffected, so the two consumers disagree when it is violated.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
