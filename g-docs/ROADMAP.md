...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.0 · M47 PLANNING-PIPELINE HONESTY SHIPPED · 2026-08-12
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **M47 SHIPPED AND MERGED** (`6590b60` on main, pushed): decomposer same-file serial-chain sizing rule + scoped-Write output_file self-check (early M45 exception, todo task 6 amended) · /g-plan Step 2 validated per-run fallback + Step 3c **task-based review-chain cost term** (bands re-tuned with worked example; keyline's scales-with-code-touched honored) + `-split<N>` depth cap propagated into /g-roadmap templates (replace-not-append) · /g-forecast **outcome-corpus calibration** (Step 5b, N>=5 floor, bounded bidirectional, `mitigation-held:` half-credit with its /g-retro writer) · g-docs/forecasts/ promoted to commit content (formula input). Gate: 4 code-lead rounds + parallel cautious reviewer (HOLDs r1-r3 all fixed; r2 critical was a fix-minted stale fallback; r3/r4 converged on delta scoping), r4 MERGE READY, 6 minors swept pre-commit. Suite 564/564 attested twice on the final tree (mid-pass 2-test red = load flake on the abandoned-stdin timing pair, quiet re-run green x2). Retro: g-docs/retros/2026-08-12-m47-planning-honesty.md.
Next up:          · **MILESTONE CLOSE SWARM for M47, deferred by the context gate — run first**: /g-patterns · /g-telemetry · /g-align · /g-wiki refresh (fold task 1's widened scope) · /g-doctor odd-count check. Then **/g-plan M48** (folds todo task 5 test-teeth riders). · **NEW INTAKE:** abandoned-stdin GUARD_WINDOW_MS 20000 vs 24.3s worst-observed-under-load — repo's >=2x rule says revisit; decide raise-vs-quiet-runner-convention. · **CARRIED:** M38 delivery decision at its plan gate · announcement block order at release · field-reports/ registration at M38 · intake rows (a)-(h) · task 6 M45 rider (now incl. wave-planner Step 3 seam + 11 remaining agents) · task 7 remainder. · resolve pattern report (g-docs/patterns/latest.md, 3 PENDING) — check g-docs/inbox/adversarial/ first.
Active context:   · merge `6590b60` pushed and clean; the pass-close commit carrying this block follows it. Suite = **564 / 18 suites / green attested 2026-08-12 twice** (g-forge-dev runner, quiet machine, totals summed from per-suite table) — run timing-guard suites on a QUIET machine, parallel agent load flakes them. **ADR-010/011/012 all CURRENT — build on them, re-verify none**; checkpoint's ADR-verify nudge stays a spent-entry false positive. **Version identity: 2.5.0 last G-Forge release; G-Proof ships 1.0, never 3.0; M47 rode v2.5.0, no bump.** M47's shipped surfaces: agents/task-decomposer.md (Write scoped to own output file) · skills/g-plan/SKILL.md (Step 2 fallback, Step 3c review-chain term + depth cap) · skills/g-roadmap/SKILL.md (split templates) · skills/g-forecast/SKILL.md (Step 5b calibration) · skills/g-retro/SKILL.md (mitigation-held writer) · README.md (calibration :368, formula + cap :465/:625-628) · CHANGELOG.md (Unreleased). Review records: g-docs/agent-output/review/*2026-08-12*.md — code-lead r1-r4 + cautious r1-r3, 7 records, LOCAL-ONLY (gitignored, expensive to regenerate); the M47 commit carried no doc sentinel, doc coverage = the cautious reviewer's in-review backstop. LOCAL, NON-COMMITTED STATE (gitignored): voice profile `gian` · caveman ACTIVE · settings matcher `Bash|PowerShell` · .claude/hooks + rules synced 2026-08-11 · review-holds = 13 (3 HOLDs this pass). REPORT-VS-REPO (don't re-derive): 8 hook scripts + 6 libs, 19 agents, 38 skills, single router, 25 doctor checks. Backlog riders: fixture sleep-leak · ADR-006 implementation · Check-24 bare-prose backlog · stale per-session counter files in .claude/ (reaping rider). Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass — never append a second generation, and never repeat this line's own leading label text anywhere else on it (workflow-checkpoint.sh strips with a greedy BRE through the LAST occurrence; pre-compact.sh uses awk block capture — the two consumers disagree when violated, observed live 2026-07-26).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
