...
2. **Slug collision handling** — whether re-opening an already-open slug is a hard fail or silently accepted. Unspecified; an implementer must decide before shipping, or the state machine may pair opener A with closer B. — resolved: rider (iii) (`skills/g-doctor/SKILL.md:291`) makes re-opening a hard unpaired-marker finding — opener-A is never silently paired with a later closer-B.
3. **Unterminated-marker terminal state** — as specced, an opener with no closer would leave the state machine in `LOCAL`/`MARKER` through EOF, silently reclassifying everything after it. The implementation needs an explicit end-of-file check rather than folding into the BARE-PROSE bucket. — resolved: rider (iv) (`skills/g-doctor/SKILL.md:292`) adds the explicit end-of-file check as a named unterminated-marker-at-eof finding.
