...
|---|---|
| `hooks/workflow-checkpoint.sh` | Reads only the `Active context:` line (`workflow-checkpoint.sh:97`, `grep -m1`) and block existence (`:387`). The Step-10 bullet lands under `Next up:` — no parsing impact. The protection both hook consumers rely on is g-patterns Step 10's CRITICAL in-place-edit constraint (never a second `## Active Session` block), which is already written into the skill. |
