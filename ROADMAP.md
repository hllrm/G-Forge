...
Next up:          · STRAYS (none blocking 2.0.0): (1) LICENSE is the GPL-3.0 *notice*, not full text — proxy blocked gnu.org; fetch the full ~674-line gpl-3.0.txt with working network so GitHub detects the license. (2) ROADMAP skeleton mismatch (now more relevant since /g-init is the front door): /g-init scaffolds `## Current Milestone`/`## Backlog`/`## Done` but /g-roadmap writes `## Milestones`/`### MN` (and this repo's own ROADMAP uses `## Milestones`) — pick one skeleton and align g-init + g-roadmap. (3) Milestone status markers disagree: /g-init uses 🚧, /g-roadmap uses ⬜/🔄/✅, /g-resume + /g-status read 🔄/🚧 — standardize one set across all four. (4) Verify the new /g-init front-door flow end-to-end on a real repo (existing-codebase → onboard, greenfield → kickoff). (5) Open the v2.0.0 PR to main (not yet opened — needs explicit go).
