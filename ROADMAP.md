...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · v2.0.0 (M23) production-readiness audit + follow-ons: hooks reconciled to one canonical 7-hook set with single-registrar + self-guard (manifest registers none; guard on .claude/integration-tier; idempotent install; g-doctor dupe + legacy-plugin checks) · legacy "G-Team" purged · tests/ split out · renamed docs/ → g-docs/ (no collision with project docs) · g-init reviewed/hardened · handoff unified on ROADMAP ## Active Session (todo.md = tasks only; /g-retro owns the write) · killed /g-team→/g-forge command duplication (removed alias, completed router, migration detection in /g-update + /g-doctor) · GPL-3.0 declared in README · /g-help <topic> + archive map · /g-roadmap premortem+reprio on M change · NEW /g-wiki skill (committed g-wiki/ project wiki, refreshed each milestone)
Next up:          · Optional: replace the stub LICENSE with full GPL-3.0 text (proxy blocked gnu.org — needs a working fetch). Otherwise M24 open; consider opening the 2.0.0 PR to main.
Active context:   · v2.0.0 on branch claude/m23-release-u3rx0d. Name kept as G-Forge. Hooks: single registrar (.claude/settings.json via /g-init), manifest registers none, all 7 self-guard on .claude/integration-tier. G-Forge's docs namespace is now g-docs/ (plans, decisions, retros, agent-output, forecasts, …) so it never mixes with a project's docs/. Counts: 17 agents · 36 skills · 37 commands (36 skill-backed + /g-forge router; /g-wiki added, deprecated /g-team alias removed in 2.0) · 56 profiles (48 stack + 7 combo + 1 supplementary).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
